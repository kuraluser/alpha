...
/**
 * Class to perform common confirm task functions
 */
@Injectable()
export class CommonConfirmTaskService {
    ...
    constructor(
        ...
        this.logger.setContext('CommonConfirmTaskService');
    }

    /**
     * Method to recursively confirm data sending using websockets Socket.IO library
     *
     * @param eventData
     */
    public confirmDataTransfer(eventData: InboundEventData, taskCompleted: TaskCompleted): void {
        try {
            ...
            if (this.ioClient && this.ioClient.connected) {
                ...
                this.ioClient.emit('confirm', dataToSend);
            } else {
                this.logger.warn('Error socket connection');
            }

        } catch (err) {
            this.logger.error(`Error in confirm request ${err.message}`, err.stack);
        } finally {
            (async () => {
                ...
                const updateResult: UpdateResult = await this.commonDbService.updateData(this.commonDbStore.getInboundEventStore(),
                    ...
                    { confirmReadTimeStamp: new Date().getTime() });
                if (updateResult.affected === 0) {
                    this.logger.error(`Error in updating confirm read timestamp for the client ${eventData.clientId} 
