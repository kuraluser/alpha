...
import { ISequenceDataResponse } from '../loading-discharging-sequence-chart/loading-discharging-sequence-chart.model';
...
export class LoadingDischargingSequenceApiService {
  ...
  getSequenceData(vesselId: number, voyageId: number, infoId: number, operation: OPERATIONS): Observable<ISequenceDataResponse> {
    ...
    apiUrl += operation === OPERATIONS.LOADING ? `loading-info/${infoId}/loading-sequence` : `discharge-info/${infoId}/discharging-sequence`;
