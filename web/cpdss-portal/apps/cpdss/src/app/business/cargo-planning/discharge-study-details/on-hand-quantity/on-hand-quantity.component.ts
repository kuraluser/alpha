...
export class OnHandQuantityComponent implements OnInit, OnDestroy {
  ...
  set selectedPortOHQTankDetails(selectedPortOHQTankDetails: IPortOHQTankDetailValueObject[]) {
    this._selectedPortOHQTankDetails = selectedPortOHQTankDetails.map((ohqTankDetail, index) => {
      const _ohqTankDetail = this.dischargeStudyDetailsTransformationService.formatOHQTankDetail(ohqTankDetail);
      return _ohqTankDetail;
    });
    this.dischargeStudyDetailsTransformationService.setOHQPortValidity(this.selectedPort.id, this.ohqGroupValidity(this._selectedPortOHQTankDetails));
  }
  ...
  constructor(private dischargeStudyDetailsApiService: DischargeStudyDetailsApiService,
    ...
    public dischargeStudyDetailsTransformationService: DischargeStudyDetailsTransformationService,
   ...
   */
  ngOnInit(): void {
    this.columns = this.dischargeStudyDetailsTransformationService.getOHQDatatableColumns();
  ...
  async getPortOHQDetails(portRotationId: number) {
    const result = await this.dischargeStudyDetailsApiService.getPortOHQDetails(this.vesselId, this.voyageId, this.dischargeStudyId, portRotationId).toPromise();
    ...
    const _selectedPortOHQTankDetails = selectedPortOHQTankDetails?.map((ohqTankDetail) => {
      ...
      const _ohqTankDetail = this.dischargeStudyDetailsTransformationService.getOHQTankDetailsAsValueObject(ohqTankDetail, false);
      return _ohqTankDetail;
    });
  ...
  async checkForPendingUpdates(): Promise<boolean> {
    const count = await this.dischargeStudyDetailsApiService.getOHQTankDetailsPendingUpdatesCount(this.vesselId, this.voyageId, this.dischargeStudyId);
    if (!count) {
      return false;
    } else {
      this.ngxSpinnerService.hide();
  ...
  async onEditComplete(event: IPortOHQTankDetailEvent) {
    ...
    this.ohqForm.updateValueAndValidity();
    this.dischargeStudyDetailsTransformationService.setOHQPortValidity(this.selectedPort.id, this.ohqGroupValidity(this.selectedPortOHQTankDetails));
  ...
  getSelectedPortOHQTankDetailsAsValue(selectedPortOHQTankDetails: IPortOHQTankDetailValueObject[]) {
    return selectedPortOHQTankDetails.map(ohqTankDetails => this.dischargeStudyDetailsTransformationService.getOHQTankDetailAsValue(ohqTankDetails));
  }
