...
export class AddPortComponent implements OnInit {
  ...
  initForm(portData: IPortDetails) {
    this.addPortDetailsForm = this.fb.group({
      ...
      tideHeightHigh: this.fb.control(portData?.tideHeightHigh, [Validators.required, Validators.max(30), numberValidator(2, 2, false)]),
      tideHeightLow: this.fb.control(portData?.tideHeightLow, [Validators.required, Validators.max(0), numberValidator(2, 2, false)]),
      maxPermissibleDraft: this.fb.control(portData?.maxPermissibleDraft, [Validators.required, numberValidator(2, 4, false)]),
      densityOfWater: this.fb.control(portData?.densityOfWater, [Validators.required, numberValidator(3, 1, false)]),
