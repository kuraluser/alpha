...
export class AddPortComponent implements OnInit {
  ...
  initForm(portData: IPortDetails) {
    this.addPortDetailsForm = this.fb.group({
      ...
      tideHeightHigh: this.fb.control(portData?.tideHeightHigh, [Validators.required, Validators.max(30), numberValidator(2, 4, false)]),
      tideHeightLow: this.fb.control(portData?.tideHeightLow, [Validators.required, Validators.max(0), numberValidator(2, 4, false)]),
