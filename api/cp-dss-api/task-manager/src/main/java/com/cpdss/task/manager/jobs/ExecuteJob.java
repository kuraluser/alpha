...
public class ExecuteJob extends QuartzJobBean {
    ...
    protected void executeInternal(final JobExecutionContext context) throws JobExecutionException {
        ...
        try {
            ...
            channel = ManagedChannelBuilder.forTarget("static://" + detail.getJobDataMap().getString(TASK_URL))
                    .usePlaintext()
                    .build();
            Optional<SchedulerJobInfo> schedulerJobInfoOptional = schedulerRepository.findByTaskName(detail.getKey().getName());
            if (!schedulerJobInfoOptional.isPresent()) {
                throw new JobExecutionException("Job not found with the name " + detail.getKey().getName());
            }
            Map<String, String> requestParams = gson.fromJson(detail.getJobDataMap().getString(REQUEST_BODY), Map.class);
            ...
            ScheduledTaskProperties.TaskTypeEnum taskTypeEnum = ScheduledTaskProperties.TaskTypeEnum.valueOf(schedulerJobInfoOptional.get().getTaskType());
            switch (taskTypeEnum) {
                case SYNC:
                    TaskManagerServiceGrpc.TaskManagerServiceBlockingStub taskManagerBlockingService = TaskManagerServiceGrpc.newBlockingStub(channel);
                    TaskManager.ExecuteTaskReply executeTaskReply = taskManagerBlockingService.executeTask(builder.build());
                    ...
                    if (responseStatus.getStatus().contentEquals(Utils.STATUS.FAILED.name())) {
                        ...
                        throw new GenericServiceException(responseStatus.getMessage(), responseStatus.getCode(), HttpStatusCode.INTERNAL_SERVER_ERROR);
                    }
                    log.info(JOB_COMPLETED + detail.getKey());
                    break;
                case ASYNC:
                    final JobKey asycJobKey = detail.getKey();
                    TaskManagerServiceGrpc.TaskManagerServiceFutureStub taskManagerNonBlockingService = TaskManagerServiceGrpc.newFutureStub(channel);
                    ListenableFuture<TaskManager.ExecuteTaskReply> executeTaskReplyListener = taskManagerNonBlockingService.executeTask(builder.build());
                    executeTaskReplyListener.addListener(() -> {
                        try {
                            if(!executeTaskReplyListener.isDone()){
                                return;
                            }
                            Common.ResponseStatus asyncResponseStatus = executeTaskReplyListener.get().getResponseStatus();
                            if (asyncResponseStatus.getStatus().contentEquals(Utils.STATUS.FAILED.name())) {
                                log.error("Task Execution has failed with message" + asyncResponseStatus.getMessage());
                                throw new GenericServiceException(asyncResponseStatus.getMessage(), asyncResponseStatus.getCode(), HttpStatusCode.INTERNAL_SERVER_ERROR);
                            }
                    ...
                    break;
                default:
                    throw new JobExecutionException("Unknown task type for the task with name " + detail.getKey().getName());
            }
        } catch (Exception e) {
