...
//			com.cpdss.loadablestudy.domain.LoadableStudy loadableStudy = new com.cpdss.loadablestudy.domain.LoadableStudy();

			DischargeStudyAlgoJson AlgoJsonPayload = this.generateJson(request.getLoadableStudyId(),loadableStudyOpt.get());
			...
			replyBuilder.setResponseStatus(Common.ResponseStatus.newBuilder().setStatus(FAILED)
					.setMessage(INVALID_LOADABLE_STUDY_ID).setCode(CommonErrorCodes.E_HTTP_BAD_REQUEST).build());
		}
		return replyBuilder;
	}
	...
	public DischargeStudyAlgoJson generateJson(Long dischargeStudyId, LoadableStudy loadableStudy) throws GenericServiceException {
		...
		DischargeStudyAlgoJson dischargeStudyAlgoJson = new DischargeStudyAlgoJson();
			...
			dischargeStudyAlgoJson
					.setLoadablePlanPortWiseDetails(getLoadablePlanPortWiseDetails(dischargeStudyId)); //TODO
	...
	private LoadablePatternJson getLoadablePlanPortWiseDetails(Long dischargeStudyId)
		...
		if (loadablePatternReply.getResponseStatus().getStatus() != SUCCESS) {
			...
			LoadablePattern loadablePattern = loadablePatternReply.getLoadablePatternList().get(0); // TBD TODO
	...
	private List<LoadablePlanBallastDetails> getLoadablePlanBallastDetails(
			List<com.cpdss.common.generated.LoadableStudy.LoadablePlanBallastDetails> loadablePlanBallastDetails) {
		List<LoadablePlanBallastDetails> ballastDetailsList = new ArrayList<>();
		loadablePlanBallastDetails.forEach(item -> {
...
//			ofNullable(item.get).ifPresent(ballastDetails::setFillingRatio); TBD TODO
	...
	private List<LoadablePlanStowageDetailsJson> getLoadablePlanStowageDetails(
		...
		loadablePlanStowageDetails.forEach(item -> {
			...
			ofNullable(item.getCargoNominationTemperature()).ifPresent(stowageDetails::setCargoNominationTemperature);
