...
public class CargoNominationService {
	...
	@Autowired
	private CommingleCargoRepository commingleCargoRepository;

	@Autowired
	private ApiTempHistoryRepository apiTempHistoryRepository;
	...
	@Autowired
	CargoOperationRepository cargoOperationRepository;
	...
	@Autowired
	private CargoNominationValveSegregationRepository valveSegregationRepository;
	...
	public List<CargoNomination> getCargoNominationByLoadableStudyId(Long loadableStudyId)
		...
		if (cargos.isEmpty()) {
			throw new GenericServiceException("cargo nomination data missing", CommonErrorCodes.E_HTTP_BAD_REQUEST,
	...
	public List<CargoNomination> getCargoNominations(Long loadableStudyId) {
		List<CargoNomination> cargos = cargoNominationRepository.findByLoadableStudyXIdAndIsActive(loadableStudyId,
				true);
		return cargos;
	}

	public List<CargoNomination> saveDsichargeStudyCargoNominations(Long dischargeStudyId, Long loadableStudyId,
			Long portId) throws GenericServiceException {
		...
		cargos.parallelStream().forEach(cargo -> {
	...
	public CargoNomination createDsCargoNomination(Long dischargeStudyId, CargoNomination cargo, Long portId) {
		...
		dischargeStudyCargo
				.setCargoNominationPortDetails(createCargoNominationPortDetails(dischargeStudyCargo, cargo, portId));
		dischargeStudyCargo.setMode(2L);
		return dischargeStudyCargo;
	}

	public Set<CargoNominationPortDetails> createCargoNominationPortDetails(CargoNomination dischargeStudyCargo,
			CargoNomination cargo, Long portId) {
		...
		if (cargo != null) {
			portDetail.setQuantity(cargo.getCargoNominationPortDetails().stream()
					.map(CargoNominationPortDetails::getQuantity).reduce(BigDecimal.ZERO, BigDecimal::add));
		} else {
	...
	public LoadableStudy.CargoNominationReply.Builder saveCargoNomination(LoadableStudy.CargoNominationRequest request,
			LoadableStudy.CargoNominationReply.Builder cargoNominationReplyBuilder) throws GenericServiceException {
		Optional<com.cpdss.loadablestudy.entity.LoadableStudy> loadableStudy = this.loadableStudyRepository
				.findByIdAndIsActive(request.getLoadableStudyId(), true);
		...
		if (request.getCargoNominationDetail() != null && request.getCargoNominationDetail().getId() != 0) {
			Optional<CargoNomination> existingCargoNomination = this.cargoNominationRepository
					.findByIdAndIsActive(request.getCargoNominationDetail().getId(), true);
			if (!existingCargoNomination.isPresent()) {
				throw new GenericServiceException("Cargo Nomination does not exist",
			...
			if (!CollectionUtils.isEmpty(cargoNomination.getCargoNominationPortDetails())) {
				existingCargoPortIds = cargoNomination.getCargoNominationPortDetails().stream()
						.map(CargoNominationPortDetails::getPortId).collect(Collectors.toList());
			}
		...
		} else if (request.getCargoNominationDetail() != null && request.getCargoNominationDetail().getId() == 0) {
		...
		if (!cargoNomination.getCargoNominationPortDetails().isEmpty()) {
			List<Long> requestedPortIds = cargoNomination.getCargoNominationPortDetails().stream()
					.map(CargoNominationPortDetails::getPortId).collect(Collectors.toList());
		...
		if (request.getCargoNominationDetail() != null) {
			loadableStudyRecord
					.setIsCargoNominationComplete(request.getCargoNominationDetail().getIsCargoNominationComplete());
		...
		cargoNominationReplyBuilder.setResponseStatus(Common.ResponseStatus.newBuilder().setStatus(SUCCESS))
				.setCargoNominationId((cargoNomination.getId() != null) ? cargoNomination.getId() : 0);
		return cargoNominationReplyBuilder;
	}

	private CargoNomination buildCargoNomination(CargoNomination cargoNomination,
			LoadableStudy.CargoNominationRequest request, com.cpdss.loadablestudy.entity.LoadableStudy loadableStudy)
			throws GenericServiceException {
		if (!request.getCargoNominationDetail().getLoadingPortDetailsList().isEmpty()) {
			List<Long> requestedPortIds = request.getCargoNominationDetail().getLoadingPortDetailsList().stream()
					.map(LoadableStudy.LoadingPortDetail::getPortId).collect(Collectors.toList());
			List<Long> transitPorts = this.loadableStudyPortRotationRepository.getTransitPorts(loadableStudy,
					requestedPortIds);
			if (!CollectionUtils.isEmpty(transitPorts)) {
				throw new GenericServiceException(
						"Ports exist as transit ports " + StringUtils.collectionToCommaDelimitedString(transitPorts),
						CommonErrorCodes.E_CPDSS_TRANSIT_PORT_EXISTS, HttpStatusCode.BAD_REQUEST);
			}
		}
		...
		cargoNomination.setQuantity(!StringUtils.isEmpty(request.getCargoNominationDetail().getQuantity())
		...
		if (!request.getCargoNominationDetail().getLoadingPortDetailsList().isEmpty()) {
			...
			Set<CargoNominationPortDetails> cargoNominationPortDetailsList = request.getCargoNominationDetail()
					.getLoadingPortDetailsList().stream().map(loadingPortDetail -> {
						...
						cargoNominationPortDetails.setQuantity(!loadingPortDetail.getQuantity().isEmpty()
	...
	private ApiTempHistory buildApiTempHistory(CargoNomination cargoNomination,
			LoadableStudy.CargoNominationRequest request, List<Long> existingCargoPortIds) {

		Long portId = null;
		if (existingCargoPortIds != null && existingCargoPortIds.size() > 0) {
			portId = existingCargoPortIds.stream().findFirst().get();
		}
		return ApiTempHistory.builder().vesselId(request.getVesselId()).cargoId(cargoNomination.getCargoXId())
				.loadingPortId(portId != null ? portId : null).api(cargoNomination.getApi()).isActive(true)
				.temp(cargoNomination.getTemperature()).build();
	}

	/**
	 * Fetch ports for the specific loadableStudy already available in port rotation
	 * and update to port rotation along with port master fields water density,
	 * maxDraft, maxAirDraft only those ports that are not already available
	 *
	 * @param cargoNomination
	 * @throws GenericServiceException
	 */
	private void updatePortRotationWithLoadingPorts(com.cpdss.loadablestudy.entity.LoadableStudy loadableStudy,
			CargoNomination cargoNomination, List<Long> existingCargoPortIds) throws GenericServiceException {
		List<LoadableStudyPortRotation> loadableStudyPortRotations = this.loadableStudyPortRotationRepository
				.findByLoadableStudyAndOperationAndIsActive(loadableStudy,
						cargoOperationRepository.getOne(LOADING_OPERATION_ID), true);
		List<Long> requestedPortIds = null;
		List<Long> existingPortIds = null;
		if (!cargoNomination.getCargoNominationPortDetails().isEmpty()) {
			requestedPortIds = cargoNomination.getCargoNominationPortDetails().stream()
					.map(CargoNominationPortDetails::getPortId).collect(Collectors.toList());
		}
		if (!loadableStudyPortRotations.isEmpty()) {
			existingPortIds = loadableStudyPortRotations.stream().map(LoadableStudyPortRotation::getPortXId)
		...
		if (!CollectionUtils.isEmpty(requestedPortIds) && !CollectionUtils.isEmpty(existingCargoPortIds)) {
			...
			if (!CollectionUtils.isEmpty(existingCargoPortIds)) {
				existingCargoPortIds.forEach(existingPortId -> {
					Long otherCargoRefExistCount = this.cargoNominationRepository.getCountCargoNominationWithPortIds(
					...
					if (Objects.equals(otherCargoRefExistCount, Long.valueOf("0"))) {
						loadableStudyPortRotationRepository.deleteLoadingPortRotationByPort(loadableStudy,
								existingPortId);
						synopticalTableRepository.deleteSynopticalPorts(loadableStudy.getId(), existingPortId);
						onHandQuantityRepository.deleteByLoadableStudyAndPortXId(loadableStudy, existingPortId);
					}
				});
			}
		}
		...
		if (!CollectionUtils.isEmpty(requestedPortIds)) {
			...
			// attributes
			this.loadableStudyPortRotationRepository.findByLoadableStudyAndIsActive(loadableStudy.getId(), true)
		...
		loadableStudyPortRotations.forEach(portRotation -> {
	 ...
	 * @param cargoNomination
	 */
	private void buildGetPortInfoByPortIdsRequest(PortInfo.GetPortInfoByPortIdsRequest.Builder reqBuilder,
			CargoNomination cargoNomination) {
		// build fetch port details request object
		if (cargoNomination != null && !CollectionUtils.isEmpty(cargoNomination.getCargoNominationPortDetails())) {
			cargoNomination.getCargoNominationPortDetails().forEach(loadingPort -> {
	...
	/**
	 * Create Port rotation entities for each loading port from cargoNomination with
	 * pre-populate port master attributes
	 *
	 * @param loadableStudy
	...
	private void buildAndSaveLoadableStudyPortRotationEntities(
		...
		if (!CollectionUtils.isEmpty(requestedPortIds) && portReply != null
			...
			requestedPortIds.stream().forEach(requestedPortId -> portReply.getPortsList().stream()
					.filter(port -> Objects.equals(requestedPortId, port.getId())).forEach(port -> {
						...
						portRotationEntity.setOperation(this.cargoOperationRepository.getOne(LOADING_OPERATION_ID));
						portRotationEntity.setSeaWaterDensity(
								!StringUtils.isEmpty(port.getWaterDensity()) ? new BigDecimal(port.getWaterDensity())
										: null);
						...
						portRotationEntity.setAirDraftRestriction(
								!StringUtils.isEmpty(port.getMaxAirDraft()) ? new BigDecimal(port.getMaxAirDraft())
										: null);
						portRotationEntity.setPortOrder(atomLong.incrementAndGet());
						// add ports to synoptical table by reusing the function called by
						// port-rotation flow
						synopticService.buildPortsInfoSynopticalTable(portRotationEntity, LOADING_OPERATION_ID,
								port.getId());
	...
	public LoadableStudy.CargoNominationReply.Builder getCargoNominationById(
			LoadableStudy.CargoNominationRequest request, LoadableStudy.CargoNominationReply.Builder replyBuilder)
			throws GenericServiceException {
		Optional<com.cpdss.loadablestudy.entity.LoadableStudy> loadableStudyOpt = this.loadableStudyRepository
				.findById(request.getLoadableStudyId());
		if (!loadableStudyOpt.isPresent()) {
			throw new GenericServiceException("Loadable study does not exist", CommonErrorCodes.E_HTTP_BAD_REQUEST,
					null);
		}
		List<CargoNomination> cargoNominationList = this.cargoNominationRepository
				.findByLoadableStudyXIdAndIsActiveOrderByCreatedDateTime(request.getLoadableStudyId(), true);
	...
	private void buildCargoNominationReply(List<CargoNomination> cargoNominationList,
			List<ApiTempHistory> apiTempHistoriesAll,
			com.cpdss.common.generated.LoadableStudy.CargoNominationReply.Builder cargoNominationReplyBuilder) {
		if (!CollectionUtils.isEmpty(cargoNominationList)) {
			cargoNominationList.forEach(cargoNomination -> {
				LoadableStudy.CargoNominationDetail.Builder builder = LoadableStudy.CargoNominationDetail.newBuilder();
				ofNullable(cargoNomination.getId()).ifPresent(builder::setId);
				ofNullable(cargoNomination.getLoadableStudyXId()).ifPresent(builder::setLoadableStudyId);
				...
				Optional.ofNullable(cargoNomination.getApi()).ifPresent(val -> builder.setApi(String.valueOf(val)));
				...
				if (!CollectionUtils.isEmpty(cargoNomination.getCargoNominationPortDetails())) {
					cargoNomination.getCargoNominationPortDetails().forEach(loadingPort -> {
						LoadableStudy.LoadingPortDetail.Builder loadingPortDetailBuilder = LoadableStudy.LoadingPortDetail
								.newBuilder();
						ofNullable(loadingPort.getPortId()).ifPresent(loadingPortDetailBuilder::setPortId);
				...
				if (!CollectionUtils.isEmpty(cargoNomination.getCargoNominationPortDetails())) {
					CargoNominationPortDetails cargoNominationPortDetail = cargoNomination
							.getCargoNominationPortDetails().iterator().next();
					if (cargoNomination.getCargoXId() != null && cargoNominationPortDetail.getPortId() != null) {
						...
						List<ApiTempHistory> apiTempHistories = apiTempHistoryRepository
								.findByLoadingPortIdAndCargoIdAndIsActiveOrderByCreatedDateTimeDesc(
										cargoNominationPortDetail.getPortId(), cargoNomination.getCargoXId(), true);
				...
				if (!CollectionUtils.isEmpty(apiTempHistoriesAll)) {
					...
					apiTempHistoriesAll.forEach(apiTempHistory -> {
						LoadableStudy.CargoHistoryDetail.Builder cargoBuilder = LoadableStudy.CargoHistoryDetail
								.newBuilder();

						Optional.ofNullable(apiTempHistory.getCargoId()).ifPresent(cargoBuilder::setCargoId);

						Optional.ofNullable(apiTempHistory.getVesselId()).ifPresent(cargoBuilder::setVesselId);

						Optional.ofNullable(apiTempHistory.getLoadingPortId())
								.ifPresent(cargoBuilder::setLoadingPortId);

						if (apiTempHistory.getApi() != null) {
							Optional.ofNullable(apiTempHistory.getApi().toString()).ifPresent(cargoBuilder::setApi);
						}
	...
	public String getMaxQuantityFromBillOfLadding(Long cargoNominationId) {
		log.info("Getting max quantity of each cargo nomination, from loading plan {}", cargoNominationId);
		BillOfLaddingRequest request = BillOfLaddingRequest.newBuilder().setCargoNominationId(cargoNominationId)
				.build();
		LoadingInformationSynopticalReply reply = loadingPlanGrpcService.getBillOfLaddingDetails(request);
		if (SUCCESS.equals(reply.getResponseStatus().getStatus())
			...
			if (reply.getBillOfLaddingList().size() == 1) {
				...
				return String.valueOf(reply.getBillOfLaddingList().stream().map(item -> new BigDecimal(item.getQuantityMt()))
	...
	public LoadableStudy.ValveSegregationReply.Builder getValveSegregation(
			LoadableStudy.ValveSegregationRequest request, LoadableStudy.ValveSegregationReply.Builder reply) {
		Iterable<CargoNominationValveSegregation> segregationsList = valveSegregationRepository.findAll();
		segregationsList.forEach(segregation -> {
			LoadableStudy.ValveSegregation.Builder segregationDetail = LoadableStudy.ValveSegregation.newBuilder();
	...
	public LoadableStudy.CargoNominationReply.Builder deleteCargoNomination(
			...
			LoadableStudy.CargoNominationReply.Builder cargoNominationReplyBuilder) throws GenericServiceException {
		Optional<CargoNomination> existingCargoNomination = this.cargoNominationRepository
				.findById(request.getCargoNominationId());
		if (!existingCargoNomination.isPresent()) {
			throw new GenericServiceException("Cargo Nomination does not exist", CommonErrorCodes.E_HTTP_BAD_REQUEST,
					HttpStatusCode.BAD_REQUEST);
		}
		this.validateDeleteCargoNomination(existingCargoNomination.get());

		this.commingleCargoRepository
				.deleteCommingleCargoByLodableStudyXId(existingCargoNomination.get().getLoadableStudyXId());
		Optional<com.cpdss.loadablestudy.entity.LoadableStudy> loadableStudyOpt = this.loadableStudyRepository
				.findById(existingCargoNomination.get().getLoadableStudyXId());
		if (loadableStudyOpt.isPresent()) {
		...
		this.cargoNominationOperationDetailsRepository.deleteCargoNominationPortDetails(request.getCargoNominationId());
		/*
		...
		if (!existingCargoNomination.get().getCargoNominationPortDetails().isEmpty()) {
			List<Long> requestedPortIds = existingCargoNomination.get().getCargoNominationPortDetails().stream()
					.map(CargoNominationPortDetails::getPortId).collect(Collectors.toList());
			if (!CollectionUtils.isEmpty(requestedPortIds)) {
				requestedPortIds.forEach(requestPortId -> {
					Long otherCargoRefExistCount = this.cargoNominationRepository.getCountCargoNominationWithPortIds(
							existingCargoNomination.get().getLoadableStudyXId(), existingCargoNomination.get(),
							requestPortId);
					if (Objects.equals(otherCargoRefExistCount, Long.valueOf("0")) && loadableStudyOpt.isPresent()) {
						loadableStudyPortRotationRepository.deleteLoadingPortRotationByPort(loadableStudyOpt.get(),
								requestPortId);
						synopticalTableRepository.deleteSynopticalPorts(loadableStudyOpt.get().getId(), requestPortId);
	...
	void validateDeleteCargoNomination(CargoNomination cargoNomination) throws GenericServiceException {
		Optional<com.cpdss.loadablestudy.entity.LoadableStudy> entityOpt = this.loadableStudyRepository
				.findById(cargoNomination.getLoadableStudyXId());
	...
	public LoadableStudy.CargoNominationDetailReply.Builder getCargoNominationByCargoNominationId(
			LoadableStudy.CargoNominationRequest request, LoadableStudy.CargoNominationDetailReply.Builder builder) {
		CargoNomination cargoNomination = this.cargoNominationRepository.getOne(request.getCargoNominationId());
		if (cargoNomination != null) {
			com.cpdss.common.generated.LoadableStudy.CargoNominationDetail.Builder cargoNom = com.cpdss.common.generated.LoadableStudy.CargoNominationDetail
					.newBuilder();
			...
			com.cpdss.common.generated.CargoInfo.CargoDetailReply cargoDetailReply = this
					.getCargoInfoById(cargoReq.build());
	...
	public com.cpdss.common.generated.CargoInfo.CargoDetailReply getCargoInfoById(CargoInfo.CargoRequest build) {
		return cargoInfoGrpcService.getCargoInfoById(build);
	}

	/** @param loadableStudy void */
	public void validateLoadableStudyWithCommingle(com.cpdss.loadablestudy.entity.LoadableStudy loadableStudy)
			throws GenericServiceException {
		List<CargoNomination> cargoNominations = cargoNominationRepository
				.findByLoadableStudyXIdAndIsActive(loadableStudy.getId(), true);
		List<com.cpdss.loadablestudy.entity.CommingleCargo> commingleCargos = commingleCargoRepository
				.findByLoadableStudyXIdAndPurposeXidAndIsActive(loadableStudy.getId(), 2L, true);
		if (!cargoNominations.isEmpty() && !commingleCargos.isEmpty()) {
			BigDecimal cargoSum = cargoNominations.stream().map(CargoNomination::getQuantity).reduce(BigDecimal::add)
					.get();
			BigDecimal commingleCargoSum = commingleCargos.stream()
					.map(com.cpdss.loadablestudy.entity.CommingleCargo::getQuantity).reduce(BigDecimal::add).get();
			if (commingleCargoSum.compareTo(cargoSum) == 1) {
				log.info("commingle quanity is gerater for LS - {}", loadableStudy.getId());
				throw new GenericServiceException("Commingle quanity is gerater for LS - {}" + loadableStudy.getId(),
						CommonErrorCodes.E_CPDSS_LS_INVALID_COMMINGLE_QUANTITY, HttpStatusCode.INTERNAL_SERVER_ERROR);
			}
		}
	}
	 ...
	 * @param modelMapper
	 */
	public void buildCargoNominationDetails(long loadableStudyId,
			com.cpdss.loadablestudy.domain.LoadableStudy loadableStudy, ModelMapper modelMapper) {
		List<CargoNomination> cargoNominations = cargoNominationRepository
				.findByLoadableStudyXIdAndIsActive(loadableStudyId, true);

		loadableStudy.setCargoNomination(new ArrayList<>());
		cargoNominations.forEach(cargoNomination -> {
			com.cpdss.loadablestudy.domain.CargoNomination cargoNominationDto = new com.cpdss.loadablestudy.domain.CargoNomination();
			cargoNominationDto = modelMapper.map(cargoNomination, com.cpdss.loadablestudy.domain.CargoNomination.class);
			CargoInfo.CargoRequest.Builder cargoReq = CargoInfo.CargoRequest.newBuilder();
			cargoReq.setCargoId(cargoNomination.getCargoXId());
			com.cpdss.common.generated.CargoInfo.CargoDetailReply cargoDetailReply = this
					.getCargoInfoById(cargoReq.build());
			if (cargoDetailReply.getResponseStatus().getStatus().equals(SUCCESS)) {
				...
				cargoNominationDto.setIsCondensateCargo(cargoDetailReply.getCargoDetail().getIsCondensateCargo());
				cargoNominationDto.setIsHrvpCargo(cargoDetailReply.getCargoDetail().getIsHrvpCargo());
			} else {
				log.error("Could not fetch cargo info of cargo with id {}", cargoNomination.getCargoXId());
			}
	 ...
	 * @param loadableStudy   void
	 */
	public void buildCargoNominationPortDetails(long loadableStudyId,
			com.cpdss.loadablestudy.domain.LoadableStudy loadableStudy) {
		List<CargoNomination> cargoNominations = cargoNominationRepository
				.findByLoadableStudyXIdAndIsActive(loadableStudyId, true);
		loadableStudy.setCargoNominationOperationDetails(new ArrayList<>());
		List<CargoNominationPortDetails> cargoNominationOperationDetails = cargoNominationOperationDetailsRepository
				.findByCargoNominationAndIsActive(cargoNominations, true);
		cargoNominationOperationDetails.forEach(cargoNominationOperationDetail -> {
			com.cpdss.loadablestudy.domain.CargoNominationOperationDetails cargoNominationOperationDetailDto = new com.cpdss.loadablestudy.domain.CargoNominationOperationDetails();
			cargoNominationOperationDetailDto
					.setCargoNominationId(cargoNominationOperationDetail.getCargoNomination().getId());
			...
			cargoNominationOperationDetailDto.setQuantity(String.valueOf(cargoNominationOperationDetail.getQuantity()));
