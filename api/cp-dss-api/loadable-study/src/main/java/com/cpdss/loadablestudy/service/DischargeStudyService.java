...
public class DischargeStudyService extends DischargeStudyOperationServiceImplBase {
	...
	@Autowired
	private LoadableStudyRepository dischargeStudyRepository;
	@Autowired
	VoyageService voyageService;
	@Autowired
	private LoadableStudyStatusRepository loadableStudyStatusRepository;
	@Autowired
	private VoyageRepository voyageRepository;
	@Autowired
	private SynopticalTableRepository synopticalTableRepository;
	@Autowired
	private OnHandQuantityRepository onHandQuantityRepository;
	@Autowired
	private LoadableStudyPortRotationRepository loadableStudyPortRotationRepository;
	@Autowired
	private CargoNominationService cargoNominationService;
	@Autowired
	...
	@Autowired
	private LoadablePatternService loadablePatternService;
	@Autowired
	private CargoOperationRepository cargoOperationRepository;
	@Autowired
	private CargoNominationOperationDetailsRepository cargoNominationOperationRepository;
	@Autowired
	private SynopticService synopticService;
	@Autowired
	private CowDetailService cowDetailService;
	@Autowired
	private PortInstructionService portInstructionService;
	@Autowired
	private BackLoadingService backLoadingService;
	@Autowired
	private GenerateDischargeStudyJson generateDischargeStudyJson;
	@Autowired
	private LoadablePlanService loadablePlanService;
	@Autowired
	private LoadablePatternRepository loadablePatternRepository;
	@Autowired
	private DischargeStudyCowDetailRepository dischargeStudyCowDetailRepository;
	@Autowired
	CowHistoryRepository cowHistoryRepository;
	@Autowired
	LoadableStudyAlgoStatusRepository loadableStudyAlgoStatusRepository;

	@GrpcClient("dischargeInformationService")
	private DischargePlanServiceGrpc.DischargePlanServiceBlockingStub dischargePlanServiceBlockingStub;

	@Autowired
	private DischargePatternQuantityCargoPortwiseRepository dischargePatternQuantityCargoPortwiseRepository;
	...
	@Override
	public void saveDischargeStudy(DischargeStudyDetail request, StreamObserver<DischargeStudyReply> responseObserver) {
		...
		try {
			...
			if (null == voyage) {
				throw new GenericServiceException("Voyage does not exist", CommonErrorCodes.E_HTTP_BAD_REQUEST,
						HttpStatusCode.BAD_REQUEST);
			}

			List<LoadableStudy> loadables = this.dischargeStudyRepository
					.findByVesselXIdAndVoyageAndIsActiveAndLoadableStudyStatus_id(request.getVesselId(), voyage, true,
							CONFIRMED_STATUS_ID);
			if (loadables.isEmpty()) {
				throw new GenericServiceException("No confirmed loadable study", CommonErrorCodes.E_HTTP_BAD_REQUEST,
						HttpStatusCode.BAD_REQUEST);
			}
			LoadableStudy loadableStudy = loadables.get(0);
			List<LoadableStudyPortRotation> portRotationsForNonDischargingOperations = loadableStudyPortRotationRepository
					.findByLoadableStudyAndOperation_idNotAndIsActive(loadableStudy, DISCHARGING_OPERATION_ID, true);
			for (LoadableStudyPortRotation loadableStudyPortRotation : portRotationsForNonDischargingOperations) {
				List<SynopticalTable> synopticalTables = this.synopticalTableRepository
						.findByLoadableStudyXIdAndLoadableStudyPortRotation_idAndIsActive(loadableStudy.getId(),
								loadableStudyPortRotation.getId(), true);
				for (SynopticalTable synopticalTable : synopticalTables) {
					if (synopticalTable.getEtaActual() == null && synopticalTable.getEtdActual() == null) {
						throw new GenericServiceException("No Actual ETA/ETD values found",
								CommonErrorCodes.E_CPDSS_NO_ACTUAL_ETA_OR_ETD_FOUND, HttpStatusCode.BAD_REQUEST);
					}
				}
			}
			LoadableStudyPortRotation loadableStudyPortRotation = getportRotationData(loadableStudy);
			validateActuals(loadableStudy);
			if (dischargeStudyRepository.existsByNameIgnoreCaseAndPlanningTypeXIdAndVoyageAndIsActive(request.getName(),
					2, voyage, true)) {
				throw new GenericServiceException("name already exists", CommonErrorCodes.E_CPDSS_LS_NAME_EXISTS,
						HttpStatusCode.BAD_REQUEST);
			}
			List<SynopticalTable> synopticalData = this.synopticalTableRepository
					.findByLoadableStudyXIdAndLoadableStudyPortRotation_idAndIsActive(loadableStudy.getId(),
							loadableStudyPortRotation.getId(), true);
			if (synopticalData.isEmpty()) {
				throw new GenericServiceException("synoptical data missing", CommonErrorCodes.E_HTTP_BAD_REQUEST,
						HttpStatusCode.BAD_REQUEST);
			}
			List<OnHandQuantity> onhandQuantity = this.onHandQuantityRepository
					.findByLoadableStudyAndPortRotationAndIsActive(loadableStudy, loadableStudyPortRotation, true);
			if (onhandQuantity.isEmpty()) {
				throw new GenericServiceException("on hand quantity data missing", CommonErrorCodes.E_HTTP_BAD_REQUEST,
						HttpStatusCode.BAD_REQUEST);
			}

			LoadableStudy savedDischargeStudy = saveDischargeStudy(request, voyage, loadableStudy);
			LoadableStudyPortRotation dischargeStudyPortRotation = createDischargeStudyPortRotationData(
					loadableStudyPortRotation, savedDischargeStudy);
			LoadableStudyPortRotation savedDischargeport = loadableStudyPortRotationRepository
					.save(dischargeStudyPortRotation);
			this.cargoNominationService.saveDsichargeStudyCargoNominations(savedDischargeStudy.getId(),
					loadableStudy.getId(), savedDischargeport.getPortXId(), savedDischargeport.getOperation().getId());
			...
			e.printStackTrace();
			builder.setResponseStatus(ResponseStatus.newBuilder().setCode(e.getCode()).setMessage(e.getMessage())
					.setStatus(FAILED).setHttpStatusCode(e.getStatus().value()).build());
		} catch (Exception e) {
			TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
	...
	private void validateActuals(LoadableStudy loadableStudy) throws GenericServiceException {
		...
		List<CargoNomination> cargoNominations = cargoNominationService
				.getCargoNominationByLoadableStudyId(loadableStudy.getId());
		List<LoadableStudyPortRotation> ports = loadableStudyPortRotationRepository
		...
		StowageAndBillOfLaddingValidationRequest.Builder request = StowageAndBillOfLaddingValidationRequest
				.newBuilder();
		ports.forEach(port -> {
			...
			List<Long> cargoIds = cargoNominations.stream()
					.filter(cargo -> cargo.getCargoNominationPortDetails().stream()
							.anyMatch(portData -> portData.getPortId().equals(port.getPortXId())))
					.map(CargoNomination::getId).collect(Collectors.toList());

			portBuilder.addAllCargoIds(cargoIds);
			request.addPortWiseCargos(portBuilder);
		});
		ResponseStatus response = loadingPlanServiceBlockingStub.validateStowageAndBillOfLadding(request.build());
		if (!SUCCESS.equalsIgnoreCase(response.getStatus())) {
			throw new GenericServiceException("No Atcuals", CommonErrorCodes.E_CPDSS_NO_ACUTALS_OR_BL_VALUES_FOUND,
					HttpStatusCode.BAD_REQUEST);
		}
	}
	 ...
	 * @return
	 */
	private List<OnHandQuantity> createDischargeOnHandQuantity(List<OnHandQuantity> onhandQuantity,
	 ...
	 * @return
	 */
	private List<SynopticalTable> createDischargeSynoptical(List<SynopticalTable> synopticalData,
	 ...
	 * @return
	 */
	private LoadableStudy saveDischargeStudy(DischargeStudyDetail request, Voyage voyage, LoadableStudy loadableStudy) {
	...
	private LoadableStudyPortRotation getportRotationData(LoadableStudy loadableStudy) throws GenericServiceException {
		List<LoadableStudyPortRotation> portrotation = loadableStudyPortRotationRepository
				.findByLoadableStudyAndOperation_idAndIsActive(loadableStudy, DISCHARGING_OPERATION_ID, true);
		if (portrotation.isEmpty()) {
			throw new GenericServiceException("No discharging port in port rotation against loadable study",
	...
	@Override
	public void updateDischargeStudy(DischargeStudyDetail request,
			StreamObserver<UpdateDischargeStudyReply> responseObserver) {
		UpdateDischargeStudyReply.Builder builder = UpdateDischargeStudyReply.newBuilder();
		try {
			LoadableStudy loadable = this.dischargeStudyRepository.findById(request.getDischargeStudyId()).orElse(null);
			if (loadable == null) {
				throw new GenericServiceException("No discharge study found", CommonErrorCodes.E_HTTP_BAD_REQUEST,
						HttpStatusCode.BAD_REQUEST);
			}

			if (dischargeStudyRepository.existsByNameIgnoreCaseAndPlanningTypeXIdAndVoyageAndIsActiveAndIdNot(
	...
	public void deleteDischargeStudy(DischargeStudyRequest request,
		...
		try {
			Optional<LoadableStudy> entityOpt = this.dischargeStudyRepository.findById(request.getDischargeStudyId());
			if (!entityOpt.isPresent()) {
				throw new GenericServiceException("Loadable study does not exist", CommonErrorCodes.E_HTTP_BAD_REQUEST,
						HttpStatusCode.BAD_REQUEST);
			}
			...
			replyBuilder.setResponseStatus(ResponseStatus.newBuilder().setCode(e.getCode()).setMessage(e.getMessage())
					.setStatus(FAILED).setHttpStatusCode(e.getStatus().value()).build());
		} catch (Exception e) {
			...
			replyBuilder.setResponseStatus(ResponseStatus.newBuilder().setCode(CommonErrorCodes.E_GEN_INTERNAL_ERR)
					.setMessage("Exception  when deleting discharge study").setStatus(FAILED)
					.setHttpStatusCode(Integer.valueOf(CommonErrorCodes.E_GEN_INTERNAL_ERR)).build());
		} finally {
	...
	public com.cpdss.common.generated.LoadableStudy.PortRotationReply.Builder saveDischargingPorts(
		...
		Optional<LoadableStudy> loadableStudyOpt = this.loadableStudyRepository.findById(request.getLoadableStudyId());
		...
		List<LoadableStudyPortRotation> dischargingPorts = this.loadableStudyPortRotationRepository
				.findByLoadableStudyAndOperationAndIsActive(loadableStudyOpt.get(), discharging, true);
		...
		for (LoadableStudyPortRotation portRotation : dischargingPorts) {
			if (!request.getDischargingPortIdsList().contains(portRotation.getPortXId())) {
				...
				onHandQuantityRepository.deleteByLoadableStudyAndPortXId(loadableStudy, portRotation.getPortXId());
		...
		if (!CollectionUtils.isEmpty(portIds)) {
			...
			PortInfo.GetPortInfoByPortIdsRequest.Builder reqBuilder = PortInfo.GetPortInfoByPortIdsRequest.newBuilder();
			...
			if (!SUCCESS.equalsIgnoreCase(portReply.getResponseStatus().getStatus())) {
				throw new GenericServiceException("Error in calling port service", CommonErrorCodes.E_GEN_INTERNAL_ERR,
						HttpStatusCode.INTERNAL_SERVER_ERROR);
			}

			if (!CollectionUtils.isEmpty(portIds) && !CollectionUtils.isEmpty(portReply.getPortsList())) {
				dischargingPorts = this.buildDischargingPorts(portReply, loadableStudy, dischargingPorts, portIds);
				...
				for (LoadableStudyPortRotation portRotation : this.loadableStudyPortRotationRepository
	 ...
	 * @return
	 */
	private List<LoadableStudyPortRotation> buildDischargingPorts(PortInfo.PortReply portReply,
			LoadableStudy loadableStudy, List<LoadableStudyPortRotation> dischargingPorts, List<Long> portIds) {
		...
		for (Long requestedPortId : portIds) {
			Optional<PortInfo.PortDetail> portOpt = portReply.getPortsList().stream()
					.filter(portDetail -> Objects.equals(requestedPortId, portDetail.getId())).findAny();
			...
			if (portOpt.isPresent()) {
				...
				portRotationEntity.setOperation(this.cargoOperationRepository.getOne(DISCHARGING_OPERATION_ID));
				portRotationEntity.setSeaWaterDensity(
						!StringUtils.isEmpty(port.getWaterDensity()) ? new BigDecimal(port.getWaterDensity()) : null);
				...
				portRotationEntity.setAirDraftRestriction(
						!StringUtils.isEmpty(port.getMaxAirDraft()) ? new BigDecimal(port.getMaxAirDraft()) : null);
				portRotationEntity.setPortOrder(++maxPortOrder);

				// add ports to synoptical table by reusing the function called by
				// port-rotation flow
				synopticService.buildPortsInfoSynopticalTable(portRotationEntity, DISCHARGING_OPERATION_ID,
						port.getId());
	...
	@Override
	public void saveDischargeStudyBackLoading(DishargeStudyBackLoadingSaveRequest request,
			StreamObserver<DischargeStudyReply> responseObserver) {
		DischargeStudyReply.Builder builder = DischargeStudyReply.newBuilder();
		try {
			...
			if (!dischargeStudy.isPresent()) {
				throw new GenericServiceException("No discharge study found",
						CommonErrorCodes.E_CPDSS_NO_DISCHARGE_STUDY_FOUND, HttpStatusCode.BAD_REQUEST);
			}
			List<DishargeStudyBackLoadingDetail> portCargos = request.getDsBackLoadingList();
			List<LoadableStudyPortRotation> portRotations = loadableStudyPortRotationRepository
					.findByLoadableStudyAndIsActiveOrderByPortOrder(dischargeStudy.get(), true);
			List<Long> portRotationIds = portRotations.stream().map(LoadableStudyPortRotation::getId)
					.collect(Collectors.toList());

			Map<Long, DischargeStudyCowDetail> cowDetailForThePort = cowDetailService
					.getCowDetailForThePort(dischargestudyId, portRotationIds);
			Map<Long, List<DischargeStudyPortInstruction>> portWiseInstructions = portInstructionService
					.getPortWiseInstructions(dischargestudyId, portRotationIds);
			List<CargoNomination> dbCargos = cargoNominationService.getCargoNominations(dischargestudyId);
			Map<Long, List<BackLoading>> backloadingData = backLoadingService
					.getBackloadingDataByportIds(dischargestudyId, portRotationIds);
			...
			portCargos.forEach(cargo -> {
				...
				LoadableStudyPortRotation dbPortRoation = portRotations.stream()
						.filter(port -> port.getId() == portCargoId).findFirst().get();
				portRotations.forEach(portRotation -> {
					...
					if (portCargoId == portId) {
						...
						portRotation.setIsbackloadingEnabled(portRequestDetail.getIsBackLoadingEnabled());
				...
				updateCowDetails(cowDetailForThePort, cowDetailsToSave, portRequestDetail, portCargoId,
						dischargestudyId);
				updateInsrtuctions(dischargestudyId, portWiseInstructions, portInstructionsToSave, portRequestDetail,
						portCargoId);

				createBackLoading(backloadingData, backLoadingToSave, portRequestDetail, portCargoId, dischargestudyId);
				...
				for (int i = 0; i < cargoNominations.size(); i++) {
					...
					if (cargoRequest.getId() != -1) {
						Optional<CargoNomination> optionalCargoNomination = dbCargos.parallelStream()
								.filter(cargoNomination -> cargoNomination.getId() == cargoRequest.getId()).findFirst();
						...
						updateCargoNominationToSave(cargoRequest, optionalCargoNomination.get(), cargoNominationsToSave,
								portId);
					} else {
						...
						cargoNomination.setCargoNominationPortDetails(
								cargoNominationService.createCargoNominationPortDetails(cargoNomination, null, portId,
										dbPortRoation.getOperation().getId()));
						...
						updateCargoNominationToSave(cargoRequest, cargoNomination, cargoNominationsToSave, portId);
					}
				}
				List<Long> requestIds = cargoNominations.stream().map(CargoNominationDetail::getId)
						.collect(Collectors.toList());
				/** delete existing cargo nomination */
				List<CargoNomination> cargosForPort = dbCargos.stream()
						.flatMap(x -> x.getCargoNominationPortDetails().stream())
						.filter(port -> port.getPortId().equals(dbPortRoation.getPortXId()))
						.map(CargoNominationPortDetails::getCargoNomination).collect(Collectors.toList());
				List<CargoNomination> cargosToDisable = cargosForPort.parallelStream()
						.filter(cn -> !requestIds.contains(cn.getId())).collect(Collectors.toList());
				cargosToDisable.forEach(cargoToDisable -> {
					Optional<CargoNominationPortDetails> operationDetail = cargoToDisable
							.getCargoNominationPortDetails().stream()
							.filter(port -> port.getPortId().equals(dbPortRoation.getPortXId())).findFirst();
	...
	private void updateCargoNominationToSave(CargoNominationDetail cargoRequest, CargoNomination cargoNomination,
			List<CargoNomination> cargoNominationsToSave, Long portId) {
		...
		Optional<CargoNominationPortDetails> cargoOperation = cargoNomination.getCargoNominationPortDetails().stream()
				.filter(cp -> cp.getIsActive() && cp.getPortId().equals(portId)).findFirst();
	...
	private void updateInsrtuctions(long dischargestudyId,
			Map<Long, List<DischargeStudyPortInstruction>> portWiseInstructions,
			List<DischargeStudyPortInstruction> portInstructionsToSave, PortRotationDetail portRequestDetail,
			long portCargoId) {
		if (portWiseInstructions.containsKey(portCargoId)) {
			List<Long> requestInstructions = portRequestDetail.getInstructionIdList();
			List<Long> dbInstructionIds = portWiseInstructions.get(portCargoId).stream()
					.map(DischargeStudyPortInstruction::getPortInstructionId).collect(Collectors.toList());
			requestInstructions.forEach(portRequestInstructionId -> {
				if (!dbInstructionIds.contains(portRequestInstructionId)) {
					createPortInstructionToSave(dischargestudyId, portInstructionsToSave, portCargoId,
							portRequestInstructionId);
				}
			});
			/**
			 * if the instruction is a list and removed anything already assigned then need
			 * to disable that instruction
			 */
			List<DischargeStudyPortInstruction> intructionsToDisable = portWiseInstructions.get(portCargoId).stream()
					.filter(dbId -> !requestInstructions.contains(dbId.getPortInstructionId()))
					.collect(Collectors.toList());
			intructionsToDisable.forEach(instruction -> {
				instruction.setIsActive(false);
				portInstructionsToSave.add(instruction);
			});
		} else {
			portRequestDetail.getInstructionIdList().forEach(portRequestInstructionId -> {
				createPortInstructionToSave(dischargestudyId, portInstructionsToSave, portCargoId,
						portRequestInstructionId);
			});
		}
	 ...
	 */
	private void createBackLoading(Map<Long, List<BackLoading>> backloadingData, List<BackLoading> backLoadingToSave,
			PortRotationDetail portRequestDetail, long portCargoId, long dischargestudyId) {
		if (backloadingData.containsKey(portCargoId)) {
			List<BackLoading> dbBackLoadings = backloadingData.get(portCargoId);
			portRequestDetail.getBackLoadingList().stream().forEach(backLoadingRequest -> {
				if (backLoadingRequest.getId() != -1) {
					Optional<BackLoading> backLoading = dbBackLoadings.parallelStream()
							.filter(backloading -> backloading.getId() == backLoadingRequest.getId()).findFirst();
			...
			List<Long> requestIds = portRequestDetail.getBackLoadingList().stream()
					.map(com.cpdss.common.generated.loadableStudy.LoadableStudyModels.BackLoading::getId)
					.collect(Collectors.toList());
			List<BackLoading> backLoadingToDisable = backloadingData.get(portCargoId).stream()
					.filter(dbId -> !requestIds.contains(dbId.getId())).collect(Collectors.toList());
			backLoadingToDisable.forEach(backLoading -> {
			...
			if (backloadingData != null && !backloadingData.isEmpty()) {
				...
				if (backloadingData.get(portCargoId) != null) {
					backloadingData.get(portCargoId).stream().forEach(backLoading -> {
						backLoading.setActive(false);
						backLoadingToSave.add(backLoading);
					});
				}
			}
			portRequestDetail.getBackLoadingList().forEach(backLoadingRequest -> {
	...
	private void updateBackLoadingToSave(
			...
			BackLoading backLoading, List<BackLoading> backLoadingToSave) {
	...
	private void createPortInstructionToSave(long dischargestudyId,
			List<DischargeStudyPortInstruction> portInstructionsToSave, long portCargoId,
	...
	private void updateCowDetails(Map<Long, DischargeStudyCowDetail> cowDetailForThePort,
			List<DischargeStudyCowDetail> cowDetailsToSave, PortRotationDetail portDetail, long portCargoId,
		...
		if (portDetail.getCowId() == 1) {
			...
			String numberString = portDetail.getTanksList().stream().map(String::valueOf)
	...
	@Override
	public void getDischargeStudyPortWiseCargos(DischargeStudyRequest request,
			StreamObserver<DishargeStudyCargoReply> responseObserver) {
		...
		try {
			if (!dischargeStudyRepository.existsById(request.getDischargeStudyId())) {
				throw new GenericServiceException("Loadable study does not exist",
						CommonErrorCodes.E_CPDSS_CONFIRMED_LS_DOES_NOT_EXIST, HttpStatusCode.BAD_REQUEST);
			}
			List<LoadableStudyPortRotation> findByLoadableStudyIdAndIsActive = loadableStudyPortRotationRepository
					.findByLoadableStudyIdAndIsActive(request.getDischargeStudyId(), true);
			List<Long> portIds = findByLoadableStudyIdAndIsActive.stream().map(LoadableStudyPortRotation::getPortXId)
					.collect(Collectors.toList());
			PortInfo.GetPortInfoByPortIdsRequest.Builder reqBuilder = PortInfo.GetPortInfoByPortIdsRequest.newBuilder();
			portIds.forEach(port -> {
				reqBuilder.addId(port);
			...
			cargoRequest.addAllId(
					cargoPortMappings.stream().map(CargoPortMapping::getCargoId).collect(Collectors.toList()));
			CargoReply cargoInfosByCargoIds = cargoInfoGrpcService.getCargoInfosByCargoIds(cargoRequest.build());
			...
			Map<Long, List<Long>> portWiseCargos = cargoPortMappings.stream()
					.collect(Collectors.groupingBy(CargoPortMapping::getPortId,
							Collectors.mapping(CargoPortMapping::getCargoId, Collectors.toList())));
			portWiseCargos.forEach((portId, cargoIds) -> {
				List<CargoDetail> cargoDetails = cargoInfosByCargoIds.getCargosList().stream()
						.filter(c -> cargoIds.contains(c.getId())).collect(Collectors.toList());
				DishargeStudyPortCargoMapping.Builder cargoPortMappingBuilder = DishargeStudyPortCargoMapping
						.newBuilder();
			...
			replyBuilder.setResponseStatus(ResponseStatus.newBuilder().setCode(CommonErrorCodes.E_GEN_INTERNAL_ERR)
					.setMessage("Exception  when getting powtise cargo details").setStatus(FAILED)
					.setHttpStatusCode(Integer.valueOf(CommonErrorCodes.E_GEN_INTERNAL_ERR)).build());
		} finally {
			responseObserver.onNext(replyBuilder.build());
			responseObserver.onCompleted();
	...
	private void createCargoResponse(List<CargoDetail> cargoDetails,
			DishargeStudyPortCargoMapping.Builder replyBuilder) {
	...
	@Override
	public void generateDischargePatterns(AlgoRequest request, StreamObserver<AlgoReply> responseObserver) {
		log.info("Inside generateLoadablePatterns service");
		com.cpdss.common.generated.LoadableStudy.AlgoReply.Builder replyBuilder = AlgoReply.newBuilder();
		try {
			...
			replyBuilder = AlgoReply.newBuilder().setResponseStatus(ResponseStatus.newBuilder().setStatus(FAILED)
					.setMessage(ERRO_CALLING_ALGO).setCode(CommonErrorCodes.E_CPDSS_ALGO_ISSUE).build());
		} catch (Exception e) {
			log.error("Exception when when calling algo  ", e);
			replyBuilder = AlgoReply.newBuilder().setResponseStatus(ResponseStatus.newBuilder()
					.setCode(CommonErrorCodes.E_GEN_INTERNAL_ERR).setMessage("Error when calling algo ")
					.setHttpStatusCode(Integer.valueOf(CommonErrorCodes.E_GEN_INTERNAL_ERR)).setStatus(FAILED).build());
		} finally {
			responseObserver.onNext(replyBuilder.build());
			responseObserver.onCompleted();
		}
	...
	public void addCargoNominationForPortRotation(long portRotationId, long loadableStudyId)
		...
		if (newPort.getId() != portRotationId) {
			throw new GenericServiceException("port rotaion data mis match", CommonErrorCodes.E_HTTP_BAD_REQUEST,
					HttpStatusCode.BAD_REQUEST);
		}
	...
	private void createBackLoadingIfAny(long loadableStudyId, List<LoadableStudyPortRotation> portRotations,
			List<CargoNomination> cargos) {
		LoadableStudyPortRotation loadableStudyPortRotation = portRotations.get(portRotations.size() - 2);
		Map<Long, List<BackLoading>> backloadingDataByportIds = backLoadingService
				.getBackloadingDataByportIds(loadableStudyId, Arrays.asList(loadableStudyPortRotation.getId()));
		List<BackLoading> backLoadingList = backloadingDataByportIds.get(loadableStudyPortRotation.getId());
		if (backLoadingList != null && !backLoadingList.isEmpty()) {
			backLoadingList.stream().forEach(backloading -> {
				...
				cargoBackloading.setCargoNominationPortDetails(cargoNominationService.createCargoNominationPortDetails(
						cargoBackloading, null, portRotations.get(portRotations.size() - 1).getPortXId(),
	...
	private List<CargoNomination> createNewPortCargoNominations(long loadableStudyId,
			LoadableStudyPortRotation newPort) {
		List<CargoNomination> cargos = cargoNominationService.getCargoNominations(loadableStudyId);
		List<LoadableStudyPortRotation> ports = loadableStudyPortRotationRepository
				.findByLoadableStudyIdAndIsActive(loadableStudyId, true);
		Optional<LoadableStudyPortRotation> firstDischargingPort = ports.stream()
				.filter(port -> port.isActive() && port.getOperation().getId().equals(DISCHARGING_OPERATION_ID))
				.sorted(Comparator.comparing(LoadableStudyPortRotation::getPortOrder)).findFirst();
		List<CargoNomination> newPortCargo = cargos.stream()
				.filter(cargo -> cargo.getIsBackloading() == null || cargo.getIsBackloading().equals(false))
				.collect(Collectors.toList());
		newPortCargo.stream().forEach(cargo -> {
			...
			if (!checkIfCargoNominationAlreadyPresent(cargo, newPort.getPortXId())) {
				...
				if (firstDischargingPort.isPresent()
					...
					newPortDetails = cargoNominationService.createCargoNominationPortDetails(cargo, cargo,
							newPort.getPortXId(), newPort.getOperation().getId());
				} else {
					newPortDetails = cargoNominationService.createCargoNominationPortDetails(cargo, null,
							newPort.getPortXId(), newPort.getOperation().getId());
				}

				if (cargo.getCargoNominationPortDetails() != null && !cargo.getCargoNominationPortDetails().isEmpty()) {
	...
	/**
	 * Method used to check if cargo nomination and port combo already present in DB
	 * This check is needed to avoid duplication when user delete/rename a discharge
	 * port and later add the same port. Bug fix - DSS - 4493 - Checking if for
	 * duplicate entry in Cargo nomination
	 */
	private Boolean checkIfCargoNominationAlreadyPresent(CargoNomination cargo, Long portXId) {
		CargoNominationPortDetails cargoNominationOperation = cargoNominationOperationRepository
				.findByCargoNominationAndPortIdAndIsActiveTrue(cargo, portXId);
	...
	private List<LoadableStudyPortRotation> getDischargeStudyPortRotations(Long loadableStudyId)
		...
		/**
		 * if the loadable study is a discharge study then add cargo nominations of the
		 * previous port t the newly created one else return
		 */
		if (loadableStudy.getPlanningTypeXId() != 2) {
			return null;
		}
		List<LoadableStudyPortRotation> portRotations = loadableStudyPortRotationRepository
				.findByLoadableStudyIdAndIsActive(loadableStudy.getId(), true);
	...
	public void resetCargoNominationQuantityAndBackLoading(long portRotationId, long loadableStudyId)
			throws GenericServiceException {
		List<LoadableStudyPortRotation> dischargeStudyPortRotations = getDischargeStudyPortRotations(loadableStudyId);
		...
		// nominations.
		Set<Long> nominationPortIds = cargos.stream().flatMap(x -> x.getCargoNominationPortDetails().stream())
				.map(CargoNominationPortDetails::getPortId).distinct().collect(Collectors.toSet());
		List<LoadableStudyPortRotation> dischargingPorts = dischargeStudyPortRotations.stream()
		...
		if (!dischargingPorts.isEmpty()) {
			cargoNominationService.saveAll(createNewPortCargoNominations(loadableStudyId, dischargingPorts.get(0)));
		}
		Set<CargoNomination> firstPortCargos = cargos.stream()
				.filter(cargo -> cargo.getIsBackloading() == null || cargo.getIsBackloading().equals(false))
				.collect(Collectors.toSet());

		List<Long> firstPortCargoIds = firstPortCargos.stream().distinct().map(CargoNomination::getLsCargoNominationId)
				.filter(Objects::nonNull).collect(Collectors.toList());
		cargoNominationService.getMaxQuantityForCargoNomination(firstPortCargoIds, firstPortCargos);
		Optional<LoadableStudyPortRotation> dischargeStudyPortRotation = dischargeStudyPortRotations.stream()
				.filter(port -> port.getOperation().getId().equals(DISCHARGING_OPERATION_ID))
				.sorted(Comparator.comparing(LoadableStudyPortRotation::getPortOrder)).findFirst();

		cargos.stream().flatMap(cargo -> cargo.getCargoNominationPortDetails().stream()).forEach(operation -> {
		...
		cargoNominationService.saveAll(cargos);
		List<BackLoading> backLoadings = backLoadingService.getBackLoadings(loadableStudyId, dischargeStudyPortRotations
				.stream().map(LoadableStudyPortRotation::getId).collect(Collectors.toList()));
		backLoadings.parallelStream().forEach(backLoading -> {
			backLoading.setActive(false);
		});
		backLoadingService.saveAll(backLoadings);
		dischargeStudyPortRotations.stream().forEach(port -> {
	...
	public void getDischargePlanDetails(LoadablePlanDetailsRequest request,
		...
		try {
			...
			Optional<LoadableStudy> dsOptional = dischargeStudyRepository
					.findByIdAndIsActive(request.getLoadablePatternId(), true);
			if (dsOptional.isEmpty()) {
				throw new GenericServiceException("DS ID not match", CommonErrorCodes.E_CPDSS_NO_DISCHARGE_STUDY_FOUND,
						HttpStatusCode.BAD_REQUEST);
			}
			LoadableStudy dischargeStudy = dsOptional.get();
			List<LoadableStudyPortRotation> ports = loadableStudyPortRotationRepository
					.findByLoadableStudyAndIsActive(dischargeStudy.getId(), true);
			List<CargoNomination> cargos = cargoNominationService
					.getCargoNominationByLoadableStudyId(dischargeStudy.getId());
			List<DischargePatternQuantityCargoPortwiseDetails> generatedCargos = dischargePatternQuantityCargoPortwiseRepository
			...
			Map<Long, List<DischargePatternQuantityCargoPortwiseDetails>> portWiseCargo = generatedCargos.stream()
					.collect(Collectors.groupingBy(DischargePatternQuantityCargoPortwiseDetails::getPortRotationId));
			ports.stream().forEach(port -> {
				List<DischargePatternQuantityCargoPortwiseDetails> portCargos = portWiseCargo.get(port.getId());
				portCargos.stream().forEach(pCargo -> {
					CargoNominationDetail.Builder cargo = CargoNominationDetail.newBuilder();
					ofNullable(pCargo.getCargoAbbreviation()).ifPresent(cargo::setAbbreviation);
					ofNullable(pCargo.getEstimatedAPI()).ifPresent(api -> cargo.setApi(api.toString()));
					...
					ofNullable(pCargo.getEstimatedTemp()).ifPresent(value -> cargo.setTemperature(value.toString()));
					ofNullable(pCargo.getColorCode()).ifPresent(cargo::setColor);
					ofNullable(pCargo.getDischargeMT()).ifPresent(value -> cargo.setQuantity(value.toString()));
			...
			List<LoadablePattern> patterns = loadablePatternRepository
					.findByLoadableStudyAndIsActiveOrderByCaseNumberAsc(dischargeStudy, true);
			if (!patterns.isEmpty()) {
	...
	@Override
	public void confirmPlan(ConfirmPlanRequest request, StreamObserver<ConfirmPlanReply> responseObserver) {
		...
		try {
			...
			if (!SUCCESS.equals(reply.getResponseStatus().getStatus())) {
				throw new GenericServiceException("failed to confirm", reply.getResponseStatus().getCode(),
						HttpStatusCode.valueOf(Integer.valueOf(reply.getResponseStatus().getCode())));
			}
			com.cpdss.loadablestudy.entity.LoadablePattern loadablePatternOpt = this.loadablePatternRepository
					.findByIdAndIsActive(request.getLoadablePatternId(), true).get();
			transferDSConfirmedPlanData(loadablePatternOpt.getLoadableStudy());
		} catch (Exception e) {
			log.error("Exception when confirmPlan ", e);
			replyBuilder.setResponseStatus(
					ResponseStatus.newBuilder().setStatus(FAILED).setCode(CommonErrorCodes.E_GEN_INTERNAL_ERR)
	...
	private void transferDSConfirmedPlanData(LoadableStudy dischargeStudy) {
		List<LoadableStudyPortRotation> ports = loadableStudyPortRotationRepository
				.findByLoadableStudyAndIsActive(dischargeStudy.getId(), true);
		DischargeStudyDataTransferRequest.Builder request = DischargeStudyDataTransferRequest.newBuilder();
		request.setVoyageId(dischargeStudy.getVoyage().getId());
		request.setVesselId(dischargeStudy.getVesselXId());

		List<LoadableStudyAlgoStatus> algoStatuses = loadableStudyAlgoStatusRepository
				.findByLoadableStudyIdAndIsActive(dischargeStudy.getId(), true);
		Optional<LoadableStudyAlgoStatus> latestOne = algoStatuses.stream()
				.sorted(Comparator.comparing(EntityDoc::getCreatedDateTime)).findFirst();
		if (latestOne.isPresent()) {
			request.setDischargeProcessId(latestOne.get().getProcessId());
		}

		Optional<com.cpdss.loadablestudy.entity.LoadablePattern> confirmedLoadablePatternOpt = this.loadablePatternRepository
				.findByLoadableStudyAndLoadableStudyStatusAndIsActive(dischargeStudy, CONFIRMED_STATUS_ID, true);
		...
		ports.stream().forEach(port -> {
			...
			Optional.ofNullable(port.getPortOrder()).ifPresent(v -> portDataBuilder.setPortOrder(v.intValue()));
			Optional<SynopticalTable> synopticalTableOpt = port.getSynopticalTable().stream()
					.filter(synopticalTable -> synopticalTable.getIsActive()
							&& synopticalTable.getOperationType().equalsIgnoreCase(SYNOPTICAL_TABLE_OP_TYPE_ARRIVAL))
					.findFirst();
			...
			if (dischargeStudy.getId() != null) {
				portDataBuilder.setPortId(port.getPortXId());
				DischargeStudyCowDetail dsCow = this.dischargeStudyCowDetailRepository
						.findByDischargeStudyStudyIdAndPortIdAndIsActive(dischargeStudy.getId(), port.getPortXId(),
								true);
				if (dsCow != null) { // Cow Details Per Port Ids
					...
					dsCowDetails.setCowOptionType(Common.COW_OPTION_TYPE.forNumber(dsCow.getCowType().intValue()));
					if (dsCow.getTankIds() != null) {
						List<Long> list = Arrays.asList(dsCow.getTankIds().split(",")).stream().map(String::trim)
								.mapToLong(Long::parseLong).boxed().collect(Collectors.toList());
						if (!list.isEmpty()) {
							CowTankDetails.Builder tankDetails = CowTankDetails.newBuilder();
							tankDetails.setCowType(Common.COW_TYPE.forNumber(dsCow.getCowType().intValue()));
	...
	public void getCowHistoryByVesselId(com.cpdss.common.generated.LoadableStudy.CowHistoryRequest request,
			StreamObserver<com.cpdss.common.generated.LoadableStudy.CowHistoryReply> responseObserver) {
		com.cpdss.common.generated.LoadableStudy.CowHistoryReply.Builder replyBuilder = com.cpdss.common.generated.LoadableStudy.CowHistoryReply
				.newBuilder();
		try {
			Pageable pageable = PageRequest.of(0, 500, Sort.by("lastModifiedDateTime").descending());
			List<CowHistory> list = cowHistoryRepository.findAllByVesselIdAndIsActiveTrue(request.getVesselId(),
					pageable);
			for (CowHistory ch : list) {
				com.cpdss.common.generated.LoadableStudy.CowHistory.Builder cowBuilder = com.cpdss.common.generated.LoadableStudy.CowHistory
						.newBuilder();
