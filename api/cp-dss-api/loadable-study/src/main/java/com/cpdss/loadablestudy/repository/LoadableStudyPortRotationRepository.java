...
public interface LoadableStudyPortRotationRepository
  ...
  public List<LoadableStudyPortRotation> findByLoadableStudyAndIsActiveOrderByPortOrder(
	...
	/**
	 * Get active port rotation for discharging ports against a loadable study order by operation type and port order
	 * 
	 * @param loadableStudy
	 * @param isActive
	 * @return List<LoadableStudyPortRotation>
	 */
	@Query("FROM LoadableStudyPortRotation LSPR WHERE LSPR.loadableStudy = ?1 AND LSPR.isActive = ?2 ORDER BY LSPR.operation.id, LSPR.portOrder")
