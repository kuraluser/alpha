...
public class ExecuteTask extends TaskManagerServiceGrpc.TaskManagerServiceImplBase {
  ...
  @Autowired private ExecuteTaskListener listener;
  ...
  public void executeTask(
      TaskManager.ExecuteTaskRequest executeTaskRequest, StreamObserver responseObserver) {
    ...
    try {
      ...
      log.error(
          String.format(
              "GenericServiceException when executing scheduled task %s",
              executeTaskRequest.getTaskName()),
          e);
      ...
      log.error(
          String.format(
              "Exception when executing scheduled task %s", executeTaskRequest.getTaskName()),
          e);
      replyBuilder.setResponseStatus(
          Common.ResponseStatus.newBuilder()
