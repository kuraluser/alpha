...
public class LoadingInformationServiceImpl implements LoadingInformationService {
  ...
  private void buildLoadingInformation(
      ...
      throws Exception {
    if (Optional.ofNullable(request.getLoadingStage().getDuration().getId()).isPresent() && request.getLoadingStage().getDuration().getId() != 0) {
    ...
    if (Optional.ofNullable(request.getLoadingStage().getOffset().getId()).isPresent() && request.getLoadingStage().getOffset().getId() != 0) {
