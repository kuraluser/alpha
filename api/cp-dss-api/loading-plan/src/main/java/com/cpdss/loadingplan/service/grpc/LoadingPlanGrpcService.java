...
public class LoadingPlanGrpcService extends LoadingPlanServiceImplBase {
	...
	@Autowired
	LoadingPlanAlgoService loadingPlanAlgoService;
	@Autowired
	LoadingSequenceService loadingSequenceService;

	@Autowired
	LoadingPlanRuleServiceImpl loadingPlanRuleService;

	@Autowired
	PortLoadingPlanStowageDetailsRepository portLoadingPlanStowageDetailsRepository;

	@Autowired
	BillOfLaddingRepository billOfLaddingRepository;

	@Override
	public void loadingPlanSynchronization(LoadingPlanSyncDetails request,
		...
		try {
			...
			builder.setResponseStatus(ResponseStatus.newBuilder().setStatus(LoadingPlanConstants.SUCCESS).build());
			...
			builder.setResponseStatus(ResponseStatus.newBuilder().setCode(CommonErrorCodes.E_GEN_INTERNAL_ERR)
					.setMessage(e.getMessage()).setStatus(LoadingPlanConstants.FAILED).build());
		} finally {
	...
	@Override
	public void getOrSaveRulesForLoadingPlan(LoadingPlanModels.LoadingPlanRuleRequest request,
			StreamObserver<LoadingPlanModels.LoadingPlanRuleReply> responseObserver) {
		LoadingPlanModels.LoadingPlanRuleReply.Builder builder = LoadingPlanModels.LoadingPlanRuleReply.newBuilder();
		try {
	...
	@Override
	public void saveLoadingPlan(LoadingPlanSaveRequest request,
			StreamObserver<LoadingPlanSaveResponse> responseObserver) {
	...
	@Override
	public void getLoadingSequences(LoadingSequenceRequest request,
			StreamObserver<LoadingSequenceReply> responseObserver) {
	...
	@Override
	public void getLoadingPlan(LoadingPlanModels.LoadingInformationRequest request,
			StreamObserver<LoadingPlanModels.LoadingPlanReply> responseObserver) {
		LoadingPlanModels.LoadingPlanReply.Builder builder = LoadingPlanModels.LoadingPlanReply.newBuilder();
		try {
	...
	@Override
	public void getUpdateUllageDetails(LoadingPlanModels.UpdateUllageDetailsRequest request,
			io.grpc.stub.StreamObserver<LoadingPlanModels.UpdateUllageDetailsResponse> responseObserver) {
		log.info("Inside saveLoadingPlan");
		LoadingPlanModels.UpdateUllageDetailsResponse.Builder builder = LoadingPlanModels.UpdateUllageDetailsResponse
				.newBuilder();
		try {
	...
	@Override
	public void getBillOfLaddingDetails(BillOfLaddingRequest request,
			StreamObserver<LoadingInformationSynopticalReply> responseObserver) {
		log.info("Inside getBillOfLaddingDetails - getting bill of ladding details agianst cargonomination Id");
		LoadingInformationSynopticalReply.Builder reply = LoadingInformationSynopticalReply.newBuilder();
		try {
			List<com.cpdss.loadingplan.entity.BillOfLadding> billOfLaddingList = billOfLaddingRepository
					.findByCargoNominationIdAndIsActive(request.getCargoNominationId(), true);
			if (!CollectionUtils.isEmpty(billOfLaddingList)) {
				billOfLaddingList.forEach(item ->{
					...
					Optional.ofNullable(item.getCargoNominationId()).ifPresent(billOfLadding::setCargoNominationId);
					Optional.ofNullable(item.getQuantityBbls()).ifPresent(value -> billOfLadding.setQuantityBbls(String.valueOf(value)));
					Optional.ofNullable(item.getQuantityMt()).ifPresent(value -> billOfLadding.setQuantityMt(String.valueOf(value)));
					Optional.ofNullable(item.getQuantityKl()).ifPresent(value -> billOfLadding.setQuantityKl(String.valueOf(value)));
					Optional.ofNullable(item.getApi()).ifPresent(value -> billOfLadding.setApi(String.valueOf(value)));
					Optional.ofNullable(item.getTemperature()).ifPresent(value -> billOfLadding.setTemperature(String.valueOf(value)));
					reply.addBillOfLadding(billOfLadding);
				});
			}
			reply.setResponseStatus(ResponseStatus.newBuilder().setStatus(LoadingPlanConstants.SUCCESS).build());			
		} catch (Exception e) {
