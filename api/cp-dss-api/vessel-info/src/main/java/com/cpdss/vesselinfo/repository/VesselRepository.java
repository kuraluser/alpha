...
public interface VesselRepository extends CommonCrudRepository<Vessel, Long> {
  ...
  @Query(
      "SELECT new com.cpdss.vesselinfo.domain.VesselDetails(VDC.draftExtreme, V.deadWeight, V.deadWeight) from Vessel V INNER JOIN VesselDraftCondition VDC on V.id = VDC.vessel.id AND V.id = ?1 AND VDC.draftCondition.id = ?2 AND VDC.draftExtreme = ?3")
