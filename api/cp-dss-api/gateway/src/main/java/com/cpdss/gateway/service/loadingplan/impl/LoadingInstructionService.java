...
public class LoadingInstructionService {
	...
	public LoadingInstructionResponse getLoadingInstructions(Long vesselId, Long infoId, Long portRotationId)
		...
		try {
			...
			requestBuilder.setLoadingInfoId(infoId).setPortRotationId(portRotationId).setVesselId(vesselId);
			LoadingInstructionDetails grpcResponse = loadingInstructionServiceBlockingStub
					.getLoadingInstructions(requestBuilder.build());
	...
	public LoadingInstructionsSaveResponse addLoadingInstruction(Long vesselId, Long infoId, Long portRotationId,
			LoadingInstructionsSaveRequest request) throws GenericServiceException {
		...
		try {
			...
			Optional.ofNullable(request.getInstructionTypeId()).ifPresent(requestBuilder::setInstructionTypeId);
			...
			ResponseStatus grpcResponse = this.loadingInstructionServiceBlockingStub
					.addLoadingInstruction(requestBuilder.build());
			if (grpcResponse.getStatus().equalsIgnoreCase(SUCCESS)) {
				...
				throw new GenericServiceException("Failed to add new instruction", CommonErrorCodes.E_HTTP_BAD_REQUEST,
	...
	public LoadingInstructionsSaveResponse updateLoadingInstructions(Long vesselId, Long infoId, Long portRotationId,
			LoadingInstructionsUpdateRequest request) throws GenericServiceException {
		try {
			...
			Optional.ofNullable(request.getInstructionList()).ifPresent(instructionList->
					requestBuilder.addAllInstructionList(instructionList.stream()
							.map(item -> LoadingInstructionStatus.newBuilder()
									.setInstructionId(item.getInstructionId())
									.setIsChecked(item.getIsChecked()).build())
							.collect(Collectors.toList())));

			ResponseStatus grpcResponse = this.loadingInstructionServiceBlockingStub
					.updateLoadingInstructions(requestBuilder.build());
			if (grpcResponse.getStatus().equalsIgnoreCase(SUCCESS)) {
				log.info("GRPC call successfull");
				return new LoadingInstructionsSaveResponse(new CommonSuccessResponse("SUCCESS", ""));
			} else {
				log.error("Failed to update instruction status - GRPC failed");
				throw new GenericServiceException("Failed to update instruction status",
						CommonErrorCodes.E_HTTP_BAD_REQUEST, HttpStatusCode.BAD_REQUEST);
			}
		} catch (Exception e) {
	...
	public LoadingInstructionsSaveResponse deleteLoadingInstructions(Long vesselId, Long infoId, Long portRotationId,
			LoadingInstructionsStatus request) throws GenericServiceException {
		try {
			...
			if (grpcResponse.getStatus().equalsIgnoreCase(SUCCESS)) {
				...
				throw new GenericServiceException("Failed to delete instruction", CommonErrorCodes.E_HTTP_BAD_REQUEST,
						HttpStatusCode.BAD_REQUEST);
			}
		} catch (Exception e) {
