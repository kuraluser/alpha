...
public class PortInfoService {
  ...
  public PortDetailResponse getPortInformationByPortId(Long portId, String correlationId) 
	  ...
	  PortReply portReply = portInfoServiceBlockingStub.getPortInfoByPortIds(builder.build());
	    if (!SUCCESS.equals(portReply.getResponseStatus().getStatus())) {
	        throw new GenericServiceException(
	            ...
	            HttpStatusCode.valueOf(
	                Integer.valueOf(portReply.getResponseStatus().getCode())));
	      }
	    ...
	    portDetails.setDensityOfWater(portsList.get(0).getWaterDensity().isEmpty()?null:new BigDecimal(portsList.get(0).getWaterDensity()));
	    portDetails.setMaxPermissibleDraft(portsList.get(0).getMaxPermissibleDraft().isEmpty()?null:new BigDecimal(portsList.get(0).getMaxPermissibleDraft()));
	    ...
	    portDetails.setTideHeightHigh(portsList.get(0).getTideHeightTo().isEmpty()?null:new BigDecimal(portsList.get(0).getTideHeightTo()));
	    portDetails.setTideHeightLow(portsList.get(0).getTideHeightFrom().isEmpty()?null:new BigDecimal(portsList.get(0).getTideHeightFrom()));
	    portDetails.setLatitude(portsList.get(0).getLat().isEmpty()?null:portsList.get(0).getLat());
	    portDetails.setLongitude(portsList.get(0).getLon().isEmpty()?null:portsList.get(0).getLon());
	    ...
	    for (BerthDetail berth:berthDetailsList) {
	    	...
	    	berthResponse.setDepthInDatum(berth.getBerthDatumDepth().isEmpty()?null:new BigDecimal(berth.getBerthDatumDepth()));
	    	berthResponse.setMaxDwt(berth.getMaxDwt().isEmpty()?null:new BigDecimal(berth.getMaxDwt()));
	    	berthResponse.setMaxManifoldHeight(berth.getMaxManifoldHeight().isEmpty()?null:new BigDecimal(berth.getMaxManifoldHeight()));
	    	berthResponse.setMaxShipDepth(berth.getMaxShipDepth().isEmpty()?null:new BigDecimal(berth.getMaxShipDepth()));
	    	berthResponse.setPortId(berth.getPortId());
	    	berthResponse.setRegulationAndRestriction(berth.getRegulationAndRestriction());
	    	berthResponse.setMaxLoa(berth.getMaxLoa().isEmpty()?null:new BigDecimal(berth.getMaxLoa()));
