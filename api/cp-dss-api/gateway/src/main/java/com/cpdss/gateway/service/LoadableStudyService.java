...
/**
 * GatewayLoadableStudyService - service class for loadable study related
 * operations
 */
@Service
@Log4j2
public class LoadableStudyService {
	...
	private static final List<String> ATTACHMENT_ALLOWED_EXTENSIONS = Arrays.asList("docx", "pdf", "txt", "jpg", "png",
			"msg", "eml");
	 ...
	 */
	public VoyageResponse saveVoyage(Voyage voyage, long companyId, long vesselId, String correlationId)
			throws GenericServiceException {
		VoyageResponse voyageResponse = new VoyageResponse();
		VoyageRequest voyageRequest = VoyageRequest.newBuilder().setCaptainId(voyage.getCaptainId())
				.setChiefOfficerId(voyage.getChiefOfficerId()).setCompanyId(companyId).setVesselId(vesselId)
				...
				.setEndTimezoneId(voyage.getEndTimezoneId() != null ? voyage.getEndTimezoneId().intValue() : 0).build();

		VoyageReply voyageReply = this.saveVoyage(voyageRequest);
		if (!SUCCESS.equalsIgnoreCase(voyageReply.getResponseStatus().getStatus())) {
			if (CommonErrorCodes.E_CPDSS_VOYAGE_EXISTS.equalsIgnoreCase(voyageReply.getResponseStatus().getCode())) {
				throw new GenericServiceException(voyageReply.getResponseStatus().getMessage(),
			...
			voyageResponse
					.setResponseStatus(new CommonSuccessResponse(String.valueOf(HttpStatus.OK.value()), correlationId));
	 ...
	 * @throws GenericServiceException CommonSuccessResponse
	 */
	public LoadableQuantityResponse saveLoadableQuantity(LoadableQuantity loadableQuantity, long loadableStudiesId,
		...
		Optional.ofNullable(loadableQuantity.getDistanceFromLastPort()).ifPresent(builder::setDistanceFromLastPort);
		...
		Optional.ofNullable(loadableQuantity.getFoConsumptionPerDay()).ifPresent(builder::setFoConsumptionPerDay);
		...
		Optional.ofNullable(loadableQuantity.getVesselAverageSpeed()).ifPresent(builder::setVesselAverageSpeed);
		Optional.ofNullable(loadableQuantity.getVesselLightWeight()).ifPresent(builder::setVesselLightWeight);
		...
		Optional.ofNullable(loadableQuantity.getBoilerWaterOnBoard()).ifPresent(builder::setBoilerWaterOnBoard);
		...
		Optional.ofNullable(loadableQuantity.getDraftRestriction()).ifPresent(builder::setDraftRestriction);
		...
		if (!SUCCESS.equalsIgnoreCase(loadableQuantityReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException(loadableQuantityReply.getResponseStatus().getMessage(),
					loadableQuantityReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(loadableQuantityReply.getResponseStatus().getStatusCode())));
		} else {

			loadableQuantityResponse
					.setResponseStatus(new CommonSuccessResponse(String.valueOf(HttpStatus.OK.value()), correlationId));
	...
	/**
	 * This method calls loadable study microservice to get a list of loadable
	 * studies by vessel and voyage
	 *
	 ...
	 * @throws GenericServiceException
	 */
	public LoadableStudyResponse getLoadableStudies(Long companyId, Long vesselId, Long voyageId, String correlationdId,
			Long planningType) throws GenericServiceException {
		log.info("fetching loadable studies. correlationId: {}", correlationdId);
		LoadableStudyRequest request = LoadableStudyRequest.newBuilder().setVesselId(vesselId)
				.setPlanningType(Common.PLANNING_TYPE.forNumber(planningType.intValue())).setVoyageId(voyageId).build();
		LoadableStudyReply reply = this.getloadableStudyList(request);
		if (!SUCCESS.equals(reply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("failed to fetch loadable studies", reply.getResponseStatus().getCode(),
		...
		for (LoadableStudyDetail grpcReply : reply.getLoadableStudiesList()) {
			...
			dto.setDraftMark(
					StringUtils.isEmpty(grpcReply.getDraftMark()) ? null : new BigDecimal(grpcReply.getDraftMark()));
			dto.setLoadLineXId(grpcReply.getLoadLineXId());
			dto.setDraftRestriction(StringUtils.isEmpty(grpcReply.getDraftRestriction()) ? null
					: new BigDecimal(grpcReply.getDraftRestriction()));
			dto.setMaxAirTemperature(StringUtils.isEmpty(grpcReply.getMaxAirTemperature()) ? null
					: new BigDecimal(grpcReply.getMaxAirTemperature()));
			dto.setMaxWaterTemperature(StringUtils.isEmpty(grpcReply.getMaxWaterTemperature()) ? null
			...
			dto.setLoadableStudyStatusLastModifiedTime(grpcReply.getLoadableStudyStatusLastModifiedTime());

			List<LoadableStudyAttachmentData> attachmentList = new ArrayList();

			grpcReply.getAttachmentsList().forEach(attachments -> {
				LoadableStudyAttachmentData loadableStudyAttachment = new LoadableStudyAttachmentData();
			...
			grpcReply.getOhqPortsList().forEach(ohqPort -> {
		...
		response.setResponseStatus(new CommonSuccessResponse(String.valueOf(HttpStatus.OK.value()), correlationdId));
		return response;
	}
	...
	public LoadableStudyResponse saveLoadableStudy(final LoadableStudy request, String correlationId,
			MultipartFile[] files) throws GenericServiceException, IOException {
		this.validateLoadableStudyFiles(files);
		Builder builder = LoadableStudyDetail.newBuilder();
		Optional.ofNullable(request.getDeletedAttachments()).ifPresent(builder::addAllDeletedAttachments);
		...
		Optional.ofNullable(request.getDraftRestriction())
				.ifPresent(draftRestriction -> builder.setDraftRestriction(String.valueOf(draftRestriction)));
		...
		Optional.ofNullable(request.getIsCargoNominationComplete()).ifPresent(builder::setIsCargoNominationComplete);
		...
		for (MultipartFile file : files) {
			builder.addAttachments(LoadableStudyAttachment.newBuilder()
					.setFileName(file.getOriginalFilename() == null ? "" : file.getOriginalFilename())
					.setByteString(ByteString.copyFrom(file.getBytes())).build());
		}
		LoadableStudyReply reply = this.saveLoadableStudy(builder.build());
		if (!SUCCESS.equals(reply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("failed to save loadable studies", reply.getResponseStatus().getCode(),
	...
	private void validateLoadableStudyFiles(MultipartFile[] files) throws GenericServiceException, IOException {
		log.info("validating attachment files");
		for (MultipartFile file : files) {
			if (file.getSize() > LOADABLE_STUDY_ATTACHEMENT_MAX_SIZE) {
				throw new GenericServiceException("loadable study attachment size exceeds maximum allowed size",
						CommonErrorCodes.E_HTTP_BAD_REQUEST, HttpStatusCode.BAD_REQUEST);
			}
			String originalFileName = file.getOriginalFilename() == null ? "" : file.getOriginalFilename();
			if (!ATTACHMENT_ALLOWED_EXTENSIONS
					.contains(originalFileName.substring(originalFileName.lastIndexOf(".") + 1).toLowerCase())) {
				throw new GenericServiceException("unsupported file type", CommonErrorCodes.E_HTTP_BAD_REQUEST,
	...
	/**
	 * Retrieves the cargo information from cargo master, port information from port
	 * master and cargo nomination details from loadable-study service
	 *
	...
	public CargoNominationResponse getCargoNomination(Long loadableStudyId, HttpHeaders headers)
		...
		CargoNominationRequest cargoNominationRequest = CargoNominationRequest.newBuilder()
				.setLoadableStudyId(loadableStudyId).build();
		CargoNominationReply cargoNominationReply = loadableStudyServiceBlockingStub
				.getCargoNominationById(cargoNominationRequest);
		if (SUCCESS.equalsIgnoreCase(cargoNominationReply.getResponseStatus().getStatus())) {
			...
			throw new GenericServiceException("Error calling getCargoNominationById service",
		...
		if (cargoReply != null && cargoReply.getResponseStatus() != null
			...
			throw new GenericServiceException("Error in calling cargo service", CommonErrorCodes.E_GEN_INTERNAL_ERR,
					HttpStatusCode.INTERNAL_SERVER_ERROR);
		}

		// Retrieve segregation List
		ValveSegregationRequest valveSegregationRequest = ValveSegregationRequest.newBuilder()
				.setLoadableStudyId(loadableStudyId).build();
		ValveSegregationReply valveSegregationReply = loadableStudyServiceBlockingStub
				.getValveSegregation(valveSegregationRequest);
		if (SUCCESS.equalsIgnoreCase(valveSegregationReply.getResponseStatus().getStatus())) {
			buildCargoNominationResponseWithValveSegregation(cargoNominationResponse, valveSegregationReply);
		} else {
			throw new GenericServiceException("Error calling getValveSegregation service",
					CommonErrorCodes.E_GEN_INTERNAL_ERR, HttpStatusCode.INTERNAL_SERVER_ERROR);
		}
	 ...
	 * @return
	 */
	private CargoNominationResponse buildCargoNominationResponse(CargoNominationResponse cargoNominationResponse,
			CargoNominationReply reply) {
		if (reply != null && !reply.getCargoNominationsList().isEmpty()) {
			List<CargoNomination> cargoNominationList = new ArrayList<>();
			reply.getCargoNominationsList().forEach(cargoNominationDetail -> {
				...
				if (!CollectionUtils.isEmpty(cargoNominationDetail.getLoadingPortDetailsList())) {
					...
					cargoNominationDetail.getLoadingPortDetailsList().forEach(port -> {
						LoadingPort loadingPort = new LoadingPort();
						loadingPort.setId(port.getPortId());
						loadingPort.setQuantity(
								port.getQuantity() != null ? new BigDecimal(port.getQuantity()) : new BigDecimal("0"));
				...
				cargoNomination.setMaxTolerance(!StringUtils.isEmpty(cargoNominationDetail.getMaxTolerance())
						? new BigDecimal(cargoNominationDetail.getMaxTolerance())
						: new BigDecimal("0"));
				cargoNomination.setMinTolerance(!StringUtils.isEmpty(cargoNominationDetail.getMinTolerance())
						? new BigDecimal(cargoNominationDetail.getMinTolerance())
						: new BigDecimal("0"));
				cargoNomination.setApi(!StringUtils.isEmpty(cargoNominationDetail.getApiEst())
						? new BigDecimal(cargoNominationDetail.getApiEst())
						: new BigDecimal("0"));
				cargoNomination.setTemperature(!StringUtils.isEmpty(cargoNominationDetail.getTempEst())
	...
	private CargoNominationResponse buildCargoNominationResponseWithValveSegregation(
		...
		if (reply != null && !reply.getValveSegregationList().isEmpty()) {
			...
			reply.getValveSegregationList().forEach(segregationDetail -> {
	...
	private CargoNominationResponse buildCargoNominationResponseWithCargo(
			CargoNominationResponse cargoNominationResponse, CargoNominationReply cargoNominationReply,
	 ...
	 * @throws GenericServiceException
	 */
	public CargoNominationResponse saveCargoNomination(Long vesselId, Long voyageId, Long loadableStudyId,
			CargoNomination request, HttpHeaders headers) throws GenericServiceException {
		CargoNominationResponse cargoNominationResponse = new CargoNominationResponse();
		// Build response with response status
		buildCargoNominationResponseWithResponseStatus(cargoNominationResponse);
		// Build cargoNomination payload for grpc call
		com.cpdss.common.generated.LoadableStudy.CargoNominationRequest.Builder builder = CargoNominationRequest
				.newBuilder();
		...
		if (!CollectionUtils.isEmpty(request.getLoadingPorts())) {
			request.getLoadingPorts().forEach(loadingPort -> {
				...
				Optional.ofNullable(loadingPort.getId()).ifPresent(loadingPortDetailBuilder::setPortId);
				Optional.ofNullable(loadingPort.getQuantity())
						.ifPresent(quantity -> loadingPortDetailBuilder.setQuantity(String.valueOf(quantity)));
		...
		Optional.ofNullable(request.getMaxTolerance())
				.ifPresent(maxTolerance -> cargoNominationDetailbuilder.setMaxTolerance(String.valueOf(maxTolerance)));
		Optional.ofNullable(request.getMinTolerance())
				.ifPresent(minTolerance -> cargoNominationDetailbuilder.setMinTolerance(String.valueOf(minTolerance)));
		...
		Optional.ofNullable(request.getTemperature())
				.ifPresent(temperature -> cargoNominationDetailbuilder.setTempEst(String.valueOf(temperature)));
		...
		Optional.ofNullable(request.getIsCargoNominationComplete())
				.ifPresent(isCargoNominationComplete -> cargoNominationDetailbuilder
						.setIsCargoNominationComplete(isCargoNominationComplete));
		builder.setCargoNominationDetail(cargoNominationDetailbuilder);
		CargoNominationRequest cargoNominationRequest = builder.build();
		CargoNominationReply cargoNominationReply = loadableStudyServiceBlockingStub
				.saveCargoNomination(cargoNominationRequest);
		if (!SUCCESS.equals(cargoNominationReply.getResponseStatus().getStatus())) {
			if (!StringUtils.isEmpty(cargoNominationReply.getResponseStatus().getCode())) {
				throw new GenericServiceException(
						...
						cargoNominationReply.getResponseStatus().getCode(), HttpStatusCode.valueOf(
								Integer.valueOf(cargoNominationReply.getResponseStatus().getHttpStatusCode())));
			} else {
				throw new GenericServiceException("GenericServiceException saveCargoNomination",
						CommonErrorCodes.E_GEN_INTERNAL_ERR, HttpStatusCode.INTERNAL_SERVER_ERROR);
			}
		}
	 ...
	 * @throws GenericServiceException
	 */
	public PortRotationResponse getLoadableStudyPortRotationList(Long vesselId, Long voyageId, Long loadableStudyId,
			Common.PLANNING_TYPE planningType, String correlationId) throws GenericServiceException {
		PortRotationResponse response = new PortRotationResponse();
		PortRotationReply grpcReply = this.getLoadableStudyPortRotationList(PortRotationRequest.newBuilder()
				.setLoadableStudyId(loadableStudyId).setVesselId(vesselId).setVoyageId(voyageId).build());
		if (!SUCCESS.equals(grpcReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("failed to fetch loadable study - ports",
		...
		for (PortRotationDetail portDetail : grpcReply.getPortsList()) {
			...
			port.setPortTimezoneId(0 == portDetail.getPortTimezoneId() ? null : portDetail.getPortTimezoneId());
			port.setSeaWaterDensity(
					isEmpty(portDetail.getSeaWaterDensity()) ? null : new BigDecimal(portDetail.getSeaWaterDensity()));
			port.setDistanceBetweenPorts(isEmpty(portDetail.getDistanceBetweenPorts()) ? null
					: new BigDecimal(portDetail.getDistanceBetweenPorts()));
			port.setTimeOfStay(isEmpty(portDetail.getTimeOfStay()) ? null : new BigDecimal(portDetail.getTimeOfStay()));
			port.setMaxDraft(isEmpty(portDetail.getMaxDraft()) ? null : new BigDecimal(portDetail.getMaxDraft()));
			port.setMaxAirDraft(
					isEmpty(portDetail.getMaxAirDraft()) ? null : new BigDecimal(portDetail.getMaxAirDraft()));
			...
			SynopticalTableRequest synopticalRequest = SynopticalTableRequest.newBuilder()
					.setLoadableStudyId(loadableStudyId).setPortId(portDetail.getPortId()).build();
			SynopticalTableReply synopticalDataReply = loadableStudyServiceBlockingStub
					.getSynopticalPortDataByPortId(synopticalRequest);
			if (synopticalDataReply != null && synopticalDataReply.getResponseStatus() != null
			...
			if (synopticalDataReply != null
				...
				synopticalDataReply.getSynopticalRecordsList().forEach(record -> {
					port.setDistanceBetweenPorts(
							!StringUtils.isEmpty(record.getDistance()) ? new BigDecimal(record.getDistance())
									: BigDecimal.ZERO);
	 ...
	 * @throws GenericServiceException
	 */
	public PortRotationResponse savePortRotation(PortRotation request, String correlationId, HttpHeaders headers)
			throws GenericServiceException {
		log.info("Inside savePortRotation");
		PortRotationReply grpcReply = this.savePortRotation(this.createPortRotationDetail(request, headers));
		if (!SUCCESS.equals(grpcReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("failed to save loadable study - ports",
					grpcReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(grpcReply.getResponseStatus().getHttpStatusCode())));
		}
		PortRotationResponse response = new PortRotationResponse();
		response.setId(grpcReply.getPortRotationId());
		response.setResponseStatus(new CommonSuccessResponse(valueOf(HttpStatus.OK.value()), correlationId));
		return response;
	}
	 ...
	 * @throws GenericServiceException
	 */
	public PortRotationResponse savePortRotationList(com.cpdss.gateway.domain.PortRotationRequest request,
			String correlationId) throws GenericServiceException {
		log.info("Inside savePortRotationList");
		PortRotationRequest savePortRotationListRequest = buildSavePortRotationList(request);
		PortRotationReply grpcReply = loadableStudyServiceBlockingStub
				.saveLoadableStudyPortRotationList(savePortRotationListRequest);
		if (!SUCCESS.equals(grpcReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("failed to save loadable study - ports",
					grpcReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(grpcReply.getResponseStatus().getHttpStatusCode())));
	...
	private PortRotationRequest buildSavePortRotationList(com.cpdss.gateway.domain.PortRotationRequest request) {
		...
		if (!CollectionUtils.isEmpty(request.getPortList())) {
			request.getPortList().forEach(requestPort -> {
				...
				Optional.ofNullable(requestPort.getDistanceBetweenPorts()).ifPresent(
	...
	public PortRotationDetail createPortRotationDetail(PortRotation request, HttpHeaders headers) {
		...
		Optional.ofNullable(request.getDistanceBetweenPorts())
				.ifPresent(item -> builder.setDistanceBetweenPorts(valueOf(request.getDistanceBetweenPorts())));
		Optional.ofNullable(request.getEta()).ifPresent(item -> builder.setEta(valueOf(request.getEta())));
		Optional.ofNullable(request.getEtd()).ifPresent(item -> builder.setEtd(valueOf(request.getEtd())));
		...
		Optional.ofNullable(request.getPortId()).ifPresent(item -> builder.setPortId(request.getPortId()));
		...
		Optional.ofNullable(request.getIsPortsComplete()).ifPresent(item -> builder.setIsPortsComplete(item));
		Optional.ofNullable(request.getType()).ifPresent(builder::setOperationType);
		List<String> referer = headers.get("Referer");
		if (referer != null
				&& (referer.get(0).contains(VOYAGE_STATUS_URI) || referer.get(0).contains(OPERATIONS_URI))) {
	 ...
	 */
	public LoadableQuantityResponse getLoadableQuantity(long loadableStudyId, Long portRotationId, String correlationId)
			throws GenericServiceException {
		...
		if (nullCheckPortRotationId.isPresent()) {
			loadableQuantityRequest = LoadableQuantityReply.newBuilder().setLoadableStudyId(loadableStudyId)
					.setPortRotationId(portRotationId).build();
		} else {
			loadableQuantityRequest = LoadableQuantityReply.newBuilder().setLoadableStudyId(loadableStudyId)
					.setPortRotationId(-1).build();
		}
		com.cpdss.common.generated.LoadableStudy.LoadableQuantityResponse loadableQuantityResponse = this
				.getLoadableQuantityResponse(loadableQuantityRequest);
		if (!SUCCESS.equalsIgnoreCase(loadableQuantityResponse.getResponseStatus().getStatus())) {
			throw new GenericServiceException(loadableQuantityResponse.getResponseStatus().getMessage(),
					loadableQuantityResponse.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(loadableQuantityResponse.getResponseStatus().getCode())));
		}
		loadableQuantity.setLoadableQuantityId(loadableQuantityResponse.getLoadableQuantityRequest().getId());
		loadableQuantity.setConstant(loadableQuantityResponse.getLoadableQuantityRequest().getConstant());
		...
		loadableQuantity.setEstDOOnBoard(loadableQuantityResponse.getLoadableQuantityRequest().getEstDOOnBoard());
		loadableQuantity.setEstFOOnBoard(loadableQuantityResponse.getLoadableQuantityRequest().getEstFOOnBoard());
		loadableQuantity.setEstFreshWaterOnBoard(
				loadableQuantityResponse.getLoadableQuantityRequest().getEstFreshWaterOnBoard());
		loadableQuantity.setEstSagging(loadableQuantityResponse.getLoadableQuantityRequest().getEstSagging());
		loadableQuantity.setEstSeaDensity(loadableQuantityResponse.getLoadableQuantityRequest().getEstSeaDensity());

		loadableQuantity
				.setFoConsumptionPerDay(loadableQuantityResponse.getLoadableQuantityRequest().getFoConsumptionPerDay());

		loadableQuantity
				.setVesselLightWeight(loadableQuantityResponse.getLoadableQuantityRequest().getVesselLightWeight());
		loadableQuantity.setOtherIfAny(loadableQuantityResponse.getLoadableQuantityRequest().getOtherIfAny());
		loadableQuantity
				.setSaggingDeduction(loadableQuantityResponse.getLoadableQuantityRequest().getSaggingDeduction());
		loadableQuantity.setSgCorrection(loadableQuantityResponse.getLoadableQuantityRequest().getSgCorrection());
		loadableQuantity.setTotalQuantity(loadableQuantityResponse.getLoadableQuantityRequest().getTotalQuantity());
		loadableQuantity.setTpc(loadableQuantityResponse.getLoadableQuantityRequest().getTpc());
		loadableQuantity
				.setVesselAverageSpeed(loadableQuantityResponse.getLoadableQuantityRequest().getVesselAverageSpeed());
		loadableQuantity
				.setUpdateDateAndTime(loadableQuantityResponse.getLoadableQuantityRequest().getUpdateDateAndTime());
		...
		loadableQuantity.setPortRotationId(loadableQuantityResponse.getLoadableQuantityRequest().getPortRotationId());
		loadableQuantity
				.setBoilerWaterOnBoard(loadableQuantityResponse.getLoadableQuantityRequest().getBoilerWaterOnBoard());
		loadableQuantity.setBallast(loadableQuantityResponse.getLoadableQuantityRequest().getBallast());
		loadableQuantity.setRunningHours(loadableQuantityResponse.getLoadableQuantityRequest().getRunningHours());
		loadableQuantity.setRunningDays(loadableQuantityResponse.getLoadableQuantityRequest().getRunningDays());
		loadableQuantity.setFoConInSZ(loadableQuantityResponse.getLoadableQuantityRequest().getFoConInSZ());
		loadableQuantity
				.setDraftRestriction(loadableQuantityResponse.getLoadableQuantityRequest().getDraftRestriction());
		loadableQuantity.setSubTotal(loadableQuantityResponse.getLoadableQuantityRequest().getSubTotal());
		loadableQuantity.setDwt(loadableQuantityResponse.getLoadableQuantityRequest().getDwt());
		loadableQuantity.setLastUpdatedTime(loadableQuantityResponse.getLoadableQuantityRequest().getLastUpdatedTime());
		loadableQuantityResponseDto
				.setLoadableQuantityId(loadableQuantityResponse.getLoadableQuantityRequest().getId());
	 ...
	 */
	public com.cpdss.common.generated.LoadableStudy.LoadableQuantityResponse getLoadableQuantityResponse(
			LoadableQuantityReply loadableQuantityRequest) {
	 ...
	 * @throws GenericServiceException
	 */
	public VoyageResponse getVoyageListByVessel(Long vesselId, String correlationId) throws GenericServiceException {
		...
		if (!SUCCESS.equals(grpcReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("failed to fetch voyage list", grpcReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(grpcReply.getResponseStatus().getCode())));
		}
		...
		for (VoyageDetail detail : grpcReply.getVoyagesList()) {
			...
			Optional.ofNullable(detail.getActualStartDate()).filter(it -> !it.isEmpty())
					.ifPresent(actualStartDate -> voyage
							.setActualStartDate(LocalDate.parse(detail.getActualStartDate(), formatter)));

			Optional.ofNullable(detail.getActualEndDate()).filter(it -> !it.isEmpty()).ifPresent(
					actualEndDate -> voyage.setActualEndDate(LocalDate.parse(detail.getActualEndDate(), formatter)));
	...
	public CargoNominationResponse deleteCargoNomination(Long cargoNominationId, HttpHeaders headers)
		...
		CargoNominationRequest cargoNominationRequest = builder.build();
		CargoNominationReply cargoNominationReply = loadableStudyServiceBlockingStub
				.deleteCargoNomination(cargoNominationRequest);
		if (cargoNominationReply != null && cargoNominationReply.getResponseStatus() != null
			...
			throw new GenericServiceException("Error in calling deleteCargoNomination",
					cargoNominationReply.getResponseStatus().getCode(), HttpStatusCode
							.valueOf(Integer.valueOf(cargoNominationReply.getResponseStatus().getHttpStatusCode())));
		}
		return cargoNominationResponse;
	}
	 ...
	 * @throws GenericServiceException
	 */
	public PortRotationResponse saveDischargingPorts(DischargingPortRequest request, String correlationId)
			throws GenericServiceException {
		log.info("Inside savePortRotation");
		...
		Optional.ofNullable(request.getCargoNominationId()).ifPresent(portRotationRequestBuilder::setCargoNominationId);

		Optional.ofNullable(request.getIsDischargingPortComplete())
				.ifPresent(isDischargingPortComplete -> portRotationRequestBuilder
						.setIsDischargingPortsComplete(request.getIsDischargingPortComplete()));

		PortRotationReply grpcReply = this.saveDischargingPorts(portRotationRequestBuilder.build());
		if (!SUCCESS.equals(grpcReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("failed to save discharging ports",
					grpcReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(grpcReply.getResponseStatus().getHttpStatusCode())));
		}
		PortRotationResponse response = new PortRotationResponse();
	 ...
	 * @throws GenericServiceException
	 */
	public LoadableStudyResponse deleteLoadableStudy(final Long loadableStudyId, final String correlationId)
			throws GenericServiceException {
		LoadableStudyRequest request = LoadableStudyRequest.newBuilder().setLoadableStudyId(loadableStudyId).build();
		LoadableStudyReply grpcReply = this.deleteLoadableStudy(request);
		if (!SUCCESS.equals(grpcReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("failed to delete loadable study",
					grpcReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(grpcReply.getResponseStatus().getHttpStatusCode())));
		}
		LoadableStudyResponse response = new LoadableStudyResponse();
		response.setResponseStatus(new CommonSuccessResponse(String.valueOf(HttpStatusCode.OK.value()), correlationId));
		return response;
	}
	 ...
	 */
	public PortRotationResponse getPortRotation(@Min(value = 1, message = "400") Long loadableStudyId,
			String correlationId) throws GenericServiceException {
		log.info("Inside getPortRotation gateway service with correlationId : " + correlationId);
		PortRotationRequest portRotationRequest = PortRotationRequest.newBuilder().setLoadableStudyId(loadableStudyId)
				.build();
		...
		if (!SUCCESS.equals(portRotationReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("failed to get Port Rotation",
		...
		PortRotationResponse response = new PortRotationResponse();
		response.setPortList(new ArrayList<>());
		portRotationReply.getPortsList().forEach(port -> {
			PortRotation portRotation = new PortRotation();
	...
	public PortRotationReply getPortRotation(PortRotationRequest portRotationRequest) {
		return this.loadableStudyServiceBlockingStub.getPortRotationByLoadableStudyId(portRotationRequest);
	}
	 ...
	 * @throws GenericServiceException
	 */
	public PortRotationResponse deletePortRotation(Long loadableStudyId, Long id, String correlationId)
			throws GenericServiceException {
		PortRotationRequest request = PortRotationRequest.newBuilder().setId(id).setLoadableStudyId(loadableStudyId)
				.build();
		PortRotationReply grpcReply = this.deletePortRotation(request);
		if (!SUCCESS.equals(grpcReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("failed to delete port rotation", grpcReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(grpcReply.getResponseStatus().getHttpStatusCode())));
		}
		PortRotationResponse response = new PortRotationResponse();
		response.setResponseStatus(new CommonSuccessResponse(valueOf(HttpStatus.OK.value()), correlationId));
	 ...
	 */
	public LoadablePatternResponse getLoadablePatterns(Long loadableStudiesId, Long vesselId, String correlationId)
			throws GenericServiceException {
		LoadablePatternRequest loadablePatternRequest = LoadablePatternRequest.newBuilder()
				.setLoadableStudyId(loadableStudiesId).build();
		...
		if (!SUCCESS.equals(loadablePatternReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("failed to get LoadablePattern ",
					loadablePatternReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(loadablePatternReply.getResponseStatus().getCode())));
		}
		return this.buildLoadablePatternResponse(loadableStudiesId, loadablePatternReply, vesselId, correlationId);
	}
	 ...
	 * @return LoadablePatternResponse
	 */
	private LoadablePatternResponse buildLoadablePatternResponse(Long loadableStudiesId,
			LoadablePatternReply loadablePatternReply, Long vesselId, String correlationId) {
		LoadablePatternResponse loadablePatternResponse = new LoadablePatternResponse();
		loadablePatternResponse.setConfirmPlanEligibility(loadablePatternReply.getConfirmPlanEligibility());
		...
		loadablePatternResponse.setTankLists(createGroupWiseTankList(loadablePatternReply.getTanksList()));
		// DSS-2016
		// loadablePatternResponse.setTankLists(createGroupWiseTankList(grpcReply.getTanksList()));
		loadablePatternResponse
				.setFrontBallastTanks(createGroupWiseTankList(loadablePatternReply.getBallastFrontTanksList()));
		loadablePatternResponse
				.setCenterBallastTanks(createGroupWiseTankList(loadablePatternReply.getBallastCenterTanksList()));
		loadablePatternResponse
				.setRearBallastTanks(createGroupWiseTankList(loadablePatternReply.getBallastRearTanksList()));
		LocalDateTime startTime = LocalDateTime.now();

		loadablePatternReply.getLoadablePatternList().forEach(loadablePattern -> {
			...
			loadablePatternDto
					.setStabilityParameters(buildStabilityParameter(loadablePattern.getStabilityParameters()));
			loadablePatternDto.setLoadableStudyStatusId(loadablePattern.getLoadableStudyStatusId());
			loadablePatternDto.setValidated(loadablePattern.getValidated());
			loadablePatternDto.setLoadablePatternStatusId(loadablePattern.getLoadablePatternStatusId());
			loadablePatternDto.setLoadablePatternCargoDetails(new ArrayList<LoadablePatternCargoDetails>());
			loadablePattern.getLoadablePatternCargoDetailsList().forEach(loadablePatternCargoDetail -> {
				LoadablePatternCargoDetails loadablePatternCargoDetails = new LoadablePatternCargoDetails();
				Optional.ofNullable(loadablePatternCargoDetail.getPriority())
						.ifPresent(priority -> loadablePatternCargoDetails.setPriority(priority));
				Optional.ofNullable(loadablePatternCargoDetail.getQuantity())
						.ifPresent(quantity -> loadablePatternCargoDetails.setQuantity(String.valueOf(quantity)));

				Optional.ofNullable(loadablePatternCargoDetail.getOrderedQuantity())
						.ifPresent(orderedQuantity -> loadablePatternCargoDetails
								.setOrderedQuantity(String.valueOf(orderedQuantity)));

				Optional.ofNullable(loadablePatternCargoDetail.getCargoAbbreviation()).ifPresent(
						cargoAbbreviation -> loadablePatternCargoDetails.setCargoAbbreviation(cargoAbbreviation));
				Optional.ofNullable(loadablePatternCargoDetail.getCargoColor())
						.ifPresent(cargoColor -> loadablePatternCargoDetails.setCargoColor(cargoColor));

				Optional.ofNullable(loadablePatternCargoDetail.getIsCommingle())
						.ifPresent(commingle -> loadablePatternCargoDetails.setIsCommingle(commingle));

				Optional.ofNullable(loadablePatternCargoDetail.getTankName())
						.ifPresent(tankName -> loadablePatternCargoDetails.setTankName(tankName));

				Optional.ofNullable(loadablePatternCargoDetail.getLoadablePatternCommingleDetailsId())
						.ifPresent(id -> loadablePatternCargoDetails.setLoadablePatternCommingleDetailsId(id));

				Optional.ofNullable(loadablePatternCargoDetail.getLoadingOrder())
						.ifPresent(loadingOrder -> loadablePatternCargoDetails.setLoadingOrder(loadingOrder));
				...
				loadablePatternDto.getLoadablePatternCargoDetails().add(loadablePatternCargoDetails);
			});
			// DSS-2016
			try {
				buildSynopticTableForPlans(loadablePatternDto, loadableStudiesId, vesselId,
			...
			loadablePatternDto.setLoadablePlanStowageDetails(buildLoadableStudyStowageDetails(loadablePattern));
		...
		loadablePatternResponse
				.setResponseStatus(new CommonSuccessResponse(valueOf(HttpStatus.OK.value()), correlationId));
		long diff = ChronoUnit.SECONDS.between(startTime, LocalDateTime.now());
		log.info("Loadable patterns time (sec) taken - {}", diff);
		return loadablePatternResponse;
	}

	private void buildLoadableStudyBallastDetails(LoadablePattern response,
		...
		grpcReply.getLoadablePlanBallastDetailsList().forEach(lpbd -> {
	...
	private void buildLoadableStudyQuantity(LoadablePattern response,
			com.cpdss.common.generated.LoadableStudy.LoadablePattern grpcReply) {
		response.setLoadableQuantityCargoDetails(
				new ArrayList<com.cpdss.gateway.domain.LoadableQuantityCargoDetails>());
		grpcReply.getLoadableQuantityCargoDetailsList().forEach(lqcd -> {
			com.cpdss.gateway.domain.LoadableQuantityCargoDetails cargoDetails = new com.cpdss.gateway.domain.LoadableQuantityCargoDetails();
			...
			if (!lqcd.getLoadingPortsList().isEmpty()) {
				List<String> ports = lqcd.getLoadingPortsList().stream().map(var -> var.getName())
	...
	private void buildLoadableStudyCommingleCargoDetails(LoadablePattern response,
			com.cpdss.common.generated.LoadableStudy.LoadablePattern grpcReply) {
		response.setLoadableQuantityCommingleCargoDetails(new ArrayList<LoadableQuantityCommingleCargoDetails>());
		grpcReply.getLoadableQuantityCommingleCargoDetailsList().forEach(lqccd -> {
	 ...
	 * @return SynopticalTableResponse - Single data 'operation type' (DEP) will
	 *         return from LS
	 * @throws GenericServiceException
	 */
	public SynopticalTableResponse getSingleSynopticDataByLSId(Long vesselId, Long loadableStudyId,
			Long loadablePatternId) throws GenericServiceException {
		...
		SynopticalTableRequest synopticalTableRequest = SynopticalTableRequest.newBuilder()
				.setLoadableStudyId(loadableStudyId).setVesselId(vesselId).setLoadablePatternId(loadablePatternId)
				.build();
		SynopticalTableReply synopticalTableReply = this.loadableStudyServiceBlockingStub
				.getSynopticDataByLoadableStudyId(synopticalTableRequest);
		if (SUCCESS.equalsIgnoreCase(synopticalTableReply.getResponseStatus().getStatus())) {
			...
			throw new GenericServiceException("Error calling getSynopticalTable service",
					synopticalTableReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(synopticalTableReply.getResponseStatus().getCode())));
		}
		return synopticalTableResponse;
	}

	private void buildSynopticTableForPlans(LoadablePattern response, Long loadableStudyId, Long vesselId,
			Long loadablePatternId) throws GenericServiceException {

		SynopticalTableResponse synopticalTableResponse = getSingleSynopticDataByLSId(vesselId, loadableStudyId,
				loadablePatternId);
		if (!synopticalTableResponse.getResponseStatus().getStatus().equals(String.valueOf(HttpStatus.OK.value()))) {
			throw new GenericServiceException("Failed to get synoptical table data",
		...
		if (!synopticalTableResponse.getSynopticalRecords().isEmpty()) {
			synopticalTableResponse.getSynopticalRecords().stream().limit(1).forEach(str -> {
	 ...
	 */
	private com.cpdss.gateway.domain.StabilityParameter buildStabilityParameter(StabilityParameter stabilityParams) {
	...
	private List<String> buildLoadableStudyConstraints(
		...
		loadablePattern.getConstraintsList().forEach(constrain -> {
	...
	private List<LoadablePlanStowageDetails> buildLoadableStudyStowageDetails(
			com.cpdss.common.generated.LoadableStudy.LoadablePattern loadablePattern) {
		List<LoadablePlanStowageDetails> loadablePlanStowageDetails = new ArrayList<LoadablePlanStowageDetails>();
		loadablePattern.getLoadablePlanStowageDetailsList().forEach(lpsdl -> {
	 ...
	 * @throws GenericServiceException
	 */
	public OnHandQuantityResponse getOnHandQuantity(final Long companyId, final Long vesselId,
			final Long loadableStudyId, final Long portRotationId, String correlationId)
			throws GenericServiceException {
		OnHandQuantityRequest request = OnHandQuantityRequest.newBuilder().setCompanyId(companyId).setVesselId(vesselId)
				.setLoadableStudyId(loadableStudyId).setPortRotationId(portRotationId).build();
		OnHandQuantityReply grpcReply = this.getOnHandQuantity(request);
		if (!SUCCESS.equals(grpcReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("Failed to fetch on hand quantities",
					grpcReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(grpcReply.getResponseStatus().getCode())));
		}
	 ...
	 * @return
	 */
	public OnHandQuantityResponse buildOnHandQuantityResponse(OnHandQuantityReply grpcReply, String correlationId) {
		...
		for (OnHandQuantityDetail detail : grpcReply.getOnHandQuantityList()) {
			...
			onHandQuantity.setArrivalQuantity(isEmpty(detail.getArrivalQuantity()) ? BigDecimal.ZERO
					: new BigDecimal(detail.getArrivalQuantity()));
			onHandQuantity.setActualArrivalQuantity(isEmpty(detail.getActualArrivalQuantity()) ? BigDecimal.ZERO
					: new BigDecimal(detail.getActualArrivalQuantity()));
			onHandQuantity.setArrivalVolume(
					isEmpty(detail.getArrivalVolume()) ? BigDecimal.ZERO : new BigDecimal(detail.getArrivalVolume()));
			onHandQuantity.setDepartureQuantity(isEmpty(detail.getDepartureQuantity()) ? BigDecimal.ZERO
					: new BigDecimal(detail.getDepartureQuantity()));
			onHandQuantity.setActualDepartureQuantity(isEmpty(detail.getActualDepartureQuantity()) ? BigDecimal.ZERO
					: new BigDecimal(detail.getActualDepartureQuantity()));
			onHandQuantity.setDepartureVolume(isEmpty(detail.getDepartureVolume()) ? BigDecimal.ZERO
					: new BigDecimal(detail.getDepartureVolume()));
			onHandQuantity.setColorCode(isEmpty(detail.getColorCode()) ? null : detail.getColorCode());
			onHandQuantity
					.setDensity(isEmpty(detail.getDensity()) ? BigDecimal.ZERO : new BigDecimal(detail.getDensity()));
	 ...
	 * @return
	 */
	private OnHandQuantityResponse buildOhqTankLayoutArray(OnHandQuantityResponse response,
			OnHandQuantityReply grpcReply) {
		...
		for (TankList list : tankList) {
			...
			for (TankDetail detail : list.getVesselTankList()) {
				...
				tank.setFillCapcityCubm(
						isEmpty(detail.getFillCapacityCubm()) ? null : new BigDecimal(detail.getFillCapacityCubm()));
				tank.setFullCapacityCubm(isEmpty(detail.getFullCapacityCubm()) ? null : detail.getFullCapacityCubm());
	...
	public OnHandQuantityResponse saveOnHandQuantity(OnHandQuantity request, String correlationId)
		...
		Optional.ofNullable(request.getArrivalQuantity()).ifPresent(item -> builder.setArrivalQuantity(valueOf(item)));
		Optional.ofNullable(request.getArrivalVolume()).ifPresent(item -> builder.setArrivalVolume(valueOf(item)));
		Optional.ofNullable(request.getDepartureQuantity())
				.ifPresent(item -> builder.setDepartureQuantity(valueOf(item)));
		Optional.ofNullable(request.getDepartureVolume()).ifPresent(item -> builder.setDepartureVolume(valueOf(item)));
		...
		if (!SUCCESS.equals(grpcReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("Failed to save on hand quantities",
					grpcReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(grpcReply.getResponseStatus().getHttpStatusCode())));
		}
	...
	/**
	 * Retrieves the commingle cargo information along with vessel cargo tanks
	 * lookup array
	 *
	 * @param loadableStudyId
	 * @param headers
	...
	public CommingleCargoResponse getCommingleCargo(Long loadableStudyId, Long vesselId)
		...
		CommingleCargoRequest commingleCargoRequest = CommingleCargoRequest.newBuilder().setVesselId(vesselId)
				.setLoadableStudyId(loadableStudyId).build();
		CommingleCargoReply commingleCargoReply = loadableStudyServiceBlockingStub
				.getCommingleCargo(commingleCargoRequest);
		if (SUCCESS.equalsIgnoreCase(commingleCargoReply.getResponseStatus().getStatus())) {
			...
			throw new GenericServiceException("Error calling getCommingleCargo service",
					CommonErrorCodes.E_GEN_INTERNAL_ERR, HttpStatusCode.INTERNAL_SERVER_ERROR);
		}
		// Retrieve purpose of commingle
		PurposeOfCommingleRequest purposeOfCommingleRequest = PurposeOfCommingleRequest.newBuilder()
				.setLoadableStudyId(loadableStudyId).build();
		PurposeOfCommingleReply purposeOfCommingleReply = loadableStudyServiceBlockingStub
				.getPurposeOfCommingle(purposeOfCommingleRequest);
		if (SUCCESS.equalsIgnoreCase(purposeOfCommingleReply.getResponseStatus().getStatus())) {
			buildCommingleCargoResponseWithPurposeOfCommingle(commingleCargoResponse, purposeOfCommingleReply);
		} else {
			throw new GenericServiceException("Error calling getPurposeOfCommingle service",
					CommonErrorCodes.E_GEN_INTERNAL_ERR, HttpStatusCode.INTERNAL_SERVER_ERROR);
		}
		return commingleCargoResponse;
	}

	private void buildCommingleCargoResponse(CommingleCargoResponse commingleCargoResponse,
		...
		if (commingleCargoReply != null && !commingleCargoReply.getCommingleCargoList().isEmpty()) {
			...
			commingleCargoReply.getCommingleCargoList().forEach(commingleCargoGen -> {
		...
		if (commingleCargoReply != null && !CollectionUtils.isEmpty(commingleCargoReply.getTanksList())) {
			List<VesselTank> vesselTankList = new ArrayList<>();
			commingleCargoReply.getTanksList().forEach(tankDetailList -> {
				if (tankDetailList != null && !CollectionUtils.isEmpty(tankDetailList.getVesselTankList())) {
					tankDetailList.getVesselTankList().forEach(tankDetail -> {
	...
	/**
	 * builds commingleCargoResponse from the cargo nomination data for the specific
	 * loadable study
	 *
	 ...
	 * @return
	 */
	private CommingleCargoResponse buildCommingleCargoResponseWithCargos(CommingleCargoResponse commingleCargoResponse,
			CargoNominationReply reply) {
		if (reply != null && !reply.getCargoNominationsList().isEmpty()) {
			List<CargoNomination> cargoNominationList = new ArrayList<>();
			List<CargoNominationDetail> cargoNominationDetailsFiltered = reply.getCargoNominationsList().stream()
					...
					.collect(Collectors.toList());
			cargoNominationDetailsFiltered.forEach(cargoNominationDetail -> {
				CargoNomination cargoNomination = new CargoNomination();
				cargoNomination.setId(cargoNominationDetail.getId());
				cargoNomination.setColor(cargoNominationDetail.getColor());
				cargoNomination.setCargoId(cargoNominationDetail.getCargoId());
	 ...
	 * @param reply
	 */
	private void buildCommingleCargoResponseWithPurposeOfCommingle(CommingleCargoResponse commingleCargoResponse,
			PurposeOfCommingleReply reply) {
		if (reply != null && !reply.getPurposeOfCommingleList().isEmpty()) {
			List<Purpose> purposeList = new ArrayList<>();
			reply.getPurposeOfCommingleList().forEach(purposeDetail -> {
	...
	public CommingleCargoResponse saveCommingleCargo(Long loadableStudyId,
			com.cpdss.gateway.domain.CommingleCargo commingleCargo) throws GenericServiceException {
		CommingleCargoResponse commingleCargoResponse = new CommingleCargoResponse();
		// Build response with response status
		CommonSuccessResponse commonSuccessResponse = new CommonSuccessResponse();
		commonSuccessResponse.setStatus(String.valueOf(HttpStatus.OK.value()));
		...
		if (commingleCargo != null) {
			com.cpdss.common.generated.LoadableStudy.CommingleCargoRequest.Builder builder = buildSaveCommingleCargoRequest(
					loadableStudyId, commingleCargo);
			CommingleCargoRequest commingleCargoRequest = builder.build();
			CommingleCargoReply commingleCargoReply = loadableStudyServiceBlockingStub
					.saveCommingleCargo(commingleCargoRequest);
			if (commingleCargoReply != null && commingleCargoReply.getResponseStatus() != null
					&& !SUCCESS.equalsIgnoreCase(commingleCargoReply.getResponseStatus().getStatus())) {
				throw new GenericServiceException("Error in saving commingle cargo",
						commingleCargoReply.getResponseStatus().getCode(), HttpStatusCode
								.valueOf(Integer.valueOf(commingleCargoReply.getResponseStatus().getHttpStatusCode())));
			}
		}
		return commingleCargoResponse;
	}
	 ...
	 * @return
	 */
	private com.cpdss.common.generated.LoadableStudy.CommingleCargoRequest.Builder buildSaveCommingleCargoRequest(
			Long loadableStudyId, CommingleCargo commingleCargoRequest) {
		com.cpdss.common.generated.LoadableStudy.CommingleCargoRequest.Builder builder = CommingleCargoRequest
				.newBuilder();
		if (commingleCargoRequest != null) {
			Optional.ofNullable(loadableStudyId).ifPresent(builder::setLoadableStudyId);
			// build inner commingleCargo object
			Long purposeId = commingleCargoRequest.getPurposeId() != null ? commingleCargoRequest.getPurposeId() : 0;
			boolean slopOnly = commingleCargoRequest.isSlopOnly();
			List<Long> preferredTanks = !CollectionUtils.isEmpty(commingleCargoRequest.getPreferredTanks())
					? commingleCargoRequest.getPreferredTanks()
					: Collections.emptyList();
			if (!CollectionUtils.isEmpty(commingleCargoRequest.getCargoGroups())) {
				commingleCargoRequest.getCargoGroups().forEach(cargoGroup -> {
					com.cpdss.common.generated.LoadableStudy.CommingleCargo.Builder commingleCargoBuilder = com.cpdss.common.generated.LoadableStudy.CommingleCargo
							.newBuilder();
					...
					Optional.ofNullable(preferredTanks).ifPresent(commingleCargoBuilder::addAllPreferredTanks);
					Optional.ofNullable(cargoGroup.getCargoNomination1Id())
							.ifPresent(commingleCargoBuilder::setCargoNomination1Id);
					Optional.ofNullable(cargoGroup.getCargo1Id()).ifPresent(commingleCargoBuilder::setCargo1Id);
					Optional.ofNullable(cargoGroup.getCargo1pct())
							.ifPresent(cargo1pct -> commingleCargoBuilder.setCargo1Pct(String.valueOf(cargo1pct)));
					Optional.ofNullable(cargoGroup.getCargoNomination2Id())
							.ifPresent(commingleCargoBuilder::setCargoNomination2Id);
					Optional.ofNullable(cargoGroup.getCargo2Id()).ifPresent(commingleCargoBuilder::setCargo2Id);
					Optional.ofNullable(cargoGroup.getCargo2pct())
							.ifPresent(cargo2pct -> commingleCargoBuilder.setCargo2Pct(String.valueOf(cargo2pct)));
					Optional.ofNullable(cargoGroup.getQuantity())
							.ifPresent(quantity -> commingleCargoBuilder.setQuantity(String.valueOf(quantity)));
	 ...
	 */
	public LoadablePatternDetailsResponse getLoadablePatternCommingleDetails(Long loadablePatternCommingleDetailsId,
			String correlationId) throws GenericServiceException {
		LoadablePatternCommingleDetailsRequest loadablePatternCommingleDetailsRequest = LoadablePatternCommingleDetailsRequest
				.newBuilder().setLoadablePatternCommingleDetailsId(loadablePatternCommingleDetailsId).build();
		LoadablePatternCommingleDetailsReply loadablePatternCommingleDetailsReply = this
				.getLoadablePatternCommingleDetails(loadablePatternCommingleDetailsRequest);
		if (!SUCCESS.equals(loadablePatternCommingleDetailsReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("failed to get Loadable Pattern Commingle Details ",
					loadablePatternCommingleDetailsReply.getResponseStatus().getCode(), HttpStatusCode.valueOf(
							Integer.valueOf(loadablePatternCommingleDetailsReply.getResponseStatus().getCode())));
		}
		return this.buildLoadablePatternCommingleDetailsResponse(loadablePatternCommingleDetailsReply, correlationId);
	}
	...
	private LoadablePatternDetailsResponse buildLoadablePatternCommingleDetailsResponse(
			LoadablePatternCommingleDetailsReply loadablePatternCommingleDetailsReply, String correlationId) {
		LoadablePatternDetailsResponse loadablePatternDetailsResponse = new LoadablePatternDetailsResponse();
		...
		loadablePatternDetailsResponse
				.setCargo1Abbreviation(loadablePatternCommingleDetailsReply.getCargo1Abbrivation());
		loadablePatternDetailsResponse
				.setCargo2Abbreviation(loadablePatternCommingleDetailsReply.getCargo2Abbrivation());
		loadablePatternDetailsResponse.setCargo1Percentage(loadablePatternCommingleDetailsReply.getCargo1Percentage());
		loadablePatternDetailsResponse.setCargo2Percentage(loadablePatternCommingleDetailsReply.getCargo2Percentage());
		loadablePatternDetailsResponse.setCargo1Quantity(loadablePatternCommingleDetailsReply.getCargo1Quantity());
		loadablePatternDetailsResponse.setCargo2Quantity(loadablePatternCommingleDetailsReply.getCargo2Quantity());
		...
		loadablePatternDetailsResponse.setTankShortName(loadablePatternCommingleDetailsReply.getTankShortName());
		loadablePatternDetailsResponse.setTemperature(loadablePatternCommingleDetailsReply.getTemperature());
		loadablePatternDetailsResponse
				.setResponseStatus(new CommonSuccessResponse(String.valueOf(HttpStatus.OK.value()), correlationId));
		return loadablePatternDetailsResponse;
	}
	...
	public LoadablePatternCommingleDetailsReply getLoadablePatternCommingleDetails(
		...
		return this.loadableStudyServiceBlockingStub
				.getLoadablePatternCommingleDetails(loadablePatternCommingleDetailsRequest);
	}
	...
	public AlgoPatternResponse generateLoadablePatterns(Long loadableStudyId, String correlationId)
			throws GenericServiceException {
		log.info("Inside generateLoadablePatterns gateway service with correlationId : " + correlationId);
		...
		if (!SUCCESS.equals(reply.getResponseStatus().getStatus())) {
			if (reply.getResponseStatus().getCode().equals(CommonErrorCodes.E_CPDSS_LS_INVALID_LQ)) {
				...
				throw new GenericServiceException("Generate pattern failed, On Invalid Loadable Quantity",
						reply.getResponseStatus().getCode(),
						reply.getResponseStatus().getCode().equals(CommonErrorCodes.E_CPDSS_LS_INVALID_LQ)
								? HttpStatusCode.BAD_REQUEST
								: HttpStatusCode
										.valueOf(Integer.valueOf(reply.getResponseStatus().getHttpStatusCode())));
			}

			if (reply.getResponseStatus().getCode().equals(CommonErrorCodes.E_CPDSS_LS_INVALID_COMMINGLE_QUANTITY)) {
				...
				throw new GenericServiceException(
						...
						reply.getResponseStatus().getCode(),
						reply.getResponseStatus().getCode().equals(
								CommonErrorCodes.E_CPDSS_LS_INVALID_COMMINGLE_QUANTITY) ? HttpStatusCode.BAD_REQUEST
										: HttpStatusCode.valueOf(
												Integer.valueOf(reply.getResponseStatus().getHttpStatusCode())));
			}

			throw new GenericServiceException("failed to call algo", reply.getResponseStatus().getCode(),
	 ...
	 * @throws GenericServiceException
	 */
	public OnBoardQuantityResponse getOnBoardQuantites(Long vesselId, Long voyageId, Long loadableStudyId, Long portId,
			String correlationId) throws GenericServiceException {
		log.info("LoadableStudyService - getOnBoardQuantites, correlationId:{}", correlationId);
		log.debug("getOnBoardQuantites, vesselId:{}, loadableStudyId:{}, portId:{}", vesselId, loadableStudyId, portId);
		OnBoardQuantityRequest request = OnBoardQuantityRequest.newBuilder().setVoyageId(voyageId)
				.setLoadableStudyId(loadableStudyId).setVesselId(vesselId).setPortId(portId).build();
		OnBoardQuantityReply grpcReply = this.getOnBoardQuantites(request);
		if (!SUCCESS.equals(grpcReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("Error calling getOnBoardQuantites grpc service",
					grpcReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(grpcReply.getResponseStatus().getCode())));
		}
	 ...
	 * @return
	 */
	private OnBoardQuantityResponse buildOnBoardQuantityResponse(OnBoardQuantityReply grpcReply, String CorrelationId) {
		...
		for (OnBoardQuantityDetail detail : grpcReply.getOnBoardQuantityList()) {
			...
			dto.setQuantity(isEmpty(detail.getWeight()) ? BigDecimal.ZERO : new BigDecimal(detail.getWeight()));
			dto.setActualWeight(
					isEmpty(detail.getActualWeight()) ? BigDecimal.ZERO : new BigDecimal(detail.getActualWeight()));
			...
			dto.setApi(isEmpty(detail.getDensity()) ? BigDecimal.ZERO : new BigDecimal(detail.getDensity()));
	...
	public OnBoardQuantityResponse saveOnBoardQuantites(OnBoardQuantity request, String correlationId)
		...
		OnBoardQuantityResponse response = new OnBoardQuantityResponse();
		OnBoardQuantityReply grpcReply = this.saveOnBoardQuantites(this.buildObqRequest(request, correlationId),
				correlationId);
		if (!SUCCESS.equals(grpcReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("Failed to save on board quantities",
					grpcReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(grpcReply.getResponseStatus().getHttpStatusCode())));
		}
		response.setId(grpcReply.getId());
	...
	private OnBoardQuantityDetail buildObqRequest(OnBoardQuantity request, String correlationId) {
		...
		Optional.ofNullable(request.getIsObqComplete()).ifPresent(item -> builder.setIsObqComplete(item));
		return builder.build();
	}
	 ...
	 * @return
	 */
	public OnBoardQuantityReply saveOnBoardQuantites(OnBoardQuantityDetail request, String correlationId) {
	 ...
	 */
	public AlgoStatusResponse saveAlgoLoadableStudyStatus(AlgoStatusRequest request, String correlationId)
			throws GenericServiceException {
		log.info("Inside updateLoadableStudyStatus gateway service with correlationId : " + correlationId);
		AlgoStatusResponse response = new AlgoStatusResponse();
		AlgoStatusReply grpcReply = this
				.saveAlgoLoadableStudyStatus(this.buildAlgoLoadableStudyStatusRequest(request, correlationId));
		if (!SUCCESS.equals(grpcReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("Failed to update Loadable Study Status",
					grpcReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(grpcReply.getResponseStatus().getCode())));
		}
		response.setResponseStatus(new CommonSuccessResponse(String.valueOf(HttpStatus.OK.value()), correlationId));
	 ...
	 */
	private com.cpdss.common.generated.LoadableStudy.AlgoStatusRequest buildAlgoLoadableStudyStatusRequest(
			AlgoStatusRequest request, String correlationId) {
		com.cpdss.common.generated.LoadableStudy.AlgoStatusRequest.Builder builder = com.cpdss.common.generated.LoadableStudy.AlgoStatusRequest
				.newBuilder();
	 ...
	 * @throws GenericServiceException
	 */
	public SynopticalTableResponse getSynopticalTable(Long vesselId, Long loadableStudyId, Long loadablePatternId)
			throws GenericServiceException {
		SynopticalTableResponse synopticalTableResponse = new SynopticalTableResponse();
		// Build response with response status
		CommonSuccessResponse commonSuccessResponse = new CommonSuccessResponse();
	...
	private void buildSynopticalTableResponse(SynopticalTableResponse synopticalTableResponse,
		...
		if (!CollectionUtils.isEmpty(reply.getSynopticalRecordsList())) {
			...
			reply.getSynopticalRecordsList().forEach(synopticalProtoRecord -> {
	 ...
	 */
	private void buildSynopticalBallastRecords(SynopticalRecord synopticalRecord,
		...
		for (SynopticalBallastRecord ballast : synopticalProtoRecord.getBallastList()) {
			...
			record.setCapacity(isEmpty(ballast.getCapacity()) ? null : new BigDecimal(ballast.getCapacity()));
			record.setPlannedWeight(
					isEmpty(ballast.getPlannedWeight()) ? BigDecimal.ZERO : new BigDecimal(ballast.getPlannedWeight()));
			record.setActualWeight(
					isEmpty(ballast.getActualWeight()) ? BigDecimal.ZERO : new BigDecimal(ballast.getActualWeight()));
			record.setCorrectedUllage(isEmpty(ballast.getCorrectedUllage()) ? BigDecimal.ZERO
		...
		if (!list.isEmpty()) {
			synopticalRecord.setBallastPlannedTotal(list.stream().map(SynopticalCargoBallastRecord::getPlannedWeight)
					.reduce(BigDecimal.ZERO, BigDecimal::add));
			synopticalRecord.setBallastActualTotal(list.stream().map(SynopticalCargoBallastRecord::getActualWeight)
					.reduce(BigDecimal.ZERO, BigDecimal::add));
		}
	 ...
	 * @param synopticalProtoRecord
	 */
	private void buildSynopticalLoadicatorRecord(SynopticalRecord synopticalRecord,
			com.cpdss.common.generated.LoadableStudy.SynopticalRecord synopticalProtoRecord) {
		...
		synopticalRecord.setFinalDraftFwd(
				isEmpty(proto.getFinalDraftFwd()) ? BigDecimal.ZERO : new BigDecimal(proto.getFinalDraftFwd()));
		synopticalRecord.setFinalDraftAft(
				isEmpty(proto.getFinalDraftAft()) ? BigDecimal.ZERO : new BigDecimal(proto.getFinalDraftAft()));
		synopticalRecord.setFinalDraftMid(
				isEmpty(proto.getFinalDraftMid()) ? BigDecimal.ZERO : new BigDecimal(proto.getFinalDraftMid()));
		synopticalRecord.setCalculatedDraftFwdActual(isEmpty(proto.getCalculatedDraftFwdActual()) ? BigDecimal.ZERO
				: new BigDecimal(proto.getCalculatedDraftFwdActual()));
		synopticalRecord.setCalculatedDraftFwdPlanned(isEmpty(proto.getCalculatedDraftFwdPlanned()) ? BigDecimal.ZERO
				: new BigDecimal(proto.getCalculatedDraftFwdPlanned()));

		synopticalRecord.setCalculatedDraftAftActual(isEmpty(proto.getCalculatedDraftAftActual()) ? BigDecimal.ZERO
				: new BigDecimal(proto.getCalculatedDraftAftActual()));
		synopticalRecord.setCalculatedDraftAftPlanned(isEmpty(proto.getCalculatedDraftAftPlanned()) ? BigDecimal.ZERO
				: new BigDecimal(proto.getCalculatedDraftAftPlanned()));

		synopticalRecord.setCalculatedDraftMidActual(isEmpty(proto.getCalculatedDraftMidActual()) ? BigDecimal.ZERO
				: new BigDecimal(proto.getCalculatedDraftMidActual()));
		synopticalRecord.setCalculatedDraftMidPlanned(isEmpty(proto.getCalculatedDraftMidPlanned()) ? BigDecimal.ZERO
				: new BigDecimal(proto.getCalculatedDraftMidPlanned()));

		synopticalRecord.setCalculatedTrimActual(isEmpty(proto.getCalculatedTrimActual()) ? BigDecimal.ZERO
				: new BigDecimal(proto.getCalculatedTrimActual()));
		synopticalRecord.setCalculatedTrimPlanned(isEmpty(proto.getCalculatedTrimPlanned()) ? BigDecimal.ZERO
		...
		synopticalRecord.setList(isEmpty(proto.getList()) ? BigDecimal.ZERO : new BigDecimal(proto.getList()));
		synopticalRecord
				.setBm(isEmpty(proto.getBendingMoment()) ? BigDecimal.ZERO : new BigDecimal(proto.getBendingMoment()));
		synopticalRecord
				.setSf(isEmpty(proto.getShearingForce()) ? BigDecimal.ZERO : new BigDecimal(proto.getShearingForce()));
	}
	 ...
	 * @param list
	 */
	private void buildSynopticalRecord(SynopticalRecord synopticalRecord,
			com.cpdss.common.generated.LoadableStudy.SynopticalRecord synopticalProtoRecord) {
		...
		synopticalRecord.setSpecificGravity(!isEmpty(synopticalProtoRecord.getSpecificGravity())
		...
		synopticalRecord.setDistance(
				!isEmpty(synopticalProtoRecord.getDistance()) ? new BigDecimal(synopticalProtoRecord.getDistance())
						: BigDecimal.ZERO);
		synopticalRecord
				.setSpeed(!isEmpty(synopticalProtoRecord.getSpeed()) ? new BigDecimal(synopticalProtoRecord.getSpeed())
						: BigDecimal.ZERO);
		synopticalRecord.setInPortHours(!isEmpty(synopticalProtoRecord.getInPortHours())
				? new BigDecimal(synopticalProtoRecord.getInPortHours())
				: null);
		synopticalRecord.setRunningHours(!isEmpty(synopticalProtoRecord.getRunningHours())
		...
		synopticalRecord.setHwTideFrom(
				!isEmpty(synopticalProtoRecord.getHwTideFrom()) ? new BigDecimal(synopticalProtoRecord.getHwTideFrom())
						: BigDecimal.ZERO);
		synopticalRecord.setHwTideTimeFrom(synopticalProtoRecord.getHwTideTimeFrom());
		synopticalRecord.setHwTideTo(
				!isEmpty(synopticalProtoRecord.getHwTideTo()) ? new BigDecimal(synopticalProtoRecord.getHwTideTo())
						: BigDecimal.ZERO);
		synopticalRecord.setHwTideTimeTo(synopticalProtoRecord.getHwTideTimeTo());
		synopticalRecord.setLwTideFrom(
				!isEmpty(synopticalProtoRecord.getLwTideFrom()) ? new BigDecimal(synopticalProtoRecord.getLwTideFrom())
						: BigDecimal.ZERO);
		synopticalRecord.setLwTideTimeFrom(synopticalProtoRecord.getLwTideTimeFrom());
		synopticalRecord.setLwTideTo(
				!isEmpty(synopticalProtoRecord.getLwTideTo()) ? new BigDecimal(synopticalProtoRecord.getLwTideTo())
						: BigDecimal.ZERO);
		synopticalRecord.setLwTideTimeTo(synopticalProtoRecord.getLwTideTimeTo());
		synopticalRecord.setEtaEtdActual(
				isEmpty(synopticalProtoRecord.getEtaEtdActual()) ? null : synopticalProtoRecord.getEtaEtdActual());
		synopticalRecord.setEtaEtdPlanned(isEmpty(synopticalProtoRecord.getEtaEtdEstimated()) ? null
				: synopticalProtoRecord.getEtaEtdEstimated());
		synopticalRecord.setPortTimezoneId(
				isEmpty(synopticalProtoRecord.getPortTimezoneId()) ? null : synopticalProtoRecord.getPortTimezoneId());
	}
	...
	private void setSynopticalInPortHours(List<SynopticalRecord> synopticalTableList) {
		...
		temp.removeAll(temp.stream().filter(item -> item.getOperationType().equals(SYNOPTICAL_TABLE_OP_TYPE_DEPARTURE))
				.collect(Collectors.toList()));
		for (SynopticalRecord rec : temp) {
			Optional<SynopticalRecord> arrOpt = synopticalTableList.stream()
					.filter(item -> item.getPortRotationId().equals(rec.getPortRotationId())
							&& SYNOPTICAL_TABLE_OP_TYPE_ARRIVAL.equals(item.getOperationType()))
					.findAny();
			Optional<SynopticalRecord> depOpt = synopticalTableList.stream()
					.filter(item -> item.getPortRotationId().equals(rec.getPortRotationId())
			...
			if (arrOpt.isPresent() && depOpt.isPresent()) {
				...
				if (arr.getInPortHours() == null && dep.getInPortHours() == null) {
					if (null != arr.getEtaEtdActual() && null != dep.getEtaEtdActual()) {
						LocalDateTime arrDateTime = LocalDateTime
								.from(DateTimeFormatter.ofPattern(DATE_FORMAT).parse(arr.getEtaEtdActual()));
						LocalDateTime depDateTime = LocalDateTime
								.from(DateTimeFormatter.ofPattern(DATE_FORMAT).parse(dep.getEtaEtdActual()));
						BigDecimal inPortHours = new BigDecimal(arrDateTime.until(depDateTime, ChronoUnit.HOURS));
						arr.setInPortHours(inPortHours);
						dep.setInPortHours(inPortHours);
					} else if (null != arr.getEtaEtdPlanned() && null != dep.getEtaEtdPlanned()) {
						LocalDateTime arrDateTime = LocalDateTime
								.from(DateTimeFormatter.ofPattern(DATE_FORMAT).parse(arr.getEtaEtdPlanned()));
						LocalDateTime depDateTime = LocalDateTime
								.from(DateTimeFormatter.ofPattern(DATE_FORMAT).parse(dep.getEtaEtdPlanned()));
						BigDecimal inPortHours = new BigDecimal(arrDateTime.until(depDateTime, ChronoUnit.HOURS));
						arr.setInPortHours(inPortHours);
						dep.setInPortHours(inPortHours);
					}
	 ...
	 * @param synopticalProtoRecord
	 */
	private void buildVesselDataForSynopticalTable(SynopticalRecord synopticalRecord,
			com.cpdss.common.generated.LoadableStudy.SynopticalRecord synopticalProtoRecord) {
		synopticalRecord.setOthersActual(isEmpty(synopticalProtoRecord.getOthersActual()) ? BigDecimal.ZERO
				: new BigDecimal(synopticalProtoRecord.getOthersActual()));
		synopticalRecord.setOthersPlanned(isEmpty(synopticalProtoRecord.getOthersPlanned()) ? BigDecimal.ZERO
				: new BigDecimal(synopticalProtoRecord.getOthersPlanned()));
		synopticalRecord.setConstantActual(isEmpty(synopticalProtoRecord.getConstantActual()) ? BigDecimal.ZERO
				: new BigDecimal(synopticalProtoRecord.getConstantActual()));
		synopticalRecord.setConstantPlanned(isEmpty(synopticalProtoRecord.getConstantPlanned()) ? BigDecimal.ZERO
				: new BigDecimal(synopticalProtoRecord.getConstantPlanned()));
		synopticalRecord.setTotalDwtActual(isEmpty(synopticalProtoRecord.getTotalDwtActual()) ? BigDecimal.ZERO
				: new BigDecimal(synopticalProtoRecord.getTotalDwtActual()));
		synopticalRecord.setTotalDwtPlanned(isEmpty(synopticalProtoRecord.getTotalDwtPlanned()) ? BigDecimal.ZERO
				: new BigDecimal(synopticalProtoRecord.getTotalDwtPlanned()));
		synopticalRecord.setDisplacementActual(isEmpty(synopticalProtoRecord.getDisplacementActual()) ? BigDecimal.ZERO
				: new BigDecimal(synopticalProtoRecord.getDisplacementActual()));
		synopticalRecord
				.setDisplacementPlanned(isEmpty(synopticalProtoRecord.getDisplacementPlanned()) ? BigDecimal.ZERO
	 ...
	 * @param synopticalProtoRecord
	 */
	private void buildOhqDataForSynopticalTable(SynopticalRecord synopticalRecord,
			com.cpdss.common.generated.LoadableStudy.SynopticalRecord synopticalProtoRecord) {
		...
		for (com.cpdss.common.generated.LoadableStudy.SynopticalOhqRecord protoRec : synopticalProtoRecord
				.getOhqList()) {
			...
			rec.setCapacity(isEmpty(protoRec.getCapacity()) ? BigDecimal.ZERO : new BigDecimal(protoRec.getCapacity()));
			rec.setActualWeight(
					isEmpty(protoRec.getActualWeight()) ? BigDecimal.ZERO : new BigDecimal(protoRec.getActualWeight()));
			rec.setPlannedWeight(isEmpty(protoRec.getPlannedWeight()) ? BigDecimal.ZERO
					: new BigDecimal(protoRec.getPlannedWeight()));
			rec.setDensity(isEmpty(protoRec.getDensity()) ? BigDecimal.ZERO : new BigDecimal(protoRec.getDensity()));
		...
		synopticalRecord.setActualDOTotal(
				doList.stream().map(doTank -> doTank.getActualWeight()).reduce(BigDecimal.ZERO, BigDecimal::add));
		...
		synopticalRecord.setActualLubeTotal(
				lubeList.stream().map(lube -> lube.getActualWeight()).reduce(BigDecimal.ZERO, BigDecimal::add));
		...
		synopticalRecord.setPlannedDOTotal(
				doList.stream().map(doTank -> doTank.getPlannedWeight()).reduce(BigDecimal.ZERO, BigDecimal::add));
		...
		synopticalRecord.setPlannedLubeTotal(
				lubeList.stream().map(lube -> lube.getPlannedWeight()).reduce(BigDecimal.ZERO, BigDecimal::add));
	}
	 ...
	 * @return
	 */
	private void buildSynopticalTableCargos(SynopticalRecord synopticalRecord,
			com.cpdss.common.generated.LoadableStudy.SynopticalRecord synopticalProtoRecord) {
		List<SynopticalCargoBallastRecord> list = new ArrayList<>();
		for (com.cpdss.common.generated.LoadableStudy.SynopticalCargoRecord protoRec : synopticalProtoRecord
				.getCargoList()) {
			...
			rec.setCorrectedUllage(isEmpty(protoRec.getCorrectedUllage()) ? BigDecimal.ZERO
			...
			rec.setCapacity(isEmpty(protoRec.getCapacity()) ? null : new BigDecimal(protoRec.getCapacity()));
			rec.setIsCommingleCargo(isEmpty(protoRec.getIsCommingleCargo()) ? null : protoRec.getIsCommingleCargo());
		...
		synopticalRecord.setCargoPlannedTotal(isEmpty(synopticalProtoRecord.getCargoPlannedTotal()) ? BigDecimal.ZERO
				: new BigDecimal(synopticalProtoRecord.getCargoPlannedTotal()));
		synopticalRecord.setCargoActualTotal(isEmpty(synopticalProtoRecord.getCargoActualTotal()) ? BigDecimal.ZERO
	 ...
	 */
	public CommonResponse confirmPlan(Long loadablePatternId, String correlationId) throws GenericServiceException {
		...
		if (!SUCCESS.equals(grpcReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("Failed to confirm plan", grpcReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(grpcReply.getResponseStatus().getCode())));
		}
		response.setResponseStatus(new CommonSuccessResponse(String.valueOf(HttpStatus.OK.value()), correlationId));
		return response;
	 ...
	 */
	public ConfirmPlanReply confirmPlan(com.cpdss.common.generated.LoadableStudy.ConfirmPlanRequest.Builder request) {
	 ...
	 * @return AlgoPatternResponse
	 */
	public AlgoPatternResponse saveLoadicatorResult(LoadicatorResultsRequest loadicatorResultsRequest,
			Long loadableStudiesId, String correlationId) throws GenericServiceException {
		...
		com.cpdss.common.generated.LoadableStudy.LoadicatorResultsRequest.Builder request = com.cpdss.common.generated.LoadableStudy.LoadicatorResultsRequest
				.newBuilder();
		...
		if (!SUCCESS.equals(grpcReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("Failed to saveLoadicatorResult plan",
					grpcReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(grpcReply.getResponseStatus().getCode())));
		}
		response.setResponseStatus(new CommonSuccessResponse(String.valueOf(HttpStatus.OK.value()), correlationId));
	 ...
	 * @param request                  void
	 */
	private void createLoadicatorResultsRequest(LoadicatorResultsRequest loadicatorResultsRequest,
			Long loadableStudiesId, com.cpdss.common.generated.LoadableStudy.LoadicatorResultsRequest.Builder request) {
		request.setLoadableStudyId(loadableStudiesId);
		request.setProcessId(loadicatorResultsRequest.getProcessId());
		loadicatorResultsRequest.getLoadicatorResultsPatternWise().forEach(lrpw -> {
			...
			lrpw.getLoadicatorResultDetails().forEach(lrd -> {
				LodicatorResultDetails.Builder loadicatorResultsBuilder = LodicatorResultDetails.newBuilder();
				loadicatorResultsBuilder.setBlindSector(lrd.getBlindSector());
				loadicatorResultsBuilder.setCalculatedDraftAftPlanned(lrd.getCalculatedDraftAftPlanned());
				loadicatorResultsBuilder.setCalculatedDraftFwdPlanned(lrd.getCalculatedDraftFwdPlanned());
				loadicatorResultsBuilder.setCalculatedDraftMidPlanned(lrd.getCalculatedDraftMidPlanned());
				loadicatorResultsBuilder.setCalculatedTrimPlanned(lrd.getCalculatedTrimPlanned());
	 ...
	 * @return AlgoPatternResponse
	 */
	public AlgoPatternResponse saveAlgoPatterns(LoadablePlanRequest loadablePlanRequest, Long loadableStudiesId,
			String requestType, String correlationId) throws GenericServiceException {
		...
		try {
			objectMapper.writeValue(new File(this.rootFolder + fileName + loadableStudiesId + ".json"),
					loadablePlanRequest);
			StatusReply reply = this.saveJson(loadableStudiesId, jsonTypeID,
	 ...
	 * @return AlgoPatternResponse
	 */
	public AlgoPatternResponse saveLoadablePatterns(LoadablePlanRequest loadablePlanRequest, Long loadableStudiesId,
			String correlationId) throws GenericServiceException {
		log.info("Inside saveLoadablePatterns gateway service with correlationId : " + correlationId);
//		ObjectMapper objectMapper = new ObjectMapper();
//		try {
...
//		}
		...
		if (!SUCCESS.equals(algoReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("Failed to save loadable pattern",
	 ...
	 * @param request             void
	 */
	private void buildLoadablePlanDetails(LoadablePlanRequest loadablePlanRequest,
		...
		loadablePlanRequest.getLoadablePlanDetails().forEach(lpd -> {
			planBuilder.clearLoadablePlanPortWiseDetails();
			planBuilder.clearConstraints();
			LoadablePlanPortWiseDetails.Builder portWiseBuilder = LoadablePlanPortWiseDetails.newBuilder();
			lpd.getLoadablePlanPortWiseDetails().forEach(lppwd -> {
				LoadablePlanDetailsReply.Builder detailsBuilderDeparture = LoadablePlanDetailsReply.newBuilder();
				lppwd.getDepartureCondition().getLoadableQuantityCommingleCargoDetails().forEach(lqccd -> {
					buildCommingleDetails(lqccd, detailsBuilderDeparture);
				});
				lppwd.getDepartureCondition().getLoadableQuantityCargoDetails().forEach(lpqcd -> {
					buildLoadableCargoDetails(lpqcd, detailsBuilderDeparture);
				});
				lppwd.getDepartureCondition().getLoadablePlanStowageDetails().forEach(lpsd -> {
					buildLoadablePlanStowageDetails(lpsd, detailsBuilderDeparture);
				});
				lppwd.getDepartureCondition().getLoadablePlanBallastDetails().forEach(lpbd -> {
				...
				Optional.ofNullable(lppwd.getDepartureCondition().getStabilityParameters())
						.ifPresent(stabilityParameter -> detailsBuilderDeparture
								.setStabilityParameter(buildStabilityParamter(stabilityParameter)));

				// DS field
				Optional.ofNullable(lppwd.getDepartureCondition().getDischargePlanRoBDetails())
						.ifPresent(robDetails -> robDetails.forEach(rd -> {
							buildRoBDetails(rd, detailsBuilderDeparture);
						}));

				portWiseBuilder.setDepartureCondition(detailsBuilderDeparture);

				LoadablePlanDetailsReply.Builder detailsBuilderArrival = LoadablePlanDetailsReply.newBuilder();
				lppwd.getArrivalCondition().getLoadableQuantityCommingleCargoDetails().forEach(lqccd -> {
					buildCommingleDetails(lqccd, detailsBuilderArrival);
				});
				lppwd.getArrivalCondition().getLoadableQuantityCargoDetails().forEach(lpqcd -> {
					buildLoadableCargoDetails(lpqcd, detailsBuilderArrival);
				});
				lppwd.getArrivalCondition().getLoadablePlanStowageDetails().forEach(lpsd -> {
					buildLoadablePlanStowageDetails(lpsd, detailsBuilderArrival);
				});
				lppwd.getArrivalCondition().getLoadablePlanBallastDetails().forEach(lpbd -> {
				...
				Optional.ofNullable(lppwd.getArrivalCondition().getStabilityParameters())
						.ifPresent(stabilityParameter -> detailsBuilderArrival
								.setStabilityParameter(buildStabilityParamter(stabilityParameter)));
				...
				portWiseBuilder.setPortRotationId(null != lppwd.getPortRotationId() ? lppwd.getPortRotationId() : 0);
	 ...
	 */
	private void buildLoadablePlanConstrains(com.cpdss.gateway.domain.LoadablePlanDetails lpd,
			com.cpdss.common.generated.LoadableStudy.LoadablePlanDetails.Builder planBuilder) {
		if (!lpd.getConstraints().isEmpty()) {
			lpd.getConstraints().forEach(constrain -> {
	 ...
	 * @return StabilityParameter
	 */
	private StabilityParameter buildRoBDetails(DischargePlanRoBDetails robDetails,
	 ...
	 */
	private void buildLoadablePlanBallastDetails(LoadablePlanBallastDetails lpbd,
			com.cpdss.common.generated.LoadableStudy.LoadablePlanDetailsReply.Builder detailsBuilder) {
		com.cpdss.common.generated.LoadableStudy.LoadablePlanBallastDetails.Builder builder = com.cpdss.common.generated.LoadableStudy.LoadablePlanBallastDetails
				.newBuilder();
	 ...
	 * @param detailsBuilder void
	 */
	private void buildLoadablePlanStowageDetails(LoadablePlanStowageDetails lpsd,
			com.cpdss.common.generated.LoadableStudy.LoadablePlanDetailsReply.Builder detailsBuilder) {
		com.cpdss.common.generated.LoadableStudy.LoadablePlanStowageDetails.Builder builder = com.cpdss.common.generated.LoadableStudy.LoadablePlanStowageDetails
				.newBuilder();
		...
		Optional.ofNullable(lpsd.getCargoNominationTemperature()).ifPresent(builder::setCargoNominationTemperature);

		Optional.ofNullable(lpsd.getOnboard()).ifPresent(builder::setOnboard);
		Optional.ofNullable(lpsd.getMaxTankVolume()).ifPresent(builder::seMaxTankVolume);
	 ...
	 * @param detailsBuilder void
	 */
	private void buildLoadableCargoDetails(com.cpdss.gateway.domain.LoadableQuantityCargoDetails lpqcd,
			com.cpdss.common.generated.LoadableStudy.LoadablePlanDetailsReply.Builder detailsBuilder) {

		LoadableQuantityCargoDetails.Builder qunatityBuilder = LoadableQuantityCargoDetails.newBuilder();
		...
		Optional.ofNullable(lpqcd.getDifferencePercentage()).ifPresent(qunatityBuilder::setDifferencePercentage);
		Optional.ofNullable(lpqcd.getCargoAbbreviation()).ifPresent(qunatityBuilder::setCargoAbbreviation);
		...
		Optional.ofNullable(lpqcd.getCargoNominationId()).ifPresent(qunatityBuilder::setCargoNominationId);
		Optional.ofNullable(lpqcd.getTimeRequiredForLoading()).ifPresent(qunatityBuilder::setTimeRequiredForLoading);
		lpqcd.getToppingSequence().forEach(sequence -> {
			CargoToppingOffSequenceDetails.Builder toppingBuilder = CargoToppingOffSequenceDetails.newBuilder();
			Optional.ofNullable(lpqcd.getCargoId()).ifPresent(toppingBuilder::setCargoId);
			Optional.ofNullable(sequence.getSequenceOrder()).ifPresent(toppingBuilder::setOrderNumber);
		...
		Optional.ofNullable(lpqcd.getCowDetails()).ifPresent(i -> i.forEach(cowDetail -> {
			Tank.Builder tankBuilder = Tank.newBuilder(); // TODO
			Optional.ofNullable(cowDetail.getShortName()).ifPresent(tankBuilder::setShortName);
	 ...
	 * @param detailsBuilder void
	 */
	private void buildCommingleDetails(LoadableQuantityCommingleCargoDetails lqccd,
			com.cpdss.common.generated.LoadableStudy.LoadablePlanDetailsReply.Builder detailsBuilder) {
		com.cpdss.common.generated.LoadableStudy.LoadableQuantityCommingleCargoDetails.Builder builder = com.cpdss.common.generated.LoadableStudy.LoadableQuantityCommingleCargoDetails
				.newBuilder();
		...
		Optional.ofNullable(lqccd.getTimeRequiredForLoading()).ifPresent(builder::setTimeRequiredForLoading);
		...
		lqccd.getToppingSequence().forEach(sequence -> {
			CargoToppingOffSequenceDetails.Builder toppingBuilder = CargoToppingOffSequenceDetails.newBuilder();
			Optional.ofNullable(lqccd.getToppingOffCargoId()).ifPresent(toppingBuilder::setCargoId);
			Optional.ofNullable(sequence.getSequenceOrder()).ifPresent(toppingBuilder::setOrderNumber);
			Optional.ofNullable(sequence.getTankId()).ifPresent(toppingBuilder::setTankId);
	...
	public StatusReply saveJson(Long referenceId, Long jsonTypeId, String json) {
		JsonRequest jsonRequest = JsonRequest.newBuilder().setReferenceId(referenceId).setJsonTypeId(jsonTypeId)
				.setJson(json).build();
	 ...
	 */
	public AlgoErrorResponse getAlgoError(Long loadablePatternId, String correlationId) throws GenericServiceException {
		...
		if (!SUCCESS.equals(grpcReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("Failed to getAlgoError", grpcReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(grpcReply.getResponseStatus().getCode())));
		}
	...
	private AlgoErrorResponse buildErrorResponse(AlgoErrorReply grpcReply, String correlationId) {
		...
		grpcReply.getAlgoErrorsList().forEach(algoError -> {
	 ...
	 */
	public AlgoErrorReply getAlgoError(com.cpdss.common.generated.LoadableStudy.AlgoErrorRequest.Builder request) {
	 ...
	 */
	public LoadablePlanDetailsResponse getLoadablePatternDetails(Long loadablePatternId, Long loadableStudyId,
			Long vesselId, String correlationId) throws GenericServiceException {
		log.info("Inside getLoadablePatternDetails gateway service with correlationId : " + correlationId);
		...
		if (!SUCCESS.equals(grpcReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("Failed to getLoadablePatternDetails",
					grpcReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(grpcReply.getResponseStatus().getCode())));
		}
		...
		response.setLoadableQuantity(isEmpty(grpcReply.getTotalLoadableQuantity()) ? null
	 ...
	 */
	private void buildLoadableStudyDetails(LoadablePlanDetailsResponse response, LoadablePlanDetailsReply grpcReply) {
		response.setId(grpcReply.getId());
	 ...
	 * @param grpcReply void
	 */
	private void buildLoadablePlanComments(LoadablePlanDetailsResponse response, LoadablePlanDetailsReply grpcReply) {
		response.setLoadablePlanComments(new ArrayList<LoadablePlanComments>());
		grpcReply.getLoadablePlanCommentsList().forEach(lpc -> {
			...
			try {
				...
				userEntity.ifPresent(user -> {
					try {
						KeycloakUser keycloakUser = userCachingService.getUser(userEntity.get().getKeycloakId());
						commets.setUserName(
								String.format("%s %s", keycloakUser.getFirstName(), keycloakUser.getLastName()));
					} catch (GenericServiceException e) {
	 ...
	 * @throws GenericServiceException
	 */
	private void buildSynopticalTableDetails(LoadablePlanDetailsResponse response, Long loadableStudyId, Long vesselId,
			Long loadablePatternId) throws GenericServiceException {
		...
		synopticalTableResponse.getSynopticalRecords().forEach(str -> {
			LoadablePlanSynopticalRecord synopticalRecord = new LoadablePlanSynopticalRecord();
			synopticalRecord.setId(str.getId());
			synopticalRecord.setDisplacementPlanned(str.getDisplacementPlanned());
			synopticalRecord.setEtaEtdPlanned(str.getEtaEtdPlanned());
	 ...
	 * @param grpcReply void
	 */
	private void buildLoadableStudyBallastDetails(LoadablePlanDetailsResponse response,
			LoadablePlanDetailsReply grpcReply) {
		response.setLoadablePlanBallastDetails(new ArrayList<LoadablePlanBallastDetails>());
		grpcReply.getLoadablePlanBallastDetailsList().forEach(lpbd -> {
			LoadablePlanBallastDetails details = new LoadablePlanBallastDetails();
			details.setId(lpbd.getId());
	 ...
	 * @param grpcReply void
	 */
	private void buildloadableStudyCommingleCargoDetails(LoadablePlanDetailsResponse response,
			LoadablePlanDetailsReply grpcReply) {
		response.setLoadableQuantityCommingleCargoDetails(new ArrayList<LoadableQuantityCommingleCargoDetails>());
		grpcReply.getLoadableQuantityCommingleCargoDetailsList().forEach(lqccd -> {
			LoadableQuantityCommingleCargoDetails details = getLoadableQuantityCommingleCargoDetails(lqccd);
			response.getLoadableQuantityCommingleCargoDetails().add(details);
		});
	}
	 ...
	 * @param grpcReply void
	 */
	private void buildLoadableStudyStowageDetails(LoadablePlanDetailsResponse response,
			LoadablePlanDetailsReply grpcReply) {
		response.setLoadablePlanStowageDetails(new ArrayList<LoadablePlanStowageDetails>());
		grpcReply.getLoadablePlanStowageDetailsList().forEach(lpsd -> {
			...
			loadablePlanStowageDetails.setCorrectedUllageOrginal(lpsd.getCorrectedUllageOrginal());
			loadablePlanStowageDetails.setCorrectionFactorOrginal(lpsd.getCorrectionFactorOrginal());
	 ...
	 * @param grpcReply void
	 */
	private void buildLoadableStudyQuantity(LoadablePlanDetailsResponse response, LoadablePlanDetailsReply grpcReply) {
		response.setLoadableQuantityCargoDetails(
				new ArrayList<com.cpdss.gateway.domain.LoadableQuantityCargoDetails>());
		grpcReply.getLoadableQuantityCargoDetailsList().forEach(lqcd -> {
			com.cpdss.gateway.domain.LoadableQuantityCargoDetails cargoDetails = new com.cpdss.gateway.domain.LoadableQuantityCargoDetails();
	 ...
	 */
	public LoadableStudyStatusResponse getLoadableStudyStatus(Long loadableStudyId,
			LoadablePlanRequest loadablePlanRequest, String correlationId) throws GenericServiceException {
		...
		LoadableStudyStatusReply grpcReply = this.getLoadableStudyStatus(
				this.buildLoadableStudyStatusRequest(loadableStudyId, loadablePlanRequest, correlationId));
		if (!SUCCESS.equals(grpcReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("Failed to get Loadable Study Status",
					grpcReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(grpcReply.getResponseStatus().getCode())));
		}
		response.setLoadableStudyStatusId(grpcReply.getLoadableStudystatusId());
		response.setLoadableStudyStatusLastModifiedTime(grpcReply.getLoadableStudyStatusLastModifiedTime());
		response.setResponseStatus(new CommonSuccessResponse(String.valueOf(HttpStatus.OK.value()), correlationId));
		return response;
	}
	 ...
	 */
	LoadableStudyStatusReply getLoadableStudyStatus(LoadableStudyStatusRequest loadableStudyStatusRequest) {
	...
	public AlgoPatternResponse validateLoadablePlan(Long loadablePatternId, String correlationId)
		...
		if (!SUCCESS.equals(grpcReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("Failed to validateLoadablePlan", grpcReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(grpcReply.getResponseStatus().getCode())));
		}
	 ...
	 * @return Long
	 */
	private LoadableStudyStatusRequest buildLoadableStudyStatusRequest(Long loadableStudyId,
			LoadablePlanRequest loadablePlanRequest, String correlationId) {
		...
		Optional.ofNullable(loadablePlanRequest.getLoadablePatternId()).ifPresent(builder::setLoadablePatternId);
		return builder.build();
	}
	 ...
	 */
	public SynopticalTableResponse saveSynopticalTable(com.cpdss.gateway.domain.SynopticalTableRequest request,
			Long voyageId, Long loadableStudyId, Long loadablePatternId, String correlationId)
		...
		log.debug("LoadableStudyService: saveSynopticalTable, request: {}, voyageId: {}, loadbleStudyId:{}", request,
				voyageId, loadableStudyId);
		SynopticalTableResponse response = new SynopticalTableResponse();
		Map<Long, String> failedRecords = new HashMap<Long, String>();
		List<Thread> workers = new ArrayList<>();
		Set<Long> porRotationtIds = request.getSynopticalRecords().stream().map(SynopticalRecord::getPortRotationId)
				.collect(Collectors.toSet());
		CountDownLatch latch = new CountDownLatch(porRotationtIds.size());
		for (Long portRotationId : porRotationtIds) {
			SynopticalTableRequest grpcRequest = this.buildSynopticalTableRequest(portRotationId, request,
					loadableStudyId, loadablePatternId, correlationId);
			workers.add(new Thread(() -> this.saveSynopticalTable(grpcRequest, correlationId, failedRecords, latch)));
		}
		...
		if (!failedRecords.isEmpty()) {
			...
			response.setResponseStatus(
					new CommonSuccessResponse(String.valueOf(HttpStatus.MULTI_STATUS.value()), correlationId));
		} else {
			response.setResponseStatus(new CommonSuccessResponse(String.valueOf(HttpStatus.OK.value()), correlationId));
		}
		return response;
	 ...
	 * @return
	 */
	private void saveSynopticalTable(SynopticalTableRequest grpcRequest, String correlationId,
			Map<Long, String> failedRecords, CountDownLatch latch) {
		try {
			...
			if (!SUCCESS.equals(grpcReply.getResponseStatus().getStatus())) {
				grpcRequest.getSynopticalRecordList().forEach(rec -> failedRecords.put(rec.getId(),
						ERROR_CODE_PREFIX + String.valueOf(grpcReply.getResponseStatus().getCode())));
			}
		} catch (Exception e) {
	 ...
	 * @throws GenericServiceException
	 */
	private SynopticalTableRequest buildSynopticalTableRequest(Long portRotationId,
			com.cpdss.gateway.domain.SynopticalTableRequest request, Long loadableStudyId, Long loadablePatternId,
			String correlationId) throws GenericServiceException {
		...
		List<SynopticalRecord> records = request.getSynopticalRecords().stream()
				.filter(rec -> rec.getPortRotationId().equals(portRotationId)).collect(Collectors.toList());
		if (records.size() != 2) {
			throw new GenericServiceException("Invalid size of port records", CommonErrorCodes.E_HTTP_BAD_REQUEST,
					HttpStatusCode.BAD_REQUEST);
		}
		for (SynopticalRecord rec : records) {
			com.cpdss.common.generated.LoadableStudy.SynopticalRecord.Builder recordBuilder = com.cpdss.common.generated.LoadableStudy.SynopticalRecord
					.newBuilder();
	...
	private void buildSynopticalRequestBallastData(
			com.cpdss.common.generated.LoadableStudy.SynopticalRecord.Builder recordBuilder, SynopticalRecord rec) {
	...
	private void buildSynopticalRequestOhqData(
		...
		for (SynopticalOhqRecord record : ohqList) {
			com.cpdss.common.generated.LoadableStudy.SynopticalOhqRecord.Builder ohqBuilder = com.cpdss.common.generated.LoadableStudy.SynopticalOhqRecord
					.newBuilder();
			ohqBuilder.setTankId(record.getTankId());
			ohqBuilder.setFuelTypeId(record.getFuelTypeId());
			Optional.ofNullable(record.getActualWeight()).ifPresent(item -> ohqBuilder.setActualWeight(valueOf(item)));
	...
	private void buildSynopticalRequestCargoData(
			com.cpdss.common.generated.LoadableStudy.SynopticalRecord.Builder recordBuilder, SynopticalRecord request) {
		if (null != request.getCargos()) {
			for (SynopticalCargoBallastRecord cargo : request.getCargos()) {
				com.cpdss.common.generated.LoadableStudy.SynopticalCargoRecord.Builder builder = com.cpdss.common.generated.LoadableStudy.SynopticalCargoRecord
						.newBuilder();
				...
				Optional.ofNullable(cargo.getActualWeight()).ifPresent(item -> builder.setActualWeight(valueOf(item)));
	...
	private void buildSynopticalRequestLoadicatorData(
			com.cpdss.common.generated.LoadableStudy.SynopticalRecord.Builder recordBuilder, SynopticalRecord request) {
		SynopticalTableLoadicatorData.Builder loadicatorBuilder = SynopticalTableLoadicatorData.newBuilder();
		Optional.ofNullable(request.getDeflection()).ifPresent(item -> loadicatorBuilder.setDeflection(valueOf(item)));
	...
	private void buildSynopticalRequestRecord(
		...
		Optional.ofNullable(request.getSpeed()).ifPresent(item -> recordBuilder.setSpeed(valueOf(request.getSpeed())));
		...
		Optional.ofNullable(request.getOthersActual()).ifPresent(item -> recordBuilder.setOthersActual(valueOf(item)));
	...
	public LoadableStudyAttachmentReply downloadLoadableStudyAttachment(LoadableStudyAttachmentRequest grpcRequest) {
		return this.loadableStudyServiceBlockingStub.downloadLoadableStudyAttachment(grpcRequest);
	}

	private LoadableStudyAttachmentRequest builddownloadLoadableStudyAttachmentRequest(Long attachmentId,
	...
	public SaveCommentResponse saveComment(Comment request, String correlationId, long loadablePatternId,
			String authorizationToken) throws NumberFormatException, GenericServiceException {
		...
		if (!grpcReply.getResponseStatus().getStatus().equals(SUCCESS)) {
			...
			throw new GenericServiceException("Faield to save comment for Loadable pattern - " + loadablePatternId,
					grpcReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(grpcReply.getResponseStatus().getHttpStatusCode())));
		}
	...
	public LoadablePatternResponse getLoadablePatternList(Long loadableStudyId, String correlationId)
		...
		LoadablePatternRequest grpcRequest = LoadablePatternRequest.newBuilder().setLoadableStudyId(loadableStudyId)
				.build();
		LoadablePatternReply grpcReply = this.getLoadablePatternList(grpcRequest);
		if (!SUCCESS.equals(grpcReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("Failed to get pattern list from grpc service",
					grpcReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(grpcReply.getResponseStatus().getCode())));
		}
		response.setLoadablePatterns(new ArrayList<>());
		for (com.cpdss.common.generated.LoadableStudy.LoadablePattern pattern : grpcReply.getLoadablePatternList()) {
	 ...
	 */
	public ConfirmPlanStatusResponse confirmPlanStatus(Long loadablePatternId, Long voyageId, String correlationId)
			throws GenericServiceException {
		...
		if (!SUCCESS.equals(grpcReply.getResponseStatus().getStatus())) {
			...
			throw new GenericServiceException("Failed in confirmPlanStatus from grpc service",
					grpcReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(grpcReply.getResponseStatus().getHttpStatusCode())));
		}
		confirmPlanStatusResponse
				.setResponseStatus(new CommonSuccessResponse(String.valueOf(HttpStatus.OK.value()), correlationId));
	 ...
	 */
	public UpdateUllage updateUllage(UpdateUllage updateUllageRequest, Long loadablePatternId, String correlationId)
			throws GenericServiceException {
	...
	public UpdateUllage buildeUpdateUllageResponse(UpdateUllageReply grpcReply, String correlationId) {
		UpdateUllage response = new UpdateUllage();
		response.setCorrectedUllage(isEmpty(grpcReply.getLoadablePlanStowageDetails().getCorrectedUllage()) ? null
				: new BigDecimal(grpcReply.getLoadablePlanStowageDetails().getCorrectedUllage()));
		response.setCorrectionFactor(isEmpty(grpcReply.getLoadablePlanStowageDetails().getCorrectionFactor()) ? null
				: new BigDecimal(grpcReply.getLoadablePlanStowageDetails().getCorrectionFactor()));
		response.setQuantityMt(isEmpty(grpcReply.getLoadablePlanStowageDetails().getWeight()) ? null
	 ...
	 */
	public UpdateUllageReply updateUllage(com.cpdss.common.generated.LoadableStudy.UpdateUllageRequest grpcRequest) {
	 ...
	 */
	public void buildUpdateUllageRequest(UpdateUllage updateUllageRequest, Long loadablePatternId,
	 ...
	 * @throws GenericServiceException
	 */
	public VoyageStatusResponse getVoyageStatus(VoyageStatusRequest voyageStatusRequest, Long vesselId, Long voyageId,
			Long loadableStudyId, Long portId, String correlationId) throws GenericServiceException {
		...
		OnHandQuantityRequest ohqRequest = OnHandQuantityRequest.newBuilder().setVesselId(vesselId)
				.setLoadableStudyId(loadableStudyId).setPortId(portId)
				.setPortRotationId(voyageStatusRequest.getPortRotationId()).build();
		...
		if (!SUCCESS.equals(onHandQtyReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("Error calling getVoyageStatus - getOnHandQuantity grpc service",
					onHandQtyReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(onHandQtyReply.getResponseStatus().getCode())));
		}
		OnHandQuantityResponse bunkerResponse = buildOnHandQuantityResponse(onHandQtyReply, correlationId);
		// Retrieve synoptical data
		SynopticalTableRequest synopticalTableRequest = SynopticalTableRequest.newBuilder().setVoyageId(voyageId)
				.setLoadableStudyId(loadableStudyId).setVesselId(vesselId).setPortId(portId)
				.setOperationType(voyageStatusRequest.getOperationType()).build();
		SynopticalTableReply synopticalTableReply = loadableStudyServiceBlockingStub
				.getSynopticalDataByPortId(synopticalTableRequest);
		SynopticalTableResponse synopticalTableResponse = new SynopticalTableResponse();
		if (!SUCCESS.equalsIgnoreCase(synopticalTableReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("Error calling getSynopticalTable service",
					CommonErrorCodes.E_GEN_INTERNAL_ERR, HttpStatusCode.INTERNAL_SERVER_ERROR);
		...
		buildSynopticalTableResponse(synopticalTableResponse, synopticalTableReply);
		buildVoyageStatusResponse(voyageStatusResponse, voyageStatusRequest, portId, bunkerResponse,
				synopticalTableResponse, synopticalTableReply);
		return voyageStatusResponse;
	}

	private void buildVoyageStatusResponse(VoyageStatusResponse voyageStatusResponse, VoyageStatusRequest request,
			Long portId, OnHandQuantityResponse bunkerResponse, SynopticalTableResponse synopticalTableResponse,
			SynopticalTableReply synopticalTableReply) {
		String operationType = request.getOperationType();
		voyageStatusResponse.setCargoTanks(createGroupWiseTankList(synopticalTableReply.getCargoTanksList()));
		...
		if (synopticalTableResponse != null
			...
			Optional<SynopticalRecord> synopticalRecord = synopticalTableResponse.getSynopticalRecords().stream()
					.filter(record -> operationType.equalsIgnoreCase(record.getOperationType())
			...
			if (synopticalRecord.isPresent()) {
				if (!CollectionUtils.isEmpty(synopticalRecord.get().getCargos())) {
					...
					synopticalRecord.get().getCargos().stream().filter(o -> !o.getAbbreviation().equals(""))
							.forEach(item -> {
								Cargo cargo = new Cargo();
					...
					cargos.forEach(cargodata -> {
						if (cargoMap.containsKey(cargodata.getAbbreviation())) {
							...
							cargodat.setPlannedWeight(cargodat.getPlannedWeight().add(cargodata.getPlannedWeight()));
				...
				stabilityConditions.setCalculatedTrimActual(synopticalRecord.get().getCalculatedTrimActual());
				stabilityConditions.setCalculatedDraftAftActual(synopticalRecord.get().getCalculatedDraftAftActual());
				stabilityConditions.setCalculatedDraftMidActual(synopticalRecord.get().getCalculatedDraftMidActual());
				stabilityConditions.setCalculatedDraftFwdActual(synopticalRecord.get().getCalculatedDraftFwdActual());
				stabilityConditions.setCalculatedTrimPlanned(synopticalRecord.get().getCalculatedTrimPlanned());
				stabilityConditions.setCalculatedDraftAftPlanned(synopticalRecord.get().getCalculatedDraftAftPlanned());
				stabilityConditions.setCalculatedDraftFwdPlanned(synopticalRecord.get().getCalculatedDraftFwdPlanned());
				stabilityConditions.setCalculatedDraftMidPlanned(synopticalRecord.get().getCalculatedDraftMidPlanned());
		...
		voyageStatusResponse
				.setBallastFrontTanks(createGroupWiseTankList(synopticalTableReply.getBallastFrontTanksList()));
		voyageStatusResponse
				.setBallastCenterTanks(createGroupWiseTankList(synopticalTableReply.getBallastCenterTanksList()));
		voyageStatusResponse
				.setBallastRearTanks(createGroupWiseTankList(synopticalTableReply.getBallastRearTanksList()));
	}

	public SaveCommentResponse saveLoadOnTop(LoadOnTopRequest request, String correlationId, Long loadableStudyId)
		...
		if (!SUCCESS.equals(reply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("failed to save LoadOnTop", reply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(reply.getResponseStatus().getHttpStatusCode())));
		}
		SaveCommentResponse response = new SaveCommentResponse();
		response.setResponseStatus(new CommonSuccessResponse(String.valueOf(HttpStatus.OK.value()), correlationId));
	 ...
	 * @return AlgoPatternResponse
	 */
	public AlgoPatternResponse patternValidateResult(Long loadablePatternId,
			PatternValidateResultRequest patternValidateResultRequest, String correlationId)
			throws GenericServiceException {
		...
		try {
			...
			StatusReply reply = this.saveJson(loadablePatternId, LOADABLE_PATTERN_VALIDATE_RESULT_JSON_ID,
		...
		if (patternValidateResultRequest.getValidated()) {
			...
			loadablePlanRequest
					.setLoadablePlanDetails(Arrays.asList(patternValidateResultRequest.getLoadablePlanDetails()));
			buildLoadablePlanDetails(loadablePlanRequest, request);
		} else {
		...
		if (!SUCCESS.equals(algoReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("Failed to patternValidateResult from grpc service",
					algoReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(algoReply.getResponseStatus().getCode())));
		}
		algoPatternResponse
	 ...
	 * @param request void
	 */
	private void buildAlgoError(List<AlgoError> list,
			com.cpdss.common.generated.LoadableStudy.LoadablePatternAlgoRequest.Builder request) {
		list.forEach(error -> {
	 ...
	 * @throws GenericServiceException
	 */
	public VoyageResponse getVoyageList(Long vesselId, String correlationId, Map<String, String> filterParams, int page,
			int pageSize, String fromStartDate, String toStartDate, String orderBy, String sortBy)
			throws GenericServiceException {
		...
		for (VoyageDetail detail : list) {
			...
			if (detail.getLoadingPortsList() != null) {
				detail.getLoadingPortsList().forEach(port -> {
			...
			if (detail.getDischargingPortsList() != null) {
				detail.getDischargingPortsList().forEach(port -> {
			...
			if (detail.getCargosList() != null) {
				detail.getCargosList().forEach(crgo -> {
					Cargo cargo = new Cargo();
			...
			if (!detail.getActualStartDate().isEmpty()) {
				actualStartDate = LocalDateTime
						.from(DateTimeFormatter.ofPattern(VOYAGE_DATE_FORMAT).parse(detail.getActualStartDate()));
			...
			if (!detail.getActualEndDate().isEmpty()) {
				...
				actualEndDate = LocalDateTime
						.from(DateTimeFormatter.ofPattern(VOYAGE_DATE_FORMAT).parse(detail.getActualEndDate()));

				voyage.setActualEndDate(actualEndDate.toLocalDate());
			}

			if (!detail.getStartDate().isEmpty()) {
				plannedStartDate = LocalDateTime
						.from(DateTimeFormatter.ofPattern(VOYAGE_DATE_FORMAT).parse(detail.getStartDate()));

				voyage.setPlannedStartDate(plannedStartDate.toLocalDate());
			}

			if (!detail.getEndDate().isEmpty()) {
				plannedEndDate = LocalDateTime
						.from(DateTimeFormatter.ofPattern(VOYAGE_DATE_FORMAT).parse(detail.getEndDate()));
	...
	private List<VoyageDetail> getFilteredValues(Map<String, String> filterParams, List<VoyageDetail> voyageList) {
		return voyageList.stream().filter(voyage -> {
			Boolean status = true;
			if (null != filterParams.get("voyageNo")) {
				status = status
						&& voyage.getVoyageNumber().toLowerCase().contains(filterParams.get("voyageNo").toLowerCase());
			}
			if (null != filterParams.get("charterer")) {
				status = status
						&& voyage.getCharterer().toLowerCase().contains(filterParams.get("charterer").toLowerCase());
			}
			if (null != filterParams.get("status")) {
				status = status && voyage.getStatus().toLowerCase().contains(filterParams.get("status").toLowerCase());
			}
			if (null != filterParams.get("plannedStartDate")) {
				status = status && voyage.getStartDate().toLowerCase()
						.contains(filterParams.get("plannedStartDate").toLowerCase());
			}
			if (null != filterParams.get("plannedEndDate")) {
				status = status
						&& voyage.getEndDate().toLowerCase().contains(filterParams.get("plannedEndDate").toLowerCase());
			}
			if (null != filterParams.get("actualStartDate")) {
				status = status && voyage.getActualStartDate().toLowerCase()
						.contains(filterParams.get("actualStartDate").toLowerCase());
			}
			if (null != filterParams.get("actualEndDate")) {
				status = status && voyage.getActualEndDate().toLowerCase()
						.contains(filterParams.get("actualEndDate").toLowerCase());
			}

			if (null != filterParams.get("loadingPorts")) {
				status = status && containsLoadingPorts(voyage.getLoadingPortsList(), filterParams.get("loadingPorts"));
			}
			if (null != filterParams.get("dischargingPorts")) {
				status = status && containsDischargingPorts(voyage.getDischargingPortsList(),
						filterParams.get("dischargingPorts"));
			}
			if (null != filterParams.get("cargos")) {
				status = status && containsCargos(voyage.getCargosList(), filterParams.get("cargos"));
			}
			return status;
		}).collect(Collectors.toList());
	}
	...
	private List<Voyage> getSortedList(List<Voyage> voyageList, String orderBy, String sortBy) {
		...
		case "voyageno":
			if (orderBy.equalsIgnoreCase("ASC")) {
				voyages = voyageList.stream()
						.sorted(Comparator.comparing(Voyage::getVoyageNo, String.CASE_INSENSITIVE_ORDER))
						.collect(Collectors.toList());
			} else {
				voyages = voyageList.stream()
						.sorted(Comparator.comparing(Voyage::getVoyageNo, String.CASE_INSENSITIVE_ORDER).reversed())
						.collect(Collectors.toList());
			}
			break;
		case "charterer":
			if (orderBy.equalsIgnoreCase("ASC")) {
				...
				voyages = voyageList.stream()
						.sorted(Comparator.comparing(Voyage::getCharterer, String.CASE_INSENSITIVE_ORDER).reversed())
						.collect(Collectors.toList());
			}
			break;
		case "status":
			if (orderBy.equalsIgnoreCase("ASC")) {
				voyages = voyageList.stream().filter(voyage -> !StringUtils.isEmpty(voyage.getStatus()))
						.sorted(Comparator.comparing(Voyage::getStatus, String.CASE_INSENSITIVE_ORDER))
						.collect(Collectors.toList());

				voyages.addAll(voyageList.stream().filter(voyage -> StringUtils.isEmpty(voyage.getStatus()))
						.collect(Collectors.toList()));

			} else {
				voyages = voyageList.stream()
						.sorted(Comparator.comparing(Voyage::getStatus, String.CASE_INSENSITIVE_ORDER).reversed())
						.collect(Collectors.toList());
			}
			break;
		case "plannedstartdate":
			if (orderBy.equalsIgnoreCase("ASC")) {
				voyages = voyageList.stream().filter(voyage -> Objects.nonNull(voyage.getPlannedStartDate()))
						.sorted(Comparator.comparing(Voyage::getPlannedStartDate)).collect(Collectors.toList());
				voyages.addAll(voyageList.stream().filter(voyage -> Objects.isNull(voyage.getPlannedStartDate()))
						.collect(Collectors.toList()));
		...
		case "plannedenddate":
			if (orderBy.equalsIgnoreCase("ASC")) {
				voyages = voyageList.stream().filter(voyage -> Objects.nonNull(voyage.getPlannedEndDate()))
						.sorted(Comparator.comparing(Voyage::getPlannedEndDate)).collect(Collectors.toList());
				voyages.addAll(voyageList.stream().filter(voyage -> Objects.isNull(voyage.getPlannedEndDate()))
						.collect(Collectors.toList()));
			} else {
				voyages = voyageList.stream().filter(voyage -> Objects.nonNull(voyage.getPlannedEndDate()))
		...
		case "actualstartdate":
			if (orderBy.equalsIgnoreCase("ASC")) {
				voyages = voyageList.stream().filter(voyage -> Objects.nonNull(voyage.getActualStartDate()))
						.sorted(Comparator.comparing(Voyage::getActualStartDate)).collect(Collectors.toList());
				voyages.addAll(voyageList.stream().filter(voyage -> Objects.isNull(voyage.getActualStartDate()))
						.collect(Collectors.toList()));
			} else {
				voyages = voyageList.stream().filter(voyage -> Objects.nonNull(voyage.getActualStartDate()))
		...
		case "actualenddate":
			if (orderBy.equalsIgnoreCase("ASC")) {
				voyages = voyageList.stream().filter(voyage -> Objects.nonNull(voyage.getActualEndDate()))
						.sorted(Comparator.comparing(Voyage::getActualEndDate)).collect(Collectors.toList());
				voyages.addAll(voyageList.stream().filter(voyage -> Objects.isNull(voyage.getActualEndDate()))
						.collect(Collectors.toList()));
			} else {
				voyages = voyageList.stream().filter(voyage -> Objects.nonNull(voyage.getActualEndDate()))
						.sorted(Comparator.comparing(Voyage::getActualEndDate).reversed()).collect(Collectors.toList());
				voyages.addAll(voyageList.stream().filter(voyage -> Objects.isNull(voyage.getActualEndDate()))
						.collect(Collectors.toList()));
			}
			break;
	 ...
	 */
	public byte[] downloadLoadablePlanReport(Long vesselId, Long loadableStudyId, Long loadablePatternId)
			throws GenericServiceException {

		com.cpdss.common.generated.LoadableStudy.LoadablePlanReportRequest loadablePlanReportRequest = com.cpdss.common.generated.LoadableStudy.LoadablePlanReportRequest
				.newBuilder().setVesselId(vesselId).setLoadableStudyId(loadableStudyId)
				.setLoadablePatternId(loadablePatternId).build();

		// Get loadable plan report
		com.cpdss.common.generated.LoadableStudy.LoadablePlanReportReply loadablePlanReportReply = getLoadablePlanReport(
				loadablePlanReportRequest);

		if (!SUCCESS.equals(loadablePlanReportReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("Failed to generate loadable plan report",
					loadablePlanReportReply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.parseInt(loadablePlanReportReply.getResponseStatus().getCode())));
		}
	...
	public VoyageActionResponse saveVoyageStatus(VoyageActionRequest request, String correlationId)
		...
		if (!SUCCESS.equals(reply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("failed to save voyage status", reply.getResponseStatus().getCode(),
					HttpStatusCode.BAD_REQUEST);
		}
	 ...
	 * @throws GenericServiceException
	 */
	public CargoHistoryResponse getCargoHistory(CargoHistoryRequest request) throws GenericServiceException {
		...
		// Build cargoNomination payload for grpc call
		com.cpdss.common.generated.LoadableStudy.CargoHistoryRequest.Builder builder = com.cpdss.common.generated.LoadableStudy.CargoHistoryRequest
				.newBuilder();
		...
		com.cpdss.common.generated.LoadableStudy.CargoHistoryRequest cargoHistoryRequest = builder.build();
		CargoHistoryReply cargoHistoryReply = loadableStudyServiceBlockingStub
				.getCargoApiTempHistory(cargoHistoryRequest);
		if (!SUCCESS.equals(cargoHistoryReply.getResponseStatus().getStatus())) {
			if (!StringUtils.isEmpty(cargoHistoryReply.getResponseStatus().getCode())) {
				throw new GenericServiceException(
						"GenericServiceException getCargoHistory " + cargoHistoryReply.getResponseStatus().getMessage(),
						cargoHistoryReply.getResponseStatus().getCode(), HttpStatusCode
								.valueOf(Integer.valueOf(cargoHistoryReply.getResponseStatus().getHttpStatusCode())));
			} else {
				throw new GenericServiceException("GenericServiceException getCargoHistory",
						CommonErrorCodes.E_GEN_INTERNAL_ERR, HttpStatusCode.INTERNAL_SERVER_ERROR);
			}
		}
	 ...
	 * @return
	 */
	private CargoHistoryResponse buildCargoHistoryResponse(CargoHistoryRequest request,
		...
		if (reply != null && !reply.getCargoHistoryList().isEmpty()) {
			...
			reply.getCargoHistoryList().forEach(cargoHistoryDetail -> {
				...
				cargoHistory.setApi((cargoHistoryDetail.getApi() != null && cargoHistoryDetail.getApi().length() > 0
						&& !cargoHistoryDetail.getApi().trim().isEmpty()) ? new BigDecimal(cargoHistoryDetail.getApi())
								: new BigDecimal("0"));
				cargoHistory.setTemperature(
						(cargoHistoryDetail.getTemperature() != null && cargoHistoryDetail.getTemperature().length() > 0
			...
			List<CargoHistory> portHistoryList = cargoHistoryList.stream()
					.filter(var1 -> request.getLoadingPortIds().contains(var1.getLoadingPortId())).limit(5L)
					.collect(Collectors.toList());
			...
			Map<Integer, List<CargoHistoryDetail>> filteredMap = reply.getCargoHistoryList().stream()
					.collect(groupingBy(CargoHistoryDetail::getLoadedYear,
							mapping(Function.identity(), collectingAndThen(toList(),
									e -> e.stream().sorted(Comparator.comparingInt(CargoHistoryDetail::getLoadedMonth))
											.filter(distinctByKey(CargoHistoryDetail::getLoadedMonth))
											.collect(toList())))));
			filteredMap.entrySet().forEach(ch -> {
				log.info("Cargo history, api-temp: grouping info, cargo - {} year - {}, history size - {}",
						request.getCargoId(), ch.getKey(), ch.getValue().size());
			});
			if (!CollectionUtils.isEmpty(filteredMap.values())) {
				List<CargoHistoryDetail> filteredList = filteredMap.values().stream().flatMap(Collection::stream)
						.collect(toList());
				List<CargoHistory> monthlyCargoHistory = new ArrayList<CargoHistory>();
				filteredList.forEach(cargoHistoryDetail -> {
					if (request != null && !CollectionUtils.isEmpty(request.getLoadingPortIds())
						...
						cargoHistory
								.setApi(cargoHistoryDetail.getApi() != null && cargoHistoryDetail.getApi().length() > 0
										? new BigDecimal(cargoHistoryDetail.getApi())
										: new BigDecimal("0"));
						cargoHistory.setTemperature(cargoHistoryDetail.getTemperature() != null
	...
	/** @return Object */
	public Object test1() {
		EnvoyReader.EnvoyReaderResultRequest.Builder error = EnvoyReader.EnvoyReaderResultRequest.newBuilder();
		error.setClientId("KAZUSA");
		error.setMessageType("LOADABLESTUDY");
		error.setShipId("9513402");
		EnvoyReader.EnvoyReaderResultReply jsonResult = this.envoyReaderGrpcService
				.getResultFromCommServer(error.build());
		return null;
		/*
	...
	public Object test2() {
		...
		try {
			LoadablePlanRequest loadablePlanrequest = new Gson().fromJson(
					new String(Files.readAllBytes(
							Paths.get(this.rootFolder + "/json/loadableStudyResult_" + loadableStudiesId + ".json"))),
	...
	public void saveLoadingInfoToSynopticalTable(Long synopticalTableId, String sunriseTime, String sunsetTime)
			throws Exception {
		LoadingInfoSynopticalUpdateRequest.Builder builder = LoadingInfoSynopticalUpdateRequest.newBuilder();
		builder.setSynopticalTableId(synopticalTableId);
		builder.setTimeOfSunrise(sunriseTime);
		builder.setTimeOfSunset(sunsetTime);
		ResponseStatus response = this.loadableStudyServiceBlockingStub.saveLoadingInfoToSynopticData(builder.build());
		if (response.getStatus().equalsIgnoreCase("FAILED")) {
			throw new Exception("Failed to update synoptical table " + synopticalTableId);
		}
	}

	public RuleResponse getOrSaveRulesForLoadableStudy(Long vesselId, Long sectionId, Long loadableStudyId,
			RuleRequest loadableRuleRequest, String correlationId) throws GenericServiceException {
		...
		RuleUtility.buildRuleListForSave(loadableRuleRequest, null, loadableRuleRequestBuilder, null, false, false);
		LoadableRuleReply loadableRuleReply = loadableStudyServiceBlockingStub
				.getOrSaveRulesForLoadableStudy(loadableRuleRequestBuilder.build());
		if (!SUCCESS.equals(loadableRuleReply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("failed to get Vessel Details ",
		...
		ruleResponse.setResponseStatus(new CommonSuccessResponse(String.valueOf(HttpStatus.OK.value()), correlationId));
		return ruleResponse;
	}

	public LoadableStudyShoreResponse getLoadableStudyShore(String correlationId) throws GenericServiceException {
		com.cpdss.common.generated.LoadableStudy.LoadableStudyShoreRequest.Builder loadableRuleRequestBuilder = com.cpdss.common.generated.LoadableStudy.LoadableStudyShoreRequest
				.newBuilder();

		com.cpdss.common.generated.LoadableStudy.LoadableStudyShoreResponse responseShore = loadableStudyServiceBlockingStub
				.getLoadableStudyShore(loadableRuleRequestBuilder.build());
		List<LoadableStudyShore> shoreList = new ArrayList<LoadableStudyShore>();

		responseShore.getShoreListList().forEach(vesselDetail -> {
			...
			vesselDetail.getVoyagePortsList().forEach(voPorts -> {
				...
				ports.setPortOrder(voPorts.getPortOrder() == null ? "" : voPorts.getPortOrder());
				ports.setPortName(voPorts.getPortName() == null ? "" : voPorts.getPortName());
				ports.setAnchorage(voPorts.getAnchorage() == null ? "" : voPorts.getAnchorage());
				ports.setIconUrl(voPorts.getIconUrl() == null ? "" : voPorts.getIconUrl());
				ports.setPortType(voPorts.getPortType() == null ? "" : voPorts.getPortType());
	...
	public UllageBillReply saveRulesForLoadableStudy(String first, UllageBillRequest inputData)
		...
		com.cpdss.common.generated.LoadableStudy.UllageBillRequest.Builder builder = com.cpdss.common.generated.LoadableStudy.UllageBillRequest
				.newBuilder();

		com.cpdss.common.generated.LoadableStudy.BillOfLanding.Builder billOfLandingBuilder = com.cpdss.common.generated.LoadableStudy.BillOfLanding
				.newBuilder();

		com.cpdss.common.generated.LoadableStudy.UpdateUllage.Builder updateUllageBuilder = com.cpdss.common.generated.LoadableStudy.UpdateUllage
				.newBuilder();

		try {

			if (inputData.getBillOfLandingList().size() > 0) {
				inputData.getBillOfLandingList().forEach(billLanding -> {
					billOfLandingBuilder.setBblAt60F(billLanding.getBblAt60f() == null ? "" : billLanding.getBblAt60f())
							...
							.setBlRefNumber(billLanding.getBlRefNumber() == null ? "" : billLanding.getBlRefNumber())
							.setQuantityLt(
									billLanding.getQuantityLt() == null ? 0 : billLanding.getQuantityLt().longValue())
							.setKlAt15C(billLanding.getKlAt15c() == null ? 0 : billLanding.getKlAt15c().longValue())
							.setApi(billLanding.getApi() == null ? 0 : billLanding.getApi().longValue())
							.setTemperature(billLanding.getTemperature() == null ? 0 : billLanding.getApi().longValue())
							.setIsActive(billLanding.getIsActive() == null ? 0 : billLanding.getIsActive().longValue())
							.setVersion(billLanding.getVersion() == null ? 0 : billLanding.getVersion().longValue())
							.build();
				});
			} else {
			...
			if (inputData.getUllageUpdList().size() > 0) {
				inputData.getUllageUpdList().forEach(ullageList -> {
					updateUllageBuilder.setId(ullageList.getId() == null ? 0 : ullageList.getId())
							...
							.setQuantityMt(
									ullageList.getQuantityMt() == null ? 0 : ullageList.getQuantityMt().longValue())
							.setIsBallast(ullageList.getIsBallast() == null ? false : ullageList.getIsBallast())
							...
							.setObservedM3(ullageList.getSg() == null ? "" : ullageList.getSg()).build();
				});
	...
	public SimulatorJsonResponse getSimulatorJsonDataForLoadableStudy(Long vesselId, Long loadableStudyId,
			Long caseNumber, String correlationId) throws GenericServiceException {
		com.cpdss.common.generated.LoadableStudy.SimulatorJsonRequest.Builder requestBuilder = com.cpdss.common.generated.LoadableStudy.SimulatorJsonRequest
				.newBuilder();
		...
		com.cpdss.common.generated.LoadableStudy.SimulatorJsonReply reply = loadableStudyServiceBlockingStub
				.getLoadableStudySimulatorJsonData(requestBuilder.build());
		ArrivalDepartureConditionJson departureConditionJson = null;
		SimulatorJsonResponse jsonResponse = new SimulatorJsonResponse();
		try {
			departureConditionJson = new ObjectMapper().readValue(reply.getDepartureCondition(),
					ArrivalDepartureConditionJson.class);
			jsonResponse.setDepartureCondition(departureConditionJson);
			jsonResponse
					.setResponseStatus(new CommonSuccessResponse(String.valueOf(HttpStatus.OK.value()), correlationId));
		} catch (JsonProcessingException e) {
			jsonResponse.setResponseStatus(
					new CommonSuccessResponse(String.valueOf(HttpStatus.INTERNAL_SERVER_ERROR.value()), correlationId));
		}
