...
import com.cpdss.gateway.domain.filerepo.FileRepoRequest;
...
public class CommonController {
  ...
  public FileRepoGetResponse getFileRepoDetails(
    ...
    try {
      ...
      return fileRepoService.getFileRepoDetails(pageSize,pageNo,sortBy,orderBy,filterParams,headers.getFirst(CORRELATION_ID_HEADER));
  ...
  public FileRepoReply addFileToRepo(
          ...
          throws CommonRestException {
    try {
      return fileRepoService.addFileToRepo(file, voyageNo, fileName, fileType, section, category, headers.getFirst(CORRELATION_ID_HEADER));
  ...
  public ResponseEntity<Resource> getFileRepoFile(
    ...
    try {
      ...
      FileRepo fileRepo =
              this.fileRepoService.getFileRepoDetailsById(repoId);
      if (null != fileRepo) {
  ...
  public FileRepoReply editFileRepo(
          ...
          @RequestParam(name = "section", required = true) String section,
          @RequestParam(name = "category", required = true) String category,
          @RequestParam(name = "hasFileChanged", required = false, defaultValue = "true") Boolean hasFileChanged)
          throws CommonRestException {
    try {
      return fileRepoService.editFileRepo(repoId, file, section, category, hasFileChanged, headers.getFirst(CORRELATION_ID_HEADER));
