...
public class UllageReportFileParsingService {
	  ...
	  public ListOfUllageReportResponse importUllageReportFile(
		    ...
		    if (originalFileName != null && !(originalFileName.substring(originalFileName.lastIndexOf(".") + 1).toLowerCase())
		            .equals("xlsx")
		    ...
		    ListOfUllageReportResponse response = parseUllageReportFile(
		    		tankDetails, infoId, cargoNominationId, file, isLoading, vesselId);
	   ...
	   * @throws GenericServiceException
	   */
	  private ListOfUllageReportResponse parseUllageReportFile(String tankDetails, Long infoId,
			   Long cargoNominationId, MultipartFile file,boolean isLoading, Long vesselId)
	  		throws GenericServiceException {
	  	  ...
	  	  try (Workbook workbook = WorkbookFactory.create(new ByteArrayInputStream(file.getBytes()))){
	  		  ...
	  		  List<TankDetailsRequest> list = Arrays.asList(mapper.readValue(tankDetails, TankDetailsRequest[].class));
	   ...
	   */
	  	private boolean fetchCellValuesFromExcel(
	  			Cell cell, int rowCell, Map<String,Long> tankDetailMap,UllageReportImportResponse innerResponse) {
	  	 ...
	  	 * @param tankDetailMap
	  	 */
	  	private void getTankNameFromCell(Cell cell, UllageReportImportResponse innerResponse,
	  			Map<String,Long> tankDetailMap) {
	  		if (!cell.getCellType().equals(CellType.STRING)) {
                  throw new IllegalStateException(CommonErrorCodes.E_CPDSS_ULLAGE_INVALID_TANK);
              }
    			String substring = cell.getStringCellValue().substring(cell.getStringCellValue().indexOf(' ') + 1);
    				innerResponse.setTank(substring);
	  	...
	  	private void getTemperatureFromCell(Cell cell, UllageReportImportResponse innerResponse) {
	  		if(cell.getCellType().equals(CellType.BLANK)) {		    	    	    
  	   	    	innerResponse.setTemperature(0.0);
  		      			...
  		      			try {
  		      				innerResponse.setTemperature(Double.valueOf(
  		      					cell.getStringCellValue().trim().substring(
  		      							0, cell.getStringCellValue().trim().indexOf(' '))));
  		      			} catch(Exception e) {
	  	 ...
	  	 * @throws IOException
	  	 */
	  	private void saveUllageReportFileToSystemLocation(Long infoId, boolean isLoading, Workbook workbook, Long vesselId) 
	  			throws IOException {
	  		  String userId = AppContext.getCurrentUserId() == null ? "Unknown"  : AppContext.getCurrentUserId();
	  		  String fileNamePrefix = isLoading ? "Loading_" : "Discharging_";
	  		  String fileName = fileNamePrefix+"Ullage_Report_"+LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd"))+"_"+
	  				infoId+"_"+userId+".xls";
