...
public class DischargeStudyService {
	...
	public DischargeStudyResponse getDischargeStudyByVoyage(Long vesselId, Long voyageId, Long dischargeStudyId,
		...
		if (patternReply != null && patternReply.getResponseStatus() != null
			...
			LoadableStudy.LoadableCommingleDetailsReply loadableCommingleDetailsReply = loadableStudyServiceBlockingStub
					.getLoadableCommingleByPatternId(loadablePlanRequest.build());
			...
			loadableCommingleDetailsReply.getLoadableQuantityCommingleCargoDetailsList().forEach(lqcd -> {
				LoadableQuantityCommingleCargoDetails details = loadableStudyService
						.getLoadableQuantityCommingleCargoDetails(lqcd);
	...
	public PortRotationResponse getDischargeStudyPortDataByVoyage(Long vesselId, Long voyageId, Long dischargeStudyId,
		...
		response = loadableStudyService.getLoadableStudyPortRotationList(vesselId, voyageId, dischargeStudyId,
				Common.PLANNING_TYPE.DISCHARGE_STUDY, correlationId);
		return response;
	}
	...
	public DischargeStudyCargoResponse getDischargeStudyCargoByVoyage(Long vesselId, Long voyageId,
		...
		LoadableStudy.CargoNominationRequest cargoNominationRequest = LoadableStudy.CargoNominationRequest.newBuilder()
				.setLoadableStudyId(dischargeStudyId).build();
	...
	public PortWiseCargoResponse getCargosByPorts(Long dischargeStudyId, HttpHeaders headers)
		...
		if (!SUCCESS.equals(dischargeStudyPortWiseCargos.getResponseStatus().getStatus())) {
			throw new GenericServiceException("failed to get Port Rotation",
					dischargeStudyPortWiseCargos.getResponseStatus().getCode(), HttpStatusCode
							.valueOf(Integer.valueOf(dischargeStudyPortWiseCargos.getResponseStatus().getCode())));
		}
	...
	private void createPortCargoRequest(PortRotation portCargo,
		...
		dsBackLoadingDetail.addAllPortCargoDetails(createPortWiseCargoNoination(portCargo.getCargoNominationList()));
		builder.addDsBackLoading(dsBackLoadingDetail.build());
	}

	private List<CargoNominationDetail> createPortWiseCargoNoination(List<CargoNomination> cargoNominationList) {
