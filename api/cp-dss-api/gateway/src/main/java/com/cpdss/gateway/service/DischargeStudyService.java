...
public class DischargeStudyService {
	...
	private LoadingInformationServiceGrpc.LoadingInformationServiceBlockingStub loadingInfoServiceBlockingStub;

	@GrpcClient("loadingInformationService")
	private LoadingPlanServiceBlockingStub loadingPlanServiceBlockingStub;

	@GrpcClient("loadableStudyService")
	private LoadableStudyServiceGrpc.LoadableStudyServiceBlockingStub loadableStudyServiceBlockingStub;

	@GrpcClient("loadableStudyService")
	private DischargeStudyOperationServiceGrpc.DischargeStudyOperationServiceBlockingStub dischargeStudyOperationServiceBlockingStub;
	 ...
	 */
	public DischargeStudyResponse getDischargeStudyByVoyage(Long vesselId, Long voyageId, Long dischargeStudyId,
			String correlationId) throws GenericServiceException {
		log.info("Inside getDischargeStudyByVoyage gateway service with correlationId : " + correlationId);
		LoadableStudy.LoadableStudyRequest.Builder loadableStudyRequest = LoadableStudy.LoadableStudyRequest
				.newBuilder();
		loadableStudyRequest.setVoyageId(voyageId);
		LoadableStudy.LoadablePatternConfirmedReply patternReply = loadableStudyServiceBlockingStub
				.getLoadablePatternByVoyageAndStatus(loadableStudyRequest.build());
		System.out.println(patternReply);
		if (patternReply != null && patternReply.getResponseStatus() != null
				&& SUCCESS.equalsIgnoreCase(patternReply.getResponseStatus().getStatus())) {
			LoadingInformationSynopticalRequest.Builder requestBuilder = LoadingInformationSynopticalRequest
					.newBuilder();
			requestBuilder.setLoadablePatternId(patternReply.getPattern().getLoadablePatternId());
			LoadingInformationSynopticalReply grpcReply = loadingInfoServiceBlockingStub
					.getLoadigInformationByVoyage(requestBuilder.build());
			System.out.println(grpcReply);
			if (!SUCCESS.equals(grpcReply.getResponseStatus().getStatus())) {
				throw new GenericServiceException("Failed to fetch getDischargeStudyByVoyage",
						grpcReply.getResponseStatus().getCode(), HttpStatusCode.INTERNAL_SERVER_ERROR);
			}
			LoadableStudy.LoadablePlanDetailsRequest.Builder loadablePlanRequest = LoadableStudy.LoadablePlanDetailsRequest
					.newBuilder();
			...
			LoadablePlanCommingleCargoDetailsReply loadableCommingleDetailsReply = loadingPlanServiceBlockingStub
					.getLoadingPlanCommingleDetails(requestBuilder.build());
			...
			if (!SUCCESS.equals(loadableCommingleDetailsReply.getResponseStatus().getStatus())) {
				throw new GenericServiceException("Failed to fetch getDischargeStudyByVoyage",
						loadableCommingleDetailsReply.getResponseStatus().getCode(), HttpStatusCode
								.valueOf(Integer.valueOf(loadableCommingleDetailsReply.getResponseStatus().getCode())));
			}
			dischargeStudyResponse
					.setLoadableQuantityCommingleCargoDetails(new ArrayList<LoadableQuantityCommingleCargoDetails>());
			loadableCommingleDetailsReply.getLoadablePlanCommingleCargoListList().forEach(lqcd -> {
				LoadableQuantityCommingleCargoDetails details = this.buildCommingleCargoDetails(lqcd);
			...
			throw new GenericServiceException("Failed to fetch Confirmed lS",
					patternReply.getResponseStatus().getCode(), HttpStatusCode.BAD_REQUEST);
		}
	}

	private LoadableQuantityCommingleCargoDetails buildCommingleCargoDetails(LoadablePlanCommingleCargoDetails lqccd) {
		...
		Optional.ofNullable(lqccd.getQuantity1MT()).ifPresent(value -> details.setQuantity1MT(new BigDecimal(value)));
		Optional.ofNullable(lqccd.getQuantity2MT()).ifPresent(value -> details.setQuantity2MT(new BigDecimal(value)));
		Optional.ofNullable(lqccd.getQuantity1M3()).ifPresent(value -> details.setQuantity1M3(new BigDecimal(value)));
		Optional.ofNullable(lqccd.getQuantity2M3()).ifPresent(value -> details.setQuantity2M3(new BigDecimal(value)));
	...
	private DischargeStudyResponse buildDischargeStudyResponse(LoadingInformationSynopticalReply grpcReply,
			DischargeStudyResponse dischargeStudyResponse) {
		ModelMapper modelMapper = new ModelMapper();

		ArrayList<BillOfLadding> billOfLaddingValue = new ArrayList<>();
		grpcReply.getBillOfLaddingList().forEach(billOfLadding -> {
	...
	public PortRotationResponse getDischargeStudyPortDataByVoyage(Long vesselId, Long voyageId, Long dischargeStudyId,
			String correlationId, HttpHeaders headers) throws GenericServiceException {
		PortRotationResponse response = new PortRotationResponse();

		response = loadableStudyService.getLoadableStudyPortRotationList(vesselId, voyageId, dischargeStudyId,
				Common.PLANNING_TYPE.DISCHARGE_STUDY, correlationId, headers);
		return response;
	}
	...
	public PortRotationResponse savePortRotation(PortRotation request, String correlationId, HttpHeaders headers)
		...
		LoadableStudy.PortRotationDetail portRotationDetail = loadableStudyService.createPortRotationDetail(request,
				headers);
		LoadableStudy.PortRotationReply grpcReply = loadableStudyService.savePortRotation(portRotationDetail);
		if (!SUCCESS.equals(grpcReply.getResponseStatus().getStatus())) {
	...
	public PortRotationResponse deletePortRotation(Long loadableStudyId, Long id, String correlationId)
			throws GenericServiceException {
		LoadableStudy.PortRotationRequest request = LoadableStudy.PortRotationRequest.newBuilder().setId(id)
	...
	public LoadableStudyResponse saveDischargeStudy(DischargeStudyRequest request, String correlationId)
			throws GenericServiceException {
	...
	private UpdateDischargeStudyReply updateDischargeStudy(DischargeStudyDetail dischargeStudyDetail) {
		return this.dischargeStudyOperationServiceBlockingStub.updateDischargeStudy(dischargeStudyDetail);
	}

	public DischargeStudyUpdateResponse updateDischargeStudy(@Valid DischargeStudyRequest request, String correlationId,
			Long dischargeStudyId) throws GenericServiceException {
		...
		response.setDischargeStudy(new DischargeStudyValue(reply.getDischargeStudy().getId(),
				reply.getDischargeStudy().getName(), reply.getDischargeStudy().getEnquiryDetails()));
		response.setResponseStatus(new CommonSuccessResponse(String.valueOf(HttpStatus.OK.value()), correlationId));
		return response;
	}

	public OnHandQuantityResponse getOnHandQuantity(Long vesselId, Long dischargeStudyId, Long portRotationId,
			String correlationId) throws GenericServiceException {
		LoadableStudy.OnHandQuantityRequest request = LoadableStudy.OnHandQuantityRequest.newBuilder()
				.setVesselId(vesselId).setLoadableStudyId(dischargeStudyId).setPortRotationId(portRotationId).build();
	...
	public DischargeStudyResponse deleteDischargeStudy(Long dischargeStudyId, String correlationId)
		...
		com.cpdss.common.generated.loadableStudy.LoadableStudyModels.DischargeStudyRequest.Builder builder = com.cpdss.common.generated.loadableStudy.LoadableStudyModels.DischargeStudyRequest
				.newBuilder();
		...
		if (!SUCCESS.equals(reply.getResponseStatus().getStatus())) {
			throw new GenericServiceException("failed to delete discharge study", reply.getResponseStatus().getCode(),
					HttpStatusCode.valueOf(Integer.valueOf(reply.getResponseStatus().getHttpStatusCode())));
		}
	...
	private DischargeStudyReply deleteDischargeStudy(
			com.cpdss.common.generated.loadableStudy.LoadableStudyModels.DischargeStudyRequest dischargeStudyRequest) {
	...
	public DischargeStudyCargoResponse getDischargeStudyCargoByVoyage(Long vesselId, Long voyageId,
			Long dischargeStudyId, String correlationId) throws GenericServiceException {
		...
		log.info("Inside getDischargeStudyCargoByVoyage gateway service with correlationId : " + correlationId);
		LoadableStudy.PortRotationRequest portRotationRequest = LoadableStudy.PortRotationRequest.newBuilder()
				.setLoadableStudyId(dischargeStudyId).build();
		LoadableStudy.PortRotationReply portRotationReply = loadableStudyService.getPortRotation(portRotationRequest);
		if (SUCCESS.equals(portRotationReply.getResponseStatus().getStatus())) {

			LoadableStudy.CargoNominationRequest cargoNominationRequest = LoadableStudy.CargoNominationRequest
					.newBuilder().setLoadableStudyId(dischargeStudyId).build();
			LoadableStudy.CargoNominationReply cargoNominationReply = loadableStudyServiceBlockingStub
			...
			if (!SUCCESS.equals(cargoNominationReply.getResponseStatus().getStatus())) {
				throw new GenericServiceException("failed to get Port Rotation",
						...
						HttpStatusCode.valueOf(Integer.valueOf(cargoNominationReply.getResponseStatus().getCode())));
			}
	...
	private void buildDischargeStudyCargoResponse(DischargeStudyCargoResponse response,
			LoadableStudy.PortRotationReply portRotationReply, LoadableStudy.CargoNominationReply grpcReply) {
		Map<Long, List<LoadableStudy.CargoNominationDetail>> portIdsToCargoNominationMap = grpcReply
				.getCargoNominationsList().stream()
				.flatMap(cargoNomination -> cargoNomination.getLoadingPortDetailsList().stream()
						.map(port -> new AbstractMap.SimpleEntry<>(cargoNomination, port)))
				.collect(Collectors.groupingBy(cargoNomination -> cargoNomination.getValue().getPortId(),
						Collectors.mapping(Map.Entry::getKey, Collectors.toList())));

		List<PortRotationDetail> discharginPorts = portRotationReply.getPortsList().stream()
				.filter(port -> port.getOperationId() == 2).collect(Collectors.toList());
		discharginPorts.forEach(port -> {
			PortRotation portRotation = new PortRotation();
			portRotation.setPortId(port.getPortId());
			portRotation.setId(port.getId());
			portRotation.setMaxDraft((isEmpty(port.getMaxDraft()) || port.getMaxDraft().equals("null")) ? null
			...
			portRotation.setFreshCrudeOilTime(
					port.getFreshCrudeOilTime().isEmpty() ? null : new BigDecimal(port.getFreshCrudeOilTime()));
			portRotation.setDischargeRate(new BigDecimal(0));
			if (portIdsToCargoNominationMap.containsKey(port.getPortId())) {
				List<LoadableStudy.CargoNominationDetail> cargoNominationDetailList = portIdsToCargoNominationMap
						.get(port.getPortId());
	...
	private List<BackLoading> buildBackLoading(
		...
		backLoadingList.forEach(loading -> {
	...
	private void buildCargoNomination(List<LoadableStudy.CargoNominationDetail> cargoNominationDetailList,
			PortRotation portRotation) {
		List<CargoNomination> cargoNominations = new ArrayList<>();
		cargoNominationDetailList.stream().forEach(cargoNominationDetail -> {
			...
			Optional<LoadingPortDetail> nominationPort = cargoNominationDetail.getLoadingPortDetailsList().stream()
					.filter(port -> portRotation.getPortId().equals(port.getPortId())).findFirst();
			...
			cargoNomination.setTemperature(new BigDecimal(cargoNominationDetail.getTemperature()));
			cargoNomination.setMaxQuantity(
					cargoNominationDetail.getMaxQuantity() != null && !cargoNominationDetail.getMaxQuantity().isBlank()
	...
	public PortWiseCargoResponse getCargosByPorts(Long dischargeStudyId, HttpHeaders headers)
		...
		com.cpdss.common.generated.loadableStudy.LoadableStudyModels.DischargeStudyRequest.Builder request = com.cpdss.common.generated.loadableStudy.LoadableStudyModels.DischargeStudyRequest
				.newBuilder();
		request.setDischargeStudyId(dischargeStudyId);
		DishargeStudyCargoReply dischargeStudyPortWiseCargos = dischargeStudyOperationServiceBlockingStub
				.getDischargeStudyPortWiseCargos(request.build());
		if (SUCCESS.equals(dischargeStudyPortWiseCargos.getResponseStatus().getStatus())) {
			...
			dischargeStudyPortWiseCargos.getPortCargosList().stream().forEach(portCargo -> {
		...
		response.setResponseStatus(
				new CommonSuccessResponse(valueOf(HttpStatus.OK.value()), headers.getFirst("correlationId")));
		return response;
	}

	public LoadableStudyResponse saveDischargeStudyWithBackloaing(DischargeStudyCargoResponse request,
			String correlationId) throws GenericServiceException {
		DishargeStudyBackLoadingSaveRequest.Builder builder = DishargeStudyBackLoadingSaveRequest.newBuilder();
		builder.setDischargeStudyId(request.getDischargeStudyId());
		request.getPortList().forEach(portCargo -> {
			createPortCargoRequest(portCargo, builder);
		});
		DischargeStudyReply reply = this.dischargeStudyOperationServiceBlockingStub
				.saveDischargeStudyBackLoading(builder.build());
	...
	private void createPortCargoRequest(PortRotation portCargo,
			com.cpdss.common.generated.LoadableStudy.DishargeStudyBackLoadingSaveRequest.Builder builder) {
		DishargeStudyBackLoadingDetail.Builder dsBackLoadingDetail = DishargeStudyBackLoadingDetail.newBuilder();
		PortRotationDetail.Builder portDetails = PortRotationDetail.newBuilder();
		portDetails.setId(portCargo.getId());
		portDetails.setMaxDraft((portCargo.getMaxDraft() == null) ? null : portCargo.getMaxDraft().toString());
...
//    }
		...
		dsBackLoadingDetail.addAllPortCargoDetails(createPortWiseCargoNomination(portCargo.getCargoNominationList()));
		builder.addDsBackLoading(dsBackLoadingDetail.build());
	}

	private List<CargoNominationDetail> createPortWiseCargoNomination(List<CargoNomination> cargoNominationList) {
		List<CargoNominationDetail> cargoList = new ArrayList<>();
		cargoNominationList.forEach(cargo -> {
	...
	private List<com.cpdss.common.generated.loadableStudy.LoadableStudyModels.BackLoading> createBackLoading(
			List<BackLoading> backLoading) {
		List<com.cpdss.common.generated.loadableStudy.LoadableStudyModels.BackLoading> backLoadingList = new ArrayList<>();
		backLoading.forEach(backLoad -> {
			com.cpdss.common.generated.loadableStudy.LoadableStudyModels.BackLoading.Builder backLoadRequest = com.cpdss.common.generated.loadableStudy.LoadableStudyModels.BackLoading
					.newBuilder();
	...
	public AlgoPatternResponse generateDischargePatterns(Long vesselId, Long voyageId, Long dischargeStudyId,
			String correlationId) throws GenericServiceException {
		log.info("Inside generateDischargePatterns gateway service with correlationId : " + correlationId);
	...
	public DischargeStudyCargoResponse getDischargePatternDetails(Long loadableStudyId, Long vesselId,
			String correlationId) throws GenericServiceException {
		...
		CargoNominationReply cargos = dischargeStudyOperationServiceBlockingStub
				.getDischargePlanDetails(request.build());
		LoadableStudy.PortRotationRequest portRotationRequest = LoadableStudy.PortRotationRequest.newBuilder()
				.setLoadableStudyId(loadableStudyId).build();
		LoadableStudy.PortRotationReply portRotationReply = loadableStudyService.getPortRotation(portRotationRequest);
		if (!SUCCESS.equals(portRotationReply.getResponseStatus().getStatus())) {
	...
	public CommonResponse confirmPlan(Long dischargeStudyId, Long dischargePatternId, String correlationId)
			throws GenericServiceException {
		...
		ConfirmPlanReply grpcReply = this.dischargeStudyOperationServiceBlockingStub.confirmPlan(request.build());
		if (!SUCCESS.equals(grpcReply.getResponseStatus().getStatus())) {
